...
该版本只使用现有 textured mesh / Blender render，包含三行：

| 子图 | 内容 | 主 claim | 写法建议 |
|---|---|---|---|
| A | Vine family, depth `d=1..4` + branch / tip zoom | 在同一 vine family 与固定 guide / texture / renderer 下，depth 改变整体分枝尺度和局部末端结构。 | “same-condition recursive depth control with mesh-level closeups” |
| B | Connected coral scaffold, depth `d=1..4` + stage-4 local zoom | depth 控制不局限于藤蔓/树状外观；connected scaffold 也出现逐级扩展的几何复杂度。 | “connected scaffold depth behavior”，不要写成真实珊瑚物理模拟。 |
| C | Coral density parameter `lambda={0.45,0.70,0.95,1.20}` + low/high zoom | 固定 stage 与 octopus material guide 后，参数改变局部紧凑度和分布。 | “parameter-control case display”，不要写成强消融或严格单调响应。 |

如果主文版面允许，这张比旧的三行候选更直接回应 “mesh/textured mesh、same-condition、depth/parameter、zoom-in panel” 的需求。
