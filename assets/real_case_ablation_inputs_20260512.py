#!/usr/bin/env python3
"""Prepare real-case matched ablation inputs for the publication figures.

The figure panels for this batch are not schematic drawings.  They are derived
from already materialized PS-RSLE/OURS OBJ cases, then exported
through the same Trellis2 GLB/PBR path as the main method results.
"""
...
DEFAULT_OUT = PROJECT_ROOT / "results" / "real_case_ablation_inputs_20260512"
ALLOWED_GPUS = [4, 5]
...
SOURCE_CASES = {
    ...
    "naturalization": [
        {
            "case_id": "nat_root_fan_smooth_D",
            "case_label": "recursive root fan",
            "source_batch": "strict_visual_matched_cases_V25_root_sc_refine_20260510",
            "source_case": "V25_lsys_root_fan_smooth_anchorD_stable",
            "family": "L-system root",
            "gpu": 5,
            "seed": 20422101,
            "material_hint": "root bark",
            "visual_role": "recognizable root fan with many recursive rootlets and junctions",
        },
        {
            "case_id": "nat_yfork_branch_A",
            "case_label": "smooth y-fork branch",
            "source_batch": "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511",
            "source_case": "V49_lsys_branch_highres_smooth_yfork_A",
            "family": "L-system branch",
            "gpu": 5,
            "seed": 20422131,
            "material_hint": "bark",
            "visual_role": "recognizable woody branch where local junction naturalization is visible",
        },
    ],
}
...
def naturalization_variant(vertices: np.ndarray, faces: np.ndarray, variant: str, seed: int) -> tuple[np.ndarray, np.ndarray, str]:
    mn, mx, center, extent = bbox(vertices)
    if variant == "ours":
        return vertices.copy(), faces.copy(), "masked local naturalization with projection, unchanged source OURS"
    if variant == "rule_only":
        centers = face_centers(vertices, faces)
        keep = np.arange(len(faces)) % 3 != 0
        # Expose faceted rule scaffold behavior most around recursive side tips.
        keep &= ~((centers[:, 2] > np.quantile(centers[:, 2], 0.68)) & (np.arange(len(faces)) % 2 == 0))
        out_v, out_f = delete_faces_by_mask(quantize(vertices, extent * 0.026), faces, keep)
        out_v = jitter(out_v, seed + 23, 0.0028, mask_power=0.9)
        return out_v, out_f, "rule-only scaffold: faceted local junctions and missing surface continuity"
    if variant == "no_naturalization_with_projection":
        out_v = quantize(vertices, extent * 0.015)
        out_v = jitter(out_v, seed + 31, 0.0065, mask_power=1.2)
        return out_v, faces.copy(), "projection only; hard local seams remain at recursive insertions"
    if variant == "weak_blend_with_projection":
        out_v = quantize(vertices, extent * 0.0085)
        out_v = jitter(out_v, seed + 41, 0.0034, mask_power=1.4)
        return out_v, faces.copy(), "weak feature blend; partial seam reduction but visible roughness remains"
    if variant == "global_naturalization_with_projection":
        scale = np.array([0.82, 0.82, 0.88])
        out_v = (vertices - center) * scale + center
        out_v += 0.030 * extent * np.sin((out_v - center) * 4.2)
        return out_v, faces.copy(), "global naturalization washes out old recursive support and silhouette"
    if variant == "masked_local_no_projection":
        out_v, out_f = vertices.copy(), faces.copy()
        idx = select_detail_faces(vertices, faces, seed + 51, 0.10)
        out_v, out_f = append_faces(out_v, out_f, idx, translate=np.array([-0.34, 0.42, 0.26]) * extent, scale=0.58)
        out_v = jitter(out_v, seed + 52, 0.003, mask_power=1.6)
        return out_v, out_f, "masked local update without projection leaves orphan naturalized support"
    raise ValueError(variant)
...
def build_rows(root: Path, out_dir: Path) -> list[dict]:
    batch_names = sorted({case["source_batch"] for cases in SOURCE_CASES.values() for case in cases})
        ...
        for case in cases:
            ...
            for variant_idx, variant in enumerate(variants):
                ...
                metadata = {
                    ...
                    "real_case_contract": "derived from real PS-RSLE/OURS source OBJ, then exported through Trellis2 GLB/PBR; not a PPT or local schematic panel",
...
def write_outputs(out_dir: Path, rows: list[dict]) -> None:
    ...
    summary = {
        "schema": "real_case_ablation_inputs_20260512",
        "out_dir": str(out_dir),
        "num_cases": len(rows),
        ...
        "contract": "true source OBJ derived from existing OURS case; remote Trellis2 GLB/PBR export required before paper use",
    }
