...
RUN="${RUN:-strict_visual_matched_texture_v13b_implicit_coral_20260510}"
SEED="${SEED:-20260510}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_v13b_implicit_coral_20260510}"
...
echo "=== GENERATE V13b implicit coral/crystal OBJ inputs seed=$SEED $(date)"
"$PY" "$ROOT/assets/strict_visual_matched_cases_v13b_implicit_coral_20260510.py" \
  --root "$ROOT" --out "$INPUTS" --seed "$SEED"
echo "=== GENERATED V13b implicit coral/crystal $(date)"

for gpu in 4 5 6 7; do
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_v13b_implicit_coral_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_v13b_implicit_coral.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_v13b_implicit_coral:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_v13b_implicit_coral.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
