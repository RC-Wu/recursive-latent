...
DEFAULT_OUT = ROOT_DIR / "results" / "fern_depth_roots_20260512p"
...
def _build_compound_fern(seed: int, variant: str, depth: int) -> Tuple[pb.Mesh, Dict]:
    ...
    parents: List[int] = [-1]
    radii: List[float] = [0.086 if variant == "broad" else 0.078]
    rachis = [0]
    parent = 0
    ...
    for i in range(1, n + 1):
        ...
        parent = _append_child(nodes, parents, radii, parent, direction, 1.0, max(0.074 * (0.966**i), 0.020))
    ...
    for local_i, anchor in enumerate(rachis[2:-2], start=2):
        ...
        side_len = ((1.76 if variant == "broad" else 1.54) * envelope + 0.34) * (1.0 + 0.090 * min(depth, 6))
        for sign in (-1, 1):
            side_dir = np.array([sign * (1.18 + 0.05 * math.sin(local_i * 0.63)), 0.055 * math.cos(local_i * 0.48), 0.065 - 0.220 * t])
            p0 = _append_child(nodes, parents, radii, anchor, side_dir, side_len * 0.50, max(0.049 * (1.0 - 0.20 * t), 0.017))
            p1 = _append_child(nodes, parents, radii, p0, side_dir + np.array([0.0, 0.036 * sign, -0.10]), side_len * 0.38, max(0.037 * (1.0 - 0.22 * t), 0.012))
            ...
            if depth >= 2 and 0.16 < t < 0.88:
                ...
                for branch_i in range(branch_count):
                    ...
                    p2 = _append_child(nodes, parents, radii, p1, branch_dir, side_len * (0.34 if depth >= 6 else 0.29), max(0.027 * (1.0 - 0.14 * t), 0.0092))
    ...
    for k, (idx, sign, t, order) in enumerate(anchors):
        ...
        order_scale = 1.0 if order == 1 else (0.88 if order == 2 else 0.68)
        length = float(rng.uniform(0.165, 0.270) * (1.18 - 0.22 * t) * order_scale)
        width = length * float(rng.uniform(0.32, 0.45))
        _add_thick_lamina(mesh, centers[idx], normalized[idx], base, length, width, normal_hint, rows=8, thickness=0.0105, curl=0.044)
        counts["leaflet_count"] += 1
        if depth >= 4 and order == 1 and k % 5 == 2:
            _add_tube(mesh, centers, normalized, idx, _unit(0.72 * base + np.array([0.10 * sign, 0.0, -0.12])), length * 0.62, 0.0100, rng, curl=0.10, segments=5, sides=7)
            counts["tendril_count"] += 1
        if depth >= 6 and order == 2 and k % 4 == 1:
            _add_tube(mesh, centers, normalized, idx, _unit(0.64 * base + np.array([0.08 * sign, 0.0, -0.16])), length * 0.48, 0.0074, rng, curl=0.08, segments=4, sides=6)
            counts["tendril_count"] += 1
...
def _case_specs():
    return [
        ("fiddlehead_log_surface_recursive_u", "fiddlehead", "surface", 202617001),
        ("fiddlehead_log_surface_recursive_v", "fiddlehead", "dense_surface", 202617002),
        ("fern_mesh_frond_p", "fern", "broad", 202617003),
        ("fern_mesh_frond_q", "fern", "open", 202617004),
    ]


def _render_contact(out_dir: Path, rows: List[Dict]) -> Dict:
    out = out_dir / "fern_depth_roots_contact_sheet_20260512p.png"
    cmd = [
        sys.executable,
...
def main() -> None:
    ...
    summary = {
        ...
        "revision": "20260512p 12p stronger visible compound-fern hierarchy with stable 12o log-spiral controls",
    }
