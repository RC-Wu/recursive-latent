#!/usr/bin/env python3
"""V53 L-system branch natural-bough graph highres-yfork naturalization inputs.
...
dark terminal drops and occasional side fragments as the dominant read. V53
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511_dryrun"

SURFACE_STRATEGY = "v53_lsystem_branch_natural_bough_graph_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v53_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V53 still tapers
...
def _lsystem_branch_graph_v53(
    ...
    *,
    ...
    """Finite L-system branch with distributed natural-bough side branches.
    ...
    trunk surface. V53 spreads side branches along the main axis and chooses
    ...
    # V53 turns the terminal fix into graph topology.  V50's weak ellipsoid
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_distributed_natural_bough_yforks",
...
def _capsule_field_mesh(
    ...
    *,
    ...
    for k, idx in enumerate(anchors):
        ...
        child_dirs.sort(reverse=True, key=lambda item: item[0])
        v45._stamp_ball(field, coords, anchor, max(float(boosted_radii[idx]) * 1.02, 0.020), 0.40, cutoff=1.86)
        primitive_count += 1
        for _score, child_dir, _child in child_dirs[:3]:
            ...
            mid = anchor + blend_dir * float(0.040 + 0.010 * (k % 2))
            v45._stamp_ellipsoid(
                ...
                (
                    float(0.018 + 0.004 * detail_radius_scale),
                    float(0.015 + 0.003 * detail_radius_scale),
                    float(0.074 + 0.010 * lobe_scale),
                ),
                0.14,
                cutoff=1.60,
            )
            root = anchor + child_dir * 0.078
            v45._stamp_ellipsoid(
                field,
                coords,
                root,
                ...
                (
                    float(0.016 + 0.003 * detail_radius_scale),
                    float(0.019 + 0.004 * detail_radius_scale),
                    float(0.068 + 0.010 * lobe_scale),
                ),
                0.13,
                cutoff=1.58,
            )
...
def _highres_smooth_yfork_mesh_v53(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v53(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _highres_smooth_yfork_mesh_v53(
        nodes,
        parents,
        radii,
        rng,
    ...
    controls.update(
        {
            ...
            "v53_lsystem_branch_natural_bough_graph_yfork_naturalization": True,
            ...
            "v50_failure_addressed": "post-GLB connectivity remained strong, but white renders still showed dark terminal drops and occasional isolated side fragments; V53 moves terminal taper into graph topology before implicit field construction",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    palettes = {
        ...
        "natural_bough": [(104, 76, 48), (130, 92, 56), (158, 116, 72), (176, 138, 90)],
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V53_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    return {
        ...
        "grammar_guide": "v53_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v53_lsystem_branch_natural_bough_graph_yfork_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V53_lsys_branch_natural_bough_graph_yfork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.030, 0.786, 0.0058, 0.14, 0.30, 0.58, 0.070, 8, 1, 0, 0, 0, 0, 0, 0, 0.64, 0.58, "natural_bough", "natural-bough graph yfork A: smoother low/mid wood saddles with explicit three-step graph taper and very low-contrast ridges"),
        ("V53_lsys_branch_natural_bough_graph_yfork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.029, 0.794, 0.0054, 0.12, 0.28, 0.60, 0.070, 7, 1, 0, 0, 0, 0, 0, 0, 0.58, 0.52, "pale_cambium", "natural-bough graph lowfrag B: conservative connected wood fork with three-step graph taper, pale material guide, and reduced ridge contrast"),
        ("V53_lsys_branch_natural_bough_graph_yfork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.031, 0.780, 0.0060, 0.14, 0.30, 0.58, 0.068, 9, 1, 0, 0, 0, 0, 0, 0, 0.66, 0.60, "warm_cedar", "natural-bough graph dense C: stronger branch silhouette with explicit three-step graph taper and muted warm-cedar guide"),
        ("V53_lsys_branch_natural_bough_graph_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.802, 0.0050, 0.11, 0.26, 0.62, 0.074, 6, 1, 0, 0, 0, 0, 0, 0, 0.54, 0.50, "pale_cambium", "natural-bough graph slim D: V40-D-like stability with higher resolution, three-step terminal graph taper, and pale material guide"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V53_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V53_lsystem_branch_natural_bough_graph_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511.py"
    metadata["v53_lsystem_branch_naturalization_contract"] = {
    ...
    metadata["v53_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V53 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V53 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v53_lsystem_explicit_highres_smooth_yfork_mask",
            }
        )
    (out_dir / "V53_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V53_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V53 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V53 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
