#!/usr/bin/env python3
"""V31 SC-tree compact-crown seam naturalization inputs.
...
V31 keeps the V30-C metric direction and replaces free branch/leaf detail with
thicker shared-vertex compact crown-entry pads.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v31_sc_tree_compact_crown_masked_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_predeclared_sc_tree_compact_crown_candidates_one_per_gpu"
MESH_PBR_POLICY = "obj_inputs_compact_crown_transition_guides_for_trellis2_glb_export_no_2d_backprojection_claim"
EXTERNAL_SUPPORT_MAX_SEGMENT_GATE = 0.18
...
def _add_compact_crown_pads(
    ...
    *,
    ...
    """Add thick, attached crown pads to replace fragile free twig clutter."""
    ...
    for k, idx in enumerate(anchors):
        ...
        center = v26._add_transition_lobe(
            ...
            anchor,
            outward,
            float(rng.uniform(0.074, 0.116)),
            float(rng.uniform(0.018, 0.030)) * float(radius_scale),
            rng,
            rings=5,
            sides=12,
        ...
        if k % 2 == 0:
            leaf_dir = v22._unit(0.50 * outward + 0.42 * side + np.array([0.0, 0.0, 0.08]))
            length = float(rng.uniform(0.052, 0.086)) * float(radius_scale)
            v22._add_lanceolate_leaf(
                mesh,
                centers[idx],
                anchor,
                leaf_dir,
                length,
                length * float(rng.uniform(0.40, 0.58)),
                rng,
                rows=5,
            )
            counts["leaf_count"] += 1
    return counts, {
        "compact_crown_pad_count": int(counts["flare_count"]),
        "compact_crown_pad_leaf_count": int(counts["leaf_count"]),
        "compact_crown_pad_anchor_count": int(len(set(anchors))),
        "compact_crown_pad_centers": pad_centers[:80],
    }
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("multistem", "occlusion", "braided", "softleaf"):
        path = guide_dir / f"V31_sc_tree_{key}_compact_crown_lowcontrast_guide.png"
...
def _build_sc_tree_case(
    ...
    *,
    ...
    pad_counts, pad_diag = _add_compact_crown_pads(
        mesh,
        nodes,
        parents,
        rng,
    ...
    counts = _merge_counts(base_counts, entry_counts, shell_counts, pad_counts, flare_counts, ridge_counts, split_counts, bud_counts)
    seam_centers = (
        list(entry_diag.get("entry_mask_centers", []))
        + list(shell_diag.get("occlusion_shell_centers", []))
        + list(pad_diag.get("compact_crown_pad_centers", []))
    ...
    controls = v22._finalize_controls(
        {
            ...
            "grammar_rule": "space-colonization growth rerooted as compact crown-entry mass, then masked junction naturalization",
            **reroot_diag,
            **subdiv_diag,
        },
        nodes,
    ...
    controls.update(
        {
            ...
            "v31_sc_tree_compact_crown_naturalization": True,
            "v24_v30_failure_addressed": "visible branch/crown seam, smooth inserted rod, hard terminal cap, sparse branch lattice read, and post-GLB tiny fragments",
            "v31_base_from_v30": "V30_sc_tree_lowdetail_connected_entry_C metric direction with thicker compact crown-entry pads",
            "masked_local_naturalization_target": "compact crown-entry mass and terminal caps",
            "mask_scope": "object_space_compact_crown_entry_band_and_terminal_caps_only",
            "seam_mask_space": "object_xyz",
            "seam_mask_selection_rule": "short entry hubs plus compact crown pads, occlusion shell, and branch/crown junction flares; terminal caps tracked separately",
            ...
            "compact_crown_pad_count": int(pad_diag.get("compact_crown_pad_count", 0)),
            "compact_crown_pad_leaf_count": int(pad_diag.get("compact_crown_pad_leaf_count", 0)),
            ...
            "material_transition_policy": "object-space bark-to-olive-to-leaf compact crown transition; Trellis guide remains whole-object low-contrast",
            "hard_tube_cap_mitigation": "compact crown-entry mass, short-hub support, attached crown pads, entry sheaths, bark ridges, structural splits, and terminal buds",
        }
    )
    grammar = {
        ...
        "operator_to_traditional_mapping": {
            ...
            "compact_crown_entry_pad": "uses thick shared-vertex pads to read as crown mass instead of a sparse branch lattice",
...
def _spec(
    *,
    ...
    return {
        ...
        "recursive_mode": "tree-crown attractor competition with compact crown-entry mass and masked local naturalization",
        "mesh": mesh,
        "controls": controls,
        "guide_key": guide_key,
        "root_variant": root_variant,
        "grammar_guide": "v31_space_colonization_compact_crown_lowcontrast_tree_guide",
        ...
        "case_role": "v31_sc_tree_compact_crown_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V31_sc_tree_compact_entry_A", 4, 31001, 720, 252, 0.218, 0.066, 0.034, 0.08, 0.42, 6, 5, 0.118, 0.066, 0.034, 0.862, 0.0062, 0.78, 0.165, 12, 0, 18, 24, 12, 10, 10, 10, 0, 8, 2, 1.12, 0.88, "occlusion", "compact entry A: V30-C metric direction with fewer terminal twigs and thicker attached crown pads"),
        ("V31_sc_tree_crown_mass_B", 5, 31002, 760, 246, 0.224, 0.068, 0.034, 0.08, 0.46, 6, 5, 0.124, 0.070, 0.035, 0.860, 0.0065, 0.80, 0.165, 10, 0, 22, 28, 10, 8, 8, 8, 0, 6, 2, 1.18, 0.84, "softleaf", "crown mass B: largest compact crown-entry pads and the fewest free tips, targeting post-GLB r0 stability"),
        ("V31_sc_tree_lowfrag_shell_C", 6, 31003, 680, 238, 0.228, 0.070, 0.035, 0.07, 0.48, 7, 5, 0.116, 0.064, 0.034, 0.866, 0.0068, 0.82, 0.160, 8, 0, 16, 30, 8, 8, 8, 6, 0, 4, 0, 1.22, 0.82, "occlusion", "lowfrag shell C: most aggressive low-fragment candidate with broad attached pads and almost no free leaves or needles"),
        ("V31_sc_tree_balanced_leafmass_D", 7, 31004, 780, 260, 0.216, 0.066, 0.033, 0.09, 0.40, 6, 6, 0.128, 0.070, 0.034, 0.858, 0.0061, 0.78, 0.165, 12, 0, 20, 24, 12, 10, 10, 8, 0, 8, 2, 1.10, 0.90, "softleaf", "balanced leafmass D: keeps six short hubs but replaces sparse twig clutter with compact attached leaf mass"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V31_", ""),
                parameter_variant=(
                    ...
                    f"_entry{entry_sheath_count}_shell{occlusion_shell_count}_pad{compact_pad_count}_ridge{ridge_count}"
                ),
                reason=reason,
            )
        )
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V31_sc_tree_compact_crown_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511.py"
    metadata["v31_seam_naturalization_contract"] = {
        "target_failure": "V24/V26/V27/V28 branch-crown seam remains visually hard; V29 removes the long rod but leaves r0 fragments",
        "geometry_operator": "occluded short-hub entry plus shared-vertex subdivision, compact entry sheaths, occlusion shell, flares, bark ridges, splits, and terminal buds",
        "base_evidence": "V31 uses V29-D as the lower-fragment skeleton and borrows V29-B's crown-occlusion read",
        "seam_mask": {
            "space": "object_xyz",
    ...
    metadata["v31_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V31 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V31 short-hub segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v31_object_space_seam_mask",
            }
        )
    (out_dir / "V31_obj_zoom_manifest_seamtarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V31_obj_zoom_manifest_seamtarget_20260511.json"),
        "do_not_launch_remote": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V31 SC Tree Compact-Crown Naturalization Dry Run\n\n"
        "Predeclared compact crown-entry branch/crown seam candidates. V31 uses "
        "a short-hub subdivision gate, attached canopy occlusion shell, and "
