#!/usr/bin/env python3
"""V37 L-system branch continuous-junction naturalization inputs.

V35/V36 improved connectivity, but pure-white zoom QA still read as a large
cut branch tube with small side geometry attached near it. V37 keeps the same
strict branch-with-side-branches target and changes the local realization
toward a continuous saddle fork:
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V37_lsystem_branch_woody_junction_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v37_lsystem_branch_continuous_sidebranch_saddle_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v37_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
def _lsystem_branch_graph_v37(
    ...
    *,
    ...
    for step_i in range(1, int(main_steps) + 1):
        ...
        if should_branch:
            ...
            for sign in signs:
                ...
                junction_count += 1
        parent = current
        length *= float(rng.uniform(0.80, 0.88))
    ...
    return nodes, parents, {
        ...
        "target_silhouette": "single woody branch with recursive side branches and short tapered terminal",
    }
...
def _mid_junction_targets(points: List[List[float]], limit: int = 48) -> List[List[float]]:
    ...
    z = arr[:, 2]
    lo = float(np.quantile(z, 0.14))
    hi = float(np.quantile(z, 0.64))
    ...
    for point in arr:
        ...
        score = 1.5 * radial + 0.5 * mid_bonus - 0.18 * abs(float(point[2]) - hi)
...
def _add_continuous_saddle_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    rng = np.random.default_rng(seed + 3702)
    nodes, parents, lsys_diag = _lsystem_branch_graph_v37(
        seed,
    ...
    collar_counts, collar_diag = _add_continuous_saddle_collars(
        mesh,
        nodes,
        parents,
        rng,
    ...
    controls.update(
        {
            ...
            "v37_lsystem_branch_woody_junction_naturalization": True,
            ...
            "hard_tube_cap_mitigation": "shorter high-depth support, explicit saddle-neck side-branch bases, stronger parent-of-terminal taper, tiny terminal sleeves, and elongated bark ridges",
        }
    )
    grammar = {
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("cedar", "cambium", "olive", "ridge"):
        path = guide_dir / f"V37_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    return {
        ...
        "grammar_guide": "v37_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v37_lsystem_branch_continuous_saddle_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V37_lsys_branch_saddle_fork_A", 4, 37001, 7, 3, 2, True, 0.34, 0.70, 0.78, 0.030, 0.765, 0.0022, 0.105, 0.235, 0.335, 0.078, 28, 78, 14, 3, 10, 0, 0, 0, 1.02, 1.10, "cambium", "saddle fork A: short main terminal, explicit side-branch necks, collars and ridges without leaf clutter"),
        ("V37_lsys_branch_midfork_B", 5, 37002, 8, 2, 2, False, 0.30, 0.62, 0.72, 0.028, 0.775, 0.0022, 0.095, 0.220, 0.320, 0.078, 24, 84, 10, 2, 8, 0, 0, 0, 0.96, 1.04, "ridge", "midfork B: low-fragment wood-only backup emphasizing continuous bark ridges over terminal buds"),
        ("V37_lsys_branch_densefork_C", 4, 37003, 7, 3, 2, True, 0.38, 0.76, 0.82, 0.029, 0.755, 0.0020, 0.105, 0.235, 0.325, 0.076, 32, 70, 16, 3, 10, 0, 0, 0, 1.06, 1.14, "olive", "densefork C: strongest recursive side-branch hierarchy with a reduced non-dominant main tip"),
        ("V37_lsys_branch_slim_taper_D", 5, 37004, 8, 2, 2, False, 0.24, 0.58, 0.66, 0.026, 0.785, 0.0020, 0.085, 0.205, 0.305, 0.076, 22, 68, 8, 2, 6, 0, 0, 0, 0.90, 0.98, "cedar", "slim taper D: most conservative thin-branch candidate for avoiding blunt tube ends after GLB export"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V37_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V37_lsystem_branch_woody_junction_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V37_lsystem_branch_woody_junction_naturalization_20260511.py"
    metadata["v37_lsystem_branch_naturalization_contract"] = {
    ...
    metadata["v37_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V37 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V37 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v37_lsystem_object_space_junction_mask",
            }
        )
    (out_dir / "V37_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V37_lsystem_branch_woody_junction_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V37_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V37 L-system Branch Tapered Naturalization Dry Run\n\n"
        "V37 repairs the V35/V36 visual failure mode: connected metrics but pure-white zoom still read as a tube with attached side detail. "
