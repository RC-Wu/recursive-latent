#!/usr/bin/env python3
"""Prepare 20260512e fern/fiddlehead roots for publication-targeted recursion.

The 20260512d sweep was technically stable, but local Blender QA rejected it:
fiddlehead rows formed artificial ladder/rung structures under the coil, while
compound fern rows were clean but too close to no-op depth changes.  This
revision changes both root handles and the expected grammar contract:

- fiddleheads are coil-dominant croziers with buds distributed around the coil
  surface, avoiding a terminal underside band that can accumulate into rungs;
...
DEFAULT_OUT = ROOT_DIR / "results" / "fern_showcase_roots_20260512e"
...
def _solid_fiddlehead(seed: int, variant: str) -> Tuple[pb.Mesh, Dict]:
    ...
    parent = 0
    stem_steps = 1 if variant == "coil_surface" else 2
    ...
    center = stem_tip + np.array([0.06 if variant == "coil_surface" else 0.14, 0.0, -0.02])
    turns = 2.75 if variant == "coil_surface" else 3.15
    steps = 82 if variant == "coil_surface" else 92
    radius0 = 1.16 if variant == "coil_surface" else 1.24
    phase = -0.42 if variant == "coil_surface" else 0.10
    ...
    for i in range(1, steps + 1):
        ...
        p = center + np.array(
            [
                r * math.cos(theta),
                0.145 * math.sin(theta * 0.52),
                r * math.sin(theta) + 0.045 * t,
            ],
            dtype=float,
        )
        direction = p - prev
    ...
    stride = 4 if variant == "coil_surface" else 3
    for k, idx in enumerate(coil_indices[4:-3:stride]):
        ...
        tangent = _unit(0.34 * axis + 0.96 * radial + rng.normal(0.0, 0.012, 3))
        length = float(rng.uniform(0.085, 0.145) * (1.0 - 0.003 * k))
        _add_tube_bud(mesh, centers, normalized, idx, tangent, length, float(rng.uniform(0.0060, 0.0095)), rng, curl=0.14)
    ...
    return mesh, {
        ...
        "variant": variant,
        "seed": seed,
        "target": "coil-dominant fiddlehead crozier with surface-distributed recursive buds",
        "root_design": "short stem, large thick crozier coil, and attached radial tubular buds; no terminal underside strip handle",
        "leaflet_count": 0,
...
def _cases(seed: int):
    return [
        ("fiddlehead_surface_crozier_i", _solid_fiddlehead(seed + 41, "coil_surface")),
        ("fiddlehead_dense_crozier_j", _solid_fiddlehead(seed + 42, "dense_surface")),
...
def _render_contact(out_dir: Path, rows: List[Dict]) -> Dict:
    out = out_dir / "fern_showcase_roots_contact_sheet_20260512e.png"
    cmd = [
        sys.executable,
...
def main() -> None:
    ...
    summary = {
        ...
        "revision": "20260512e coil-surface crozier and hierarchical compound fern roots",
    }
