...
DEFAULT_OUT = ROOT_DIR / "results" / "fern_depth_roots_20260512m"
...
def _add_child_log_spiral(
    ...
    *,
    ...
    scale_base = (0.285 if variant == "open" else 0.255) * (0.94 ** (child_rank % 4))
    scale = scale_base * (1.0 + 0.045 * depth)
    turns = 0.82 + 0.12 * ((child_rank + depth) % 3)
    steps = 26 if depth <= 1 else 32
    r0 = 0.50
    r_min = 0.135
    anchor = np.asarray(main_points[idx], dtype=float)
    clearance = max(main_radii[idx] * 1.55, 0.024)
    child_center = anchor + radial * (clearance + 0.10 * scale) + normal * (side * rng.uniform(0.030, 0.078) * scale)
    theta0 = math.atan2(float(anchor[2] - center_hint[2]), float(anchor[0] - center_hint[0])) + side * rng.uniform(0.35, 0.72)
    child_points, _, _ = _log_spiral_points(
        ...
        scale=scale,
        center=child_center,
        phase=side * rng.uniform(0.18, 0.42) + 0.09 * child_rank,
    )
    ...
    transformed = []
    cling = 0.26 + 0.08 * rng.random()
    for q, p in enumerate(local):
        t = q / float(max(len(local) - 1, 1))
        # Bias each child curl along the parent tangent and toward the parent
        # surface so it reads as an attached botanical crozier, not a ring charm.
        transformed.append(
            child_center
            + radial * (p[0] * (1.0 - 0.20 * t) - cling * scale * t)
            + normal * (p[1] * (0.72 + 0.10 * math.sin(math.pi * t)))
            + tangent * (p[2] + side * 0.18 * scale * t)
        )
    child_points = transformed
    child_points[0] = anchor + radial * clearance
    child_radii = [max(main_radii[idx] * 0.32 * (0.972 ** i), 0.0062) for i in range(len(child_points))]
    child_mesh, child_centers = _curve_tube_from_points(child_points, child_radii, sides=9, ovality=0.10)
    ...
    _add_curved_tube_detail(
        ...
        max(main_radii[idx] * 0.32, 0.0058),
        rng,
        segments=2,
        sides=7,
        curl=0.0,
    )
    _bridge_vertices(mesh, main_centers[idx], child_centers[0], max(main_radii[idx] * 0.13, 0.0032))
...
def _build_log_fiddlehead(seed: int, variant: str, depth: int) -> Tuple[pb.Mesh, Dict]:
    ...
    base_count = 1 if variant == "open" else 2
    detail_count = base_count + int(depth) * (2 if variant == "open" else 2)
    arc_arr = np.asarray(arc_samples, dtype=float)
    target_arcs = np.linspace(arc_arr[18], arc_arr[-18], detail_count)
    selected = []
    for target in target_arcs:
        j = int(np.argmin(np.abs(arc_arr - target)))
        if 16 <= j < len(main_points) - 16 and (not selected or abs(j - selected[-1]) >= 11):
            selected.append(j)
...
def _case_specs():
    return [
        ("fiddlehead_log_organic_crozier_o", "fiddlehead", "open", 202614001),
        ("fiddlehead_log_organic_crozier_p", "fiddlehead", "dense", 202614002),
        ("fern_mesh_frond_k", "fern", "broad", 202614003),
        ("fern_mesh_frond_l", "fern", "open", 202614004),
    ]


def _render_contact(out_dir: Path, rows: List[Dict]) -> Dict:
    out = out_dir / "fern_depth_roots_contact_sheet_20260512m.png"
    cmd = [
        sys.executable,
...
def main() -> None:
    ...
    summary = {
        ...
        "revision": "20260512m per-depth connected roots",
    }
