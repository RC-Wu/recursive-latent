#!/usr/bin/env python3
"""V49 L-system branch highres-smooth-yfork naturalization inputs.
...
dropped.  V49 therefore keeps the same strict branch-with-side-branches target
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v49_lsystem_branch_highres_smooth_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v49_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
IMPLICIT_GRID_RESOLUTION = 176
IMPLICIT_FIELD_BOUND = 1.70
IMPLICIT_FIELD_LEVEL = 0.38
IMPLICIT_GAUSSIAN_SIGMA = 0.52
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V49 still tapers
...
def _capsule_field_mesh(
    ...
    *,
    ...
    return mesh, counts, {
        ...
        "low_relief_lobe_policy": "high-resolution smoothed field carries branch junction geometry; local swells are deliberately weak to avoid V48 blob/facet read",
    }


def _highres_smooth_yfork_mesh_v49(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v49(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _highres_smooth_yfork_mesh_v49(
        nodes,
        parents,
        radii,
        rng,
    ...
    controls.update(
        {
            ...
            "v49_lsystem_branch_highres_smooth_yfork_naturalization": True,
            ...
            "hard_tube_cap_mitigation": "generator-native highres smooth field with weaker junction swell, higher sampling density, and wood-only twig continuation; terminal sleeves and axis sleeve append geometry disabled",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V49_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    return {
        ...
        "grammar_guide": "v49_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v49_lsystem_branch_highres_smooth_yfork_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V49_lsys_branch_highres_smooth_yfork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.030, 0.786, 0.0058, 0.24, 0.42, 0.54, 0.070, 8, 4, 1, 0, 0, 0, 0, 0, 0.88, 0.82, "cambium", "highres-smooth-yfork A: smoother low/mid wood saddles with reduced V48 blob/facet swell and minimal ridges"),
        ("V49_lsys_branch_highres_smooth_yfork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.029, 0.794, 0.0054, 0.22, 0.40, 0.56, 0.070, 7, 3, 0, 0, 0, 0, 0, 0, 0.82, 0.76, "ridge", "highres-smooth-yfork lowfrag B: most conservative smooth connected wood-fork candidate with the weakest local swell"),
        ("V49_lsys_branch_highres_smooth_yfork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.031, 0.780, 0.0060, 0.24, 0.44, 0.54, 0.068, 9, 5, 1, 0, 0, 0, 0, 0, 0.92, 0.86, "cedar", "highres-smooth-yfork dense C: strongest Y-fork silhouette after V48, but with weakened junction swell and fewer ridge primitives"),
        ("V49_lsys_branch_highres_smooth_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.802, 0.0050, 0.22, 0.38, 0.58, 0.074, 6, 3, 0, 0, 0, 0, 0, 0, 0.78, 0.72, "olive", "highres-smooth-yfork slim D: V40-D-like stability with higher resolution and lower blob risk"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V49_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V49_lsystem_branch_highres_smooth_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511.py"
    metadata["v49_lsystem_branch_naturalization_contract"] = {
    ...
    metadata["v49_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V49 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V49 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v49_lsystem_explicit_highres_smooth_yfork_mask",
            }
        )
    (out_dir / "V49_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V49_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V49 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V49 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
