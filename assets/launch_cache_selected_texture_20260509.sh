...
RUN="${RUN:-cache_selected_texture_20260509}"
...
INPUT_ROOT="$ROOT/inputs/cache_sampler_selected_20260509"
mkdir -p "$LOG" "$OUT" \
...
declare -a CASES=(
  "4 cache_dla_masked_sampler cache_hard_dla_masked_sampler.obj octopus_suckers_square.png"
  "5 cache_radial_transform_fusion cache_hard_radial_transform_fusion.obj gear_train_square.png"
  "6 cache_bismuth_transform_fusion cache_hard_bismuth_transform_fusion.obj bismuth_crystal_square.png"
  "7 cache_scifi_connected cache_hard_scifi_naive_connected.obj gear_train_square.png"
)
...
for spec in "${CASES[@]}"; do
  ...
  script="$LOG/run_gpu${gpu}_${label}.sh"
  {
    ...
    printf 'ROOT=%q RUN=%q bash %q --worker %q %q %q %q\n' \
      "$ROOT" "$RUN" "$ROOT/assets/launch_cache_selected_texture_20260509.sh" \
