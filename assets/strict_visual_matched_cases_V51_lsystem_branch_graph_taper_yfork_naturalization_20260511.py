#!/usr/bin/env python3
"""V51 L-system branch graph-taper highres-yfork naturalization inputs.
...
dark terminal drops and occasional side fragments as the dominant read. V51
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V51_lsystem_branch_graph_taper_yfork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v51_lsystem_branch_graph_taper_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v51_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
IMPLICIT_GRID_RESOLUTION = 216
IMPLICIT_FIELD_BOUND = 1.70
IMPLICIT_FIELD_LEVEL = 0.405
IMPLICIT_GAUSSIAN_SIGMA = 0.58
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V51 still tapers
        ...
        if idx in terminal_lineage and idx not in terminals:
            ...
            radius *= min(1.0, float(tip_parent_shrink) + 0.035 * distance)
        if idx in terminals:
            radius = max(float(floor) * 0.58, radius * float(terminal_shrink))
        else:
...
def _lsystem_branch_graph_v51(
    ...
    *,
    ...
    # V51 turns the terminal fix into graph topology.  V50's weak ellipsoid
    # continuations still gave Trellis2 a round dark end-token after texturing.
    # Here every exposed side terminal receives two short taper nodes before
    # marching cubes, so the only remaining cap is tiny and branch-axis aligned.
    ...
    for terminal in initial_terminals:
        ...
        first = _append_node(nodes, parents, terminal, tip + axis * float(rng.uniform(0.042, 0.062)))
        _append_node(nodes, parents, first, np.asarray(nodes[first], dtype=float) + axis * float(rng.uniform(0.024, 0.038)))
        graph_terminal_taper_count += 2
...
def _capsule_field_mesh(
    ...
    *,
    ...
    for idx in range(len(boosted_radii)):
        ...
        if idx in terminals:
            boosted_radii[idx] = max(boosted_radii[idx] * 0.88, 0.0068)
    ...
    for k, idx in enumerate(ridge_sites[: max(0, int(ridge_count) + int(split_count))]):
        ...
        v45._stamp_ellipsoid(
            ...
            (
                float(0.0042 + 0.0012 * detail_radius_scale),
                float(0.0062 + 0.0014 * detail_radius_scale),
                float(0.036 + 0.008 * rng.random()),
            ),
            0.075,
            cutoff=1.35,
        )
    ...
    return mesh, counts, {
        ...
        "terminal_taper_source": "graph_native_taper_segments_no_terminal_ellipsoid_stamps",
        "sweep_sides": 0,
...
def _highres_smooth_yfork_mesh_v51(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v51(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _highres_smooth_yfork_mesh_v51(
        nodes,
        parents,
        radii,
        rng,
    ...
    controls.update(
        {
            ...
            "v51_lsystem_branch_graph_taper_yfork_naturalization": True,
            ...
            "terminal_taper_source": collar_diag.get("terminal_taper_source", "graph_native_taper_segments_no_terminal_ellipsoid_stamps"),
            ...
            "v50_failure_addressed": "post-GLB connectivity remained strong, but white renders still showed dark terminal drops and occasional isolated side fragments; V51 moves terminal taper into graph topology before implicit field construction",
            ...
            "hard_tube_cap_mitigation": "generator-native graph taper twigs plus highres smooth field; terminal ellipsoid stamps, terminal sleeves, cap balls, and axis sleeve append geometry are disabled",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V51_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    return {
        ...
        "grammar_guide": "v51_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v51_lsystem_branch_graph_taper_yfork_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V51_lsys_branch_graph_taper_yfork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.030, 0.786, 0.0058, 0.18, 0.36, 0.54, 0.070, 8, 3, 0, 0, 0, 0, 0, 0, 0.84, 0.74, "cambium", "graph-taper yfork A: smoother low/mid wood saddles with explicit terminal graph taper and minimal ridges"),
        ("V51_lsys_branch_graph_taper_yfork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.029, 0.794, 0.0054, 0.16, 0.32, 0.56, 0.070, 7, 2, 0, 0, 0, 0, 0, 0, 0.78, 0.68, "cambium", "graph-taper lowfrag B: conservative connected wood fork with graph-native terminal taper and lighter material guide"),
        ("V51_lsys_branch_graph_taper_yfork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.031, 0.780, 0.0060, 0.18, 0.34, 0.54, 0.068, 9, 3, 0, 0, 0, 0, 0, 0, 0.86, 0.78, "cedar", "graph-taper dense C: stronger branch silhouette with explicit terminal graph taper and reduced ridge primitives"),
        ("V51_lsys_branch_graph_taper_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.802, 0.0050, 0.15, 0.30, 0.58, 0.074, 6, 2, 0, 0, 0, 0, 0, 0, 0.74, 0.64, "cambium", "graph-taper slim D: V40-D-like stability with higher resolution, explicit terminal graph taper, and less dark material risk"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V51_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V51_lsystem_branch_graph_taper_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V51_lsystem_branch_graph_taper_yfork_naturalization_20260511.py"
    metadata["v51_lsystem_branch_naturalization_contract"] = {
    ...
    metadata["v51_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V51 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V51 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v51_lsystem_explicit_highres_smooth_yfork_mask",
            }
        )
    (out_dir / "V51_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V51_lsystem_branch_graph_taper_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V51_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V51 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V51 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
