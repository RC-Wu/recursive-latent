#!/usr/bin/env python3
"""V35 L-system branch naturalization inputs.

V34 was useful diagnostically but not visually final: the pre-export meshes
were connected, yet the white contact sheet still showed a blunt, tube-like
main branch and zoom targets sometimes landed on an uninformative terminal
cap.  V35 keeps the same strict branch-with-side-branches task and strengthens
the geometry-owned naturalization before any Trellis2 texturing:

* thinner high-depth support with parent-of-terminal taper,
* terminal sleeves and compact bud caps over exposed cuts,
* more bark-ridge breakup on the main branch,
* explicit junction-only zoom targets for local QA.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V35_lsystem_branch_tapered_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v35_lsystem_branch_tapered_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v35_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    mesh = v22._smooth_support_mesh(nodes, edges, radii, sides=13, ovality=0.22)
    ...
    junction_zoom_centers = list(collar_diag.get("seam_mask_centers", []))[:48]
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "target_silhouette": "tapered naturalized L-system branch with recursive side branches",
            ...
            "support_cross_section": "thinner_oval_nonround_bark_read",
            "support_visibility_policy": "main support remains visible but all tips and side-branch junctions are locally naturalized",
    ...
    controls.update(
        {
            ...
            "v35_lsystem_branch_tapered_naturalization": True,
            "v34_failure_addressed": "blunt tube-like main branch endpoint, uninformative terminal zoom target, and residual inserted-side-branch read",
            "masked_local_naturalization_target": "L-system side-branch junction bands plus terminal sleeves and compact buds",
            "mask_scope": "object_space_side_branch_junction_bands_and_terminal_caps_only",
            "seam_mask_space": "object_xyz",
            "seam_mask_selection_rule": "degree>=2 L-system junction nodes plus terminal sleeve anchors; no global resampling",
            ...
            "hard_tube_cap_mitigation": "thin high-depth support, parent-of-terminal taper, terminal sleeves, compact buds, bark ridges, and junction collars",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,junction_mask,cambium_collar,bark_ridge,terminal_sleeve,terminal_bud",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "terminal_bud": "turn terminal caps into compact connected buds",
        },
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("cedar", "cambium", "olive", "ridge"):
        path = guide_dir / f"V35_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    return {
        ...
        "recursive_mode": "finite L-system branch with recursive side branches, tapered tips, and terminal sleeves",
        "mesh": mesh,
        "controls": controls,
        "guide_key": guide_key,
        "root_variant": root_variant,
        "grammar_guide": "v35_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "case_role": "v35_lsystem_branch_tapered_naturalization",
        "qa_priority": "side_branch_junction_naturalness_terminal_taper_and_non_seam_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V35_lsys_branch_tapered_bud_A", 4, 35001, 9, 3, 2, True, 0.18, 0.70, 0.62, 0.052, 0.835, 0.0034, 0.26, 0.45, 0.62, 0.098, 24, 54, 18, 30, 32, 2, 2, 22, 1.02, 0.90, "cambium", "tapered bud A: direct V34 repair with thin upper support, terminal sleeves, and dense junction collars"),
        ("V35_lsys_branch_bark_sleeve_B", 5, 35002, 10, 2, 2, False, 0.15, 0.64, 0.56, 0.050, 0.842, 0.0032, 0.22, 0.42, 0.58, 0.096, 20, 64, 16, 34, 36, 1, 2, 12, 0.96, 0.84, "ridge", "bark sleeve B: low-fragment candidate with heavy bark ridges and compact terminal sleeves instead of leaf clutter"),
        ("V35_lsys_branch_fused_fork_C", 4, 35003, 8, 3, 2, True, 0.21, 0.76, 0.66, 0.050, 0.828, 0.0033, 0.24, 0.44, 0.60, 0.100, 24, 48, 20, 26, 30, 1, 2, 18, 1.08, 0.94, "olive", "fused fork C: strongest hierarchy candidate with paired side-branch collars and tapered main-tip closure"),
        ("V35_lsys_branch_lowfrag_taper_D", 5, 35004, 9, 2, 3, False, 0.12, 0.60, 0.52, 0.048, 0.850, 0.0036, 0.20, 0.40, 0.56, 0.094, 18, 46, 12, 26, 34, 0, 2, 8, 0.90, 0.78, "cedar", "lowfrag taper D: conservative two-GPU backup with few free leaves, strong taper, and terminal sleeves"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V35_", ""),
                parameter_variant=(
                    ...
                    f"_parent{tip_parent_shrink:.2f}_ridge{ridge_count}_sleeve{terminal_sleeve_count}"
                ),
                reason=reason,
            )
        )
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V35_lsystem_branch_tapered_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V35_lsystem_branch_tapered_naturalization_20260511.py"
    metadata["v35_lsystem_branch_naturalization_contract"] = {
        "target_failure": "V34 remained connected but still looked like a blunt pipe at the main branch endpoint and had unstable terminal-heavy zoom targets",
        "geometry_operator": "tip-parent taper, terminal sleeves, compact buds, short segmented swept support, object-space junction collars, bark ridges, and branch splits",
        "seam_mask": {
            "space": "object_xyz",
    ...
    metadata["v35_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V35 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V35 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v35_lsystem_object_space_junction_mask",
            }
        )
    (out_dir / "V35_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V35_lsystem_branch_tapered_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V35_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V35 L-system Branch Tapered Naturalization Dry Run\n\n"
        "V35 repairs the V34 visual failure mode: blunt pipe-like main tips and terminal-heavy zoom targets. "
        "It uses high-depth support taper, terminal sleeves, compact buds, junction-only zoom targets, "
        "object-space side-branch collars, bark ridges, structural splits, and whole-object low-contrast guides. "
