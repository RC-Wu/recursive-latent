...
RUN="${RUN:-strict_visual_matched_texture_V34_lsystem_branch_naturalization_20260511}"
SEED="${SEED:-20260511}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V34_lsystem_branch_naturalization_20260511}"
...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V34 L-system branch only allows GPU 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  ...
  while IFS='|' read -r label mesh guide seed _gpu; do
    ...
    echo "=== START $label gpu=$gpu steps=$steps tex=$tex seed=$seed v34_lsystem_branch $(date)"
...
echo "=== GENERATE V34 L-system branch OBJ inputs $(date)"
"$PY" "$ROOT/assets/strict_visual_matched_cases_V34_lsystem_branch_naturalization_20260511.py" \
  --root "$ROOT" --out "$INPUTS" --seed "$SEED"
echo "=== GENERATED V34 L-system branch OBJ inputs $(date)"
...
for gpu in ${V34_GPUS:-4 5}; do
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V34_GPUS must only contain 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_V34_lsystem_branch_naturalization_20260511.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_V34_lsystem_branch.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_V34_lsystem_branch:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_V34_lsystem_branch.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
