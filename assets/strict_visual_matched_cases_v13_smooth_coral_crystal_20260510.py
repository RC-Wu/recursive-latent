#!/usr/bin/env python3
"""V13 smooth implicit coral/crystal strict matched DLA input generator.

V12 scored well on connectivity metrics but failed visual review: the initial
inputs still read as cylinder rods with clipped ends and low surface complexity.
V13 keeps the strict DLA/frontier story in the controls - stochastic active
frontier attachment, occupancy exclusion, and one connected projection - but
changes the emitted surface. It builds an implicit metaball/SDF-like density
field from the active frontier skeleton, adds rounded terminal lobes, shallow
pores, ridge lines, and micro-branches, then exports a marching-cubes mesh.
No welded cylinder/tube mesh is used as the visual input surface.
"""
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_v13_smooth_coral_crystal_20260510_dryrun"
...
def _largest_component_mesh(mesh: pb.Mesh) -> tuple[pb.Mesh, int, float]:
    """Drop tiny marching-cubes numerical islands while preserving the main projection."""
    ...
    if len(components) <= 1:
        return mesh, len(components), 1.0
    largest = max(components, key=len)
    keep = set(largest)
    index_map = {old: new + 1 for new, old in enumerate(sorted(keep))}
    faces = []
    for face in mesh.faces:
        ids = [int(i) - 1 for i in face]
        if all(i in keep for i in ids):
            faces.append(tuple(index_map[i] for i in ids))
    vertices = [mesh.vertices[i] for i in sorted(keep)]
    ratio = len(largest) / max(len(mesh.vertices), 1)
    return pb.Mesh(vertices, faces), len(components), float(ratio)
...
def _mode_config(mode: str) -> dict:
    configs = {
        "staghorn": {
            ...
            "branch_prob": 0.78,
            "exclusion": 0.056,
            "anisotropy": np.array([0.92, 0.92, 1.22]),
            "noise": 0.155,
            "up_bias": 0.080,
            "base_radius": 0.092,
            "tip_radius": 0.018,
        ...
        "table": {
            ...
            "branch_prob": 0.72,
            "exclusion": 0.058,
            "anisotropy": np.array([1.28, 1.18, 0.40]),
            "noise": 0.135,
            "up_bias": 0.014,
            "base_radius": 0.086,
            "tip_radius": 0.016,
        ...
        "frontier": {
            ...
            "branch_prob": 0.66,
            "exclusion": 0.058,
            "anisotropy": np.array([1.65, 0.92, 0.30]),
            "noise": 0.120,
            "up_bias": 0.006,
            "base_radius": 0.078,
            "tip_radius": 0.014,
        ...
        "crystal": {
            ...
            "branch_prob": 0.58,
            "exclusion": 0.060,
            "anisotropy": np.array([1.12, 1.02, 0.58]),
            "noise": 0.052,
            "up_bias": 0.000,
            "base_radius": 0.072,
            "tip_radius": 0.014,
...
def _initial_dirs(mode: str) -> list[np.ndarray]:
    if mode == "table":
        return [_unit(np.array([math.cos(a), math.sin(a), 0.08])) for a in np.linspace(0, 2 * math.pi, 8, endpoint=False)]
    if mode == "frontier":
        xs = np.linspace(-1.0, 1.0, 8)
        return [_unit(np.array([x, 0.34 * math.sin(i * 1.7), 0.055])) for i, x in enumerate(xs)]
    if mode == "crystal":
        return [_unit(np.array([math.cos(a), math.sin(a), 0.20])) for a in np.linspace(0, 2 * math.pi, 7, endpoint=False)]
    return [_unit(np.array([math.cos(a), math.sin(a), 0.58])) for a in np.linspace(0, 2 * math.pi, 7, endpoint=False)]
...
def _frontier_skeleton(seed: int, mode: str) -> tuple[list[np.ndarray], list[tuple[int, int]], list[int], dict]:
    ...
    attempts = 0
    while attempts < 2400 and len(nodes) < int(cfg["target_nodes"]):
        if not tips:
            ...
            tips.append((parent_idx, _unit(base_dir + rng.normal(0.0, 0.42, 3)), depths[parent_idx] + 1))
        parent, direction, depth = tips.pop(0)
        attempts += 1
        if depth > int(cfg["max_depth"]):
            continue
        step = float(cfg["step"]) * (0.86 ** (depth - 1)) * rng.uniform(0.82, 1.14)
        ...
        exclusion = float(cfg["exclusion"]) * (0.91 ** max(depth - 1, 0))
        ...
        if depth < int(cfg["max_depth"]):
            ...
            if mode == "crystal" and rng.random() < 0.32:
                fan = 1
            for _ in range(fan):
                rot = rng.normal(0.0, 0.34 if mode != "crystal" else 0.16, 3)
    ...
    for i in terminal_ids:
        ...
        extension = 0.080 if mode != "crystal" else 0.068
...
def _radii_for_depths(depths: list[int], mode: str) -> list[float]:
    ...
    power = 1.18 if mode != "crystal" else 0.92
...
def _stamp_ball(
    ...
    mn = max(0, int(np.searchsorted(coords, center[0] - cutoff * r) - 1))
    mx = min(field.shape[0], int(np.searchsorted(coords, center[0] + cutoff * r) + 1))
    yn = max(0, int(np.searchsorted(coords, center[1] - cutoff * r) - 1))
    yx = min(field.shape[1], int(np.searchsorted(coords, center[1] + cutoff * r) + 1))
    zn = max(0, int(np.searchsorted(coords, center[2] - cutoff * r) - 1))
    zx = min(field.shape[2], int(np.searchsorted(coords, center[2] + cutoff * r) + 1))
    if mn >= mx or yn >= yx or zn >= zx:
        return
    x = coords[mn:mx, None, None]
    y = coords[yn:yx][None, :, None]
    z = coords[zn:zx][None, None, :]
    d2 = ((x - center[0]) ** 2 + (y - center[1]) ** 2 + (z - center[2]) ** 2) / (r * r)
    field[mn:mx, yn:yx, zn:zx] += float(strength) * np.exp(-0.5 * d2)
...
def _stamp_ellipsoid(
    ...
    mn = max(0, int(np.searchsorted(coords, center[0] - cutoff * max_scale) - 1))
    mx = min(field.shape[0], int(np.searchsorted(coords, center[0] + cutoff * max_scale) + 1))
    yn = max(0, int(np.searchsorted(coords, center[1] - cutoff * max_scale) - 1))
    yx = min(field.shape[1], int(np.searchsorted(coords, center[1] + cutoff * max_scale) + 1))
    zn = max(0, int(np.searchsorted(coords, center[2] - cutoff * max_scale) - 1))
    zx = min(field.shape[2], int(np.searchsorted(coords, center[2] + cutoff * max_scale) + 1))
    if mn >= mx or yn >= yx or zn >= zx:
        return
    x = coords[mn:mx, None, None] - center[0]
    y = coords[yn:yx][None, :, None] - center[1]
    z = coords[zn:zx][None, None, :] - center[2]
    u, v, w = axes
    du = (x * u[0] + y * u[1] + z * u[2]) / max(float(scales[0]), 1e-4)
    dv = (x * v[0] + y * v[1] + z * v[2]) / max(float(scales[1]), 1e-4)
    dw = (x * w[0] + y * w[1] + z * w[2]) / max(float(scales[2]), 1e-4)
    field[mn:mx, yn:yx, zn:zx] += float(strength) * np.exp(-0.5 * (du * du + dv * dv + dw * dw))
...
def _implicit_frontier_mesh(seed: int, mode: str) -> tuple[pb.Mesh, dict]:
    ...
    for edge_idx, (a, b) in enumerate(edges):
        ...
        if length < 1e-7:
            continue
        u, v, _ = _basis(axis)
        samples = max(5, int(length / 0.030))
        phase = edge_idx * 0.71 + rng.uniform(-0.45, 0.45)
        for s in range(samples + 1):
            t = s / max(samples, 1)
            center = pa * (1.0 - t) + pb_ * t
            wrinkle = 1.0 + 0.10 * math.sin(5.0 * t + phase) + 0.055 * math.sin(13.0 * t + phase * 0.37)
            if mode == "crystal":
                wrinkle = 0.96 + 0.06 * math.sin(4.0 * t + phase)
            radius = (float(radii[a]) * (1.0 - t) + float(radii[b]) * t) * wrinkle
            offset = (u * math.sin(2.0 * math.pi * t + phase) + v * math.cos(math.pi * t + phase)) * radius * 0.18
            _stamp_ball(field, coords, center + offset, radius, 1.08)
            metaball_samples += 1
    ...
    for idx in tips:
        ...
        for j, scale in enumerate((1.10, 0.82, 0.55)):
            center = np.asarray(nodes[idx]) + axis * (0.030 * (j + 1))
            _stamp_ball(field, coords, center, max(float(radii[idx]) * scale, 0.018), 1.16)
            metaball_samples += 1
        rounded_lobes += 1

    ridge_lines = 0
    ridge_step = max(len(edges) // 46, 1)
    for k, (a, b) in enumerate(edges[::ridge_step]):
        ...
        normal = _unit(v * side + np.array([0.0, 0.0, 0.18 if mode != "crystal" else 0.04]))
        for t in np.linspace(0.16, 0.84, 5):
            center = pa * (1.0 - t) + pb_ * t + normal * (0.018 + float(radii[a]) * 0.36)
            if mode == "crystal":
                _stamp_ellipsoid(field, coords, center, (u, normal, w), (0.058, 0.017, 0.035), 0.34)
            else:
                _stamp_ball(field, coords, center, 0.026, 0.34)
            metaball_samples += 1
        ridge_lines += 1

    micro_branches = 0
    rng.shuffle(tips)
    micro_limit = 42 if mode != "crystal" else 30
    for idx in tips[:micro_limit]:
        parent = parents[idx] if parents[idx] >= 0 else 0
        axis = _unit(np.asarray(nodes[idx]) - np.asarray(nodes[parent]))
        u, v, _ = _basis(axis)
        direction = _unit(axis * 0.72 + u * rng.normal(0.0, 0.34) + v * rng.normal(0.0, 0.34))
        anchor = np.asarray(nodes[idx])
        length = rng.uniform(0.050, 0.115 if mode != "crystal" else 0.085)
        for s in range(4):
            t = (s + 1) / 4.0
            radius = max(float(radii[idx]) * (0.58 - 0.34 * t), 0.010 if mode != "crystal" else 0.012)
            _stamp_ball(field, coords, anchor + direction * length * t, radius, 0.82)
            metaball_samples += 1
        micro_branches += 1

    pores = 0
    pore_step = max(len(edges) // 34, 1)
    for k, (a, b) in enumerate(edges[::pore_step]):
        ...
        if float(np.linalg.norm(axis)) < 1e-7:
            continue
        u, v, _ = _basis(axis)
        t = float(rng.uniform(0.24, 0.76))
        side = -1.0 if k % 2 else 1.0
        normal = _unit(v * side + u * rng.uniform(-0.25, 0.25))
        center = pa * (1.0 - t) + pb_ * t + normal * (float(radii[a]) * 0.95 + 0.012)
        pore_radius = 0.040 if mode != "crystal" else 0.033
        _stamp_ball(field, coords, center, pore_radius, -0.58 if mode != "crystal" else -0.46)
        pores += 1

    field = gaussian_filter(field, sigma=0.58 if mode != "crystal" else 0.42)
    level = 0.52 if mode != "crystal" else 0.50
    ...
    controls.update(
        {
            ...
            "surface_strategy": "implicit metaball union with subtractive pores, smoothed density, and marching cubes continuous coral/crystal surface",
            "connected_projection": "all field primitives are sampled from or overlap the single active-frontier skeleton",
            "v13_failure_addressed": "replaces V11/V12 round tube/cylinder rods, flat cut terminals, and low surface complexity with rounded implicit surfaces",
            "anti_chunk_policy": "no cube particles, no detached shard primitives, no local output selection",
        }
    )
...
def _write_guides(out_dir: Path) -> dict[str, str]:
    ...
    def save(name: str, bg: tuple[int, int, int], palette: list[tuple[int, int, int]], crystal: bool = False) -> str:
        ...
        for _ in range(470):
            ...
            if crystal:
                ...
                skew = int(rng.integers(-18, 20))
                draw.polygon([(x, y - r), (x + r + skew, y - r // 5), (x + r // 3, y + r), (x - r + skew, y + r // 6)], outline=c, width=2)
            else:
                width = int(rng.integers(7, 18))
                dx, dy = int(rng.normal(0, 112)), int(rng.normal(0, 92))
                draw.line((x, y, x + dx, y + dy), fill=c, width=width)
                if rng.random() < 0.48:
                    rr = int(rng.integers(10, 28))
                    draw.ellipse((x - rr, y - rr, x + rr, y + rr), outline=c, width=3)
                if rng.random() < 0.36:
                    draw.arc((x - 46, y - 34, x + 46, y + 34), start=int(rng.integers(0, 180)), end=int(rng.integers(181, 360)), fill=c, width=4)
    ...
    return {
        "staghorn": save("v13_smooth_coral_staghorn_guide.png", (238, 134, 126), [(255, 222, 180), (255, 170, 138), (246, 104, 118), (255, 236, 198), (198, 80, 104)]),
        "table": save("v13_smooth_table_coral_pores_guide.png", (224, 166, 122), [(255, 240, 202), (248, 202, 146), (224, 138, 104), (250, 220, 176), (168, 96, 88)]),
        "frontier": save("v13_smooth_frontier_sheet_coral_guide.png", (208, 132, 120), [(255, 224, 186), (238, 156, 124), (208, 88, 110), (250, 206, 166), (132, 82, 96)]),
        "crystal": save("v13_nonblocky_blue_gold_crystal_guide.png", (58, 70, 84), [(110, 148, 176), (188, 214, 220), (234, 210, 142), (82, 104, 130), (196, 174, 112)], crystal=True),
    }
...
def _case_specs(seed: int) -> list[dict]:
    ...
    def add(case_id: str, target: str, mode: str, guide_key: str, offset: int, gpu: int, priority: bool, why: str) -> None:
        ...
        operators = [
            ...
            "rounded_terminal_lobes",
            "subtractive_pores_and_dimples",
            "non_blocky_crystal_ridge_boundaries",
        ]
        specs.append(
            {
                "case_id": case_id,
    ...
    add("v13_dla_smooth_coral_staghorn_a", "dla_coral_cluster_900", "staghorn", "staghorn", 601, 4, True, "Strict DLA coral-cluster frontier/exclusion task; V13 emits an implicit marching-cubes coral surface to avoid V12 cylinder rods and clipped terminals.")
    add("v13_dla_smooth_coral_staghorn_b", "dla_coral_cluster_900", "staghorn", "staghorn", 602, 5, True, "Second smooth staghorn seed with the same stochastic frontier attachment and occupancy exclusion controls.")
    add("v13_dla_smooth_table_coral_a", "dla_coral_cluster_900", "table", "table", 603, 6, True, "Table-coral variant using continuous implicit surface, shallow pores, ridge lines, and rounded tapered tips.")
    add("v13_dla_smooth_table_coral_b", "dla_coral_cluster_900", "table", "table", 604, 7, True, "Second table-coral strict input for fresh remote generation, not local selection.")
    add("v13_dla_smooth_frontier_sheet_a", "dla_frontier_sheet_700", "frontier", "frontier", 605, 4, True, "Frontier sheet target with connected implicit plate-like boundary and non-blocky ridge detail.")
    add("v13_dla_smooth_frontier_sheet_b", "dla_frontier_sheet_700", "frontier", "frontier", 606, 5, True, "Second frontier sheet seed preserving stochastic attachment, occupancy exclusion, and connected projection.")
    add("v13_dla_smooth_crystal_ridge_a", "dla_crystal_cluster_520", "crystal", "crystal", 607, 6, True, "Crystal/DLA boundary case with smooth implicit body and ridge facets instead of detached block shards.")
    add("v13_dla_smooth_crystal_ridge_b", "dla_crystal_cluster_520", "crystal", "crystal", 608, 7, False, "Backup non-blocky crystal frontier seed with connected implicit ridges and rounded terminals.")
    return specs
...
def _metadata_for(spec: dict, mesh_path: Path, guide_path: str, metrics: dict) -> dict:
    ...
    return {
        ...
        "visual_readability_contract": {
            ...
            "v12_failure_addressed": "V13 replaces V11/V12 round cylinder rods, flat cut ends, and low-complexity tube surfaces with an implicit field surface",
            "frontier_structure": "single active-frontier skeleton projected into an implicit field with connected smooth surface, rounded tips, shallow pores, ridges, and micro-branches",
            "negative_constraint": "no cube block particles, no detached shards, no isolated sheet islands, no direct tube/cylinder mesh, no local result selection after generation",
        ...
        "v13_design_note": "implicit metaball/marching-cubes DLA/coral/crystal input algorithm, not a cylinder mesh or post-hoc repair pass",
        "root_selection_log": {
            "root_source_type": "v13_implicit_smooth_coral_crystal_input_generator",
            "source_generator": "assets/strict_visual_matched_cases_v13_smooth_coral_crystal_20260510.py",
            "root_pool_size": 1,
            "root_generation_budget": "local CPU dry-run geometry only; final strict case must be generated fresh on a100-2",
            "root_screening_budget": "no local manual cherry-pick and no V1-V12 post-processing",
            "selection_rank": 1,
            "projection_naturalization_schedule": spec["operators"],
            "readiness_label": "remote_input_dryrun_implicit_single_component",
...
def _write_readme(out_dir: Path, summary: dict) -> None:
    text = f"""# V13 Smooth Coral/Crystal Strict Visual-Matched Cases Dry Run

Generated by `assets/strict_visual_matched_cases_v13_smooth_coral_crystal_20260510.py`.

This is a local input dry-run only. It does not launch remote jobs, does not use
SSH, and does not pick or post-process V1-V12 outputs. Strict one-to-one visual
...
- Surface is implicit metaball density plus marching cubes, not a welded tube or
  cylinder mesh.
- Rounded tips, shallow pores, ridge lines, and connected micro-branches target
  the V12 visual failure.
...
def materialize(root: Path, out_dir: Path, seed: int = 20260510, case_limit: int | None = None) -> dict:
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "implicit_metaball_marching_cubes",
