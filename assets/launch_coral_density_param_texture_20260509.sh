...
RUN="${RUN:-coral_density_param_texture_20260509}"
...
INPUT_ROOT="$ROOT/inputs/connected_parameter_cases_20260509/$FAMILY"
...
for spec in "${CASES[@]}"; do
  ...
  script="$LOG/run_gpu${gpu}_${token}.sh"
  {
    ...
    echo "ROOT='$ROOT' RUN='$RUN' FAMILY='$FAMILY' GUIDE_NAME='$GUIDE_NAME' source '$ROOT/assets/launch_coral_density_param_texture_20260509.sh' --worker '$gpu' '$token'"
  } > "$script"
  chmod +x "$script"
