#!/usr/bin/env python3
"""Prepare connected per-depth logarithmic-spiral roots for 12n SLat naturalization.
...
12n keeps the user correction that this case must contain no long support/stem
rods.  It further changes 12m's "small hanging ring" child curls into short
surface-attached logarithmic spirals that start on the main spiral and cling to
its local tangent/radial plane.
"""
...
DEFAULT_OUT = ROOT_DIR / "results" / "fern_depth_roots_20260512n"
...
def _add_child_log_spiral(
    ...
    *,
    ...
    scale_base = (0.205 if variant == "surface" else 0.180) * (0.955 ** (child_rank % 5))
    scale = scale_base * (1.0 + 0.025 * depth)
    turns = 0.82 + 0.10 * ((child_rank + depth) % 4)
    steps = 30 if depth <= 2 else 34
    r0 = 0.62
    r_min = 0.105
    anchor = np.asarray(main_points[idx], dtype=float)
    clearance = max(main_radii[idx] * 0.40, 0.006)
    ...
    for q in range(steps):
        ...
        x = r * math.cos(theta)
        y = r * math.sin(theta)
        surface_pull = scale * (0.060 + 0.025 * rng.random()) * t
        child_points.append(
            anchor
            + radial * (clearance + scale * (x - start_x) - surface_pull)
            + tangent * (side * scale * (y - start_y) + side * scale * 0.050 * t)
            + normal * (scale * 0.022 * math.sin(math.pi * t) * math.sin(theta * 0.7))
        )
    child_points[0] = anchor + radial * clearance
    child_radii = [max(main_radii[idx] * 0.245 * (0.963 ** i), 0.0046) for i in range(len(child_points))]
    child_mesh, child_centers = _curve_tube_from_points(child_points, child_radii, sides=8, ovality=0.085)
    child_offset = _append_mesh_into_with_offset(mesh, child_mesh)
    child_centers = [child_offset + c for c in child_centers]
    _bridge_vertices(mesh, main_centers[idx], child_centers[0], max(main_radii[idx] * 0.105, 0.0028))


def _build_log_fiddlehead(seed: int, variant: str, depth: int) -> Tuple[pb.Mesh, Dict]:
    rng = np.random.default_rng(seed + depth * 97)
    turns = 2.72 if variant == "surface" else 3.02
    steps = 130 if variant == "surface" else 148
    radius0 = 1.34 if variant == "surface" else 1.42
    radius_min = 0.058 if variant == "surface" else 0.050
    theta0 = -0.18 if variant == "surface" else 0.10
    ...
    main_radii = [
        max((0.067 if variant == "surface" else 0.062) * math.exp(-0.72 * (i / max(steps - 1, 1))), 0.0145)
        for i in range(steps)
    ]
    mesh, centers = _curve_tube_from_points(main_points, main_radii, sides=18, ovality=0.055)
    counts = _counts()

    detail_count = int(depth) * (2 if variant == "surface" else 3)
    arc_arr = np.asarray(arc_samples, dtype=float)
    target_arcs = [] if detail_count <= 0 else np.linspace(arc_arr[24], arc_arr[-28], detail_count)
    selected = []
    for target in target_arcs:
        j = int(np.argmin(np.abs(arc_arr - target)))
        if 22 <= j < len(main_points) - 24 and (not selected or abs(j - selected[-1]) >= 7):
            selected.append(j)
    ...
    return mesh, {
        ...
        "root_design": "connected mesh-stage recursion; no long stem rods; main true log spiral plus short surface-attached self-similar log spirals",
...
def _case_specs():
    return [
        ("fiddlehead_log_surface_recursive_q", "fiddlehead", "surface", 202615001),
        ("fiddlehead_log_surface_recursive_r", "fiddlehead", "dense_surface", 202615002),
    ]


def _render_contact(out_dir: Path, rows: List[Dict]) -> Dict:
    out = out_dir / "fern_depth_roots_contact_sheet_20260512n.png"
    cmd = [
        sys.executable,
...
def main() -> None:
    ...
    summary = {
        ...
        "revision": "20260512n per-depth connected surface-recursive log-spiral roots",
    }
