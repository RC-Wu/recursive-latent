#!/usr/bin/env python3
"""V46 L-system branch continuous-sweep-fork naturalization inputs.
...
dropped.  V46 therefore keeps the same strict branch-with-side-branches target
but uses only a few low/mid explicit continuous sweep forks:

* two or three low/mid continuous-sweep-fork anchors, far from the terminal, are written
...
* mid-depth continuous-sweep-fork zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v46_lsystem_branch_continuous_sweep_fork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v46_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
SWEEP_SIDES = 24
JUNCTION_RADIUS_BOOST = 1.58
JUNCTION_NEIGHBOR_RADIUS_BOOST = 1.24
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V46 still tapers
...
def _lsystem_branch_graph_v46(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid continuous sweep forks.
    ...
    low/mid continuous sweep fork. The graph uses fewer free children and thicker
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_continuous_sweep_forks",
        ...
        "continuous_sweep_fork_anchor_index": int(main_indices[primary_step]),
        "continuous_sweep_fork_child_index": int(primary_child),
        "continuous_sweep_fork_anchor_indices": sorted(set(yfork_anchor_indices))[:16],
        "continuous_sweep_fork_branch_count": int(yfork_branch_count),
        "continuous_sweep_fork_zoom_pairs": yfork_pairs[:6],
...
def _continuous_sweep_fork_target_pairs(
...
def _continuous_sweep_fork_mesh_v46(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    ...
    *,
    ...
    for k, idx in enumerate(anchors):
        ...
        for _score, child_dir, child in child_dirs[:3]:
            ...
            for offset, length, radius, direction in (
                (0.030, 0.105, 0.0105, blend_dir),
                (0.082, 0.120, 0.0085, child_dir),
            ):
    ...
    for k, idx in enumerate(ridge_sites[: max(0, int(ridge_count) + int(split_count))]):
        ...
        v22._add_curved_tube_detail(
            ...
            float(rng.uniform(0.058, 0.108)),
            float(rng.uniform(0.0024, 0.0044)) * float(detail_radius_scale),
            rng,
            segments=4,
            sides=6,
    ...
    return mesh, counts, {
        ...
        "continuous_sweep_fork_centers": fork_centers[:128],
        "seam_mask_radius": 0.152,
        "continuous_sweep_support": True,
        "mechanical_sleeve_disabled": True,
        "direct_axis_sleeve_count": 0,
        "junction_continuous_sweep_count": int(len(set(anchors))),
...
def _add_continuous_sweep_fork_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    counts = _blank_counts()
    seam_centers: List[List[float]] = []
    continuous_sweep_forks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        continuous_sweep_forks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "continuous_sweep_fork_centers": continuous_sweep_forks[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_continuous_sweep_fork_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "continuous_sweep_fork_centers": yfork_centers[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v46(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _continuous_sweep_fork_mesh_v46(
        nodes,
        parents,
        radii,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _continuous_sweep_fork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("continuous_sweep_fork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_continuous_sweep_fork_read",
    ...
    controls.update(
        {
            ...
            "v46_lsystem_branch_continuous_sweep_fork_naturalization": True,
            "continuous_sweep_support": bool(collar_diag.get("continuous_sweep_support", False)),
            ...
            "junction_continuous_sweep_count": int(collar_diag.get("junction_continuous_sweep_count", 0)),
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch continuous-sweep-fork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_continuous_sweep_fork_bands_only",
            ...
            "continuous_sweep_fork_zoom_pairs": primary_pairs[:4],
            "continuous_sweep_fork_zoom_pair_count": int(len(primary_pairs)),
            "continuous_sweep_fork_anchor_indices": list(lsys_diag.get("continuous_sweep_fork_anchor_indices", []))[:16],
            "continuous_sweep_fork_branch_count": int(lsys_diag.get("continuous_sweep_fork_branch_count", 0)),
            ...
            "hard_tube_cap_mitigation": "generator-native continuous sweep wood support with wood-only twig continuation; terminal sleeves and axis sleeve append geometry disabled",
            "continuous_sweep_fork_zoom_target_policy": "first manifest pair is the explicit low/mid graph continuous sweep fork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,continuous_sweep_fork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "continuous_sweep_fork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V46_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_continuous_sweep_fork_mask",
    ...
    return {
        ...
        "grammar_guide": "v46_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v46_lsystem_branch_continuous_sweep_fork_naturalization",
            "qa_priority": "continuous_sweep_wood_fork_naturalness_no_spherical_collar_mechanical_sleeve_or_leaf_shard_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V46_lsys_branch_continuous_sweep_fork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.032, 0.782, 0.0060, 0.24, 0.42, 0.52, 0.075, 10, 8, 2, 0, 0, 0, 0, 0, 1.05, 1.06, "cambium", "continuous-sweep-fork A: broad low/mid continuous wood saddles, minimal free ridges, no leaf-shard seam cover"),
        ("V46_lsys_branch_continuous_sweep_fork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.030, 0.790, 0.0056, 0.22, 0.40, 0.54, 0.075, 8, 6, 1, 0, 0, 0, 0, 0, 0.98, 1.00, "ridge", "continuous-sweep-fork lowfrag B: most conservative connected wood-fork candidate with thick support and very few thin details"),
        ("V46_lsys_branch_continuous_sweep_fork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.034, 0.774, 0.0062, 0.24, 0.44, 0.52, 0.072, 12, 10, 3, 0, 0, 0, 0, 0, 1.10, 1.10, "cedar", "continuous-sweep-fork dense C: strongest broad Y-fork silhouette while avoiding V41/V42 needle and blade fragments"),
        ("V46_lsys_branch_continuous_sweep_fork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.798, 0.0052, 0.22, 0.38, 0.56, 0.078, 7, 5, 1, 0, 0, 0, 0, 0, 0.94, 0.96, "olive", "continuous-sweep-fork slim D: V40-D-like stability but with thicker low/mid saddle fork and no green leaf shards"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V46_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V46_lsystem_branch_continuous_sweep_fork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511.py"
    metadata["v46_lsystem_branch_naturalization_contract"] = {
        ...
        "zoom_targets": {
            "source": "continuous-sweep-fork object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("continuous_sweep_fork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_continuous_sweep_fork_band", "wood_only_terminal_twigs", "low_relief_bark_ridges"],
        "continuous_sweep": {
            "enabled": controls.get("continuous_sweep_support"),
            ...
            "junction_continuous_sweep_count": controls.get("junction_continuous_sweep_count"),
    ...
    metadata["v46_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V46 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V46 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "continuous_sweep_fork_branch_count": int(spec["controls"].get("continuous_sweep_fork_branch_count", 0)),
            "continuous_sweep_support": str(bool(spec["controls"].get("continuous_sweep_support", False))).lower(),
            ...
            "junction_continuous_sweep_count": int(spec["controls"].get("junction_continuous_sweep_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v46_lsystem_explicit_continuous_sweep_fork_mask",
            }
        )
    (out_dir / "V46_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_continuous_sweep_fork_branches": min(int(row["continuous_sweep_fork_branch_count"]) for row in rows) if rows else 0,
            "continuous_sweep_support_required": True,
            "mechanical_sleeve_disabled_required": True,
            "min_junction_continuous_sweeps": min(int(row["junction_continuous_sweep_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V46_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V46 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V46 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It replaces V44's tube-plus-sleeve construction and V45's over-blobby whole-field union with a high-sided continuous sweep, junction-local radius boost, short transition lobes, and bark-ridge primitives. "
