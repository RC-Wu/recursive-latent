...
RUN="${RUN:-strict_visual_matched_texture_v16_natural_coral_20260510}"
SEED="${SEED:-20260510}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_v16_natural_coral_20260510}"
...
echo "=== GENERATE V16 native natural coral OBJ inputs $(date)"
"$PY" "$ROOT/assets/strict_visual_matched_cases_v16_natural_coral_20260510.py" \
  --root "$ROOT" --out "$INPUTS" --seed "$SEED"
echo "=== GENERATED V16 native natural coral inputs $(date)"

for gpu in 4 5 6 7; do
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_v16_natural_coral_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_v16_natural_coral.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_v16_natural_coral:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_v16_natural_coral.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
