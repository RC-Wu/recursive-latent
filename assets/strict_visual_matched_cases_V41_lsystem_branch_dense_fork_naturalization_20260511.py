#!/usr/bin/env python3
"""V41 L-system branch dense-fork naturalization inputs.
...
small terminal sleeves rendered as isolated wedges.  V41 keeps the same strict
branch-with-side-branches target and changes the local realization toward a
wood-only dense primary/secondary fork:
...
* mid-depth dense primary/secondary fork zoom targets for local QA.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v41_lsystem_branch_dense_fork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v41_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
def _lsystem_branch_graph_v41(
    ...
    *,
    ...
    """Finite L-system branch with explicit primary and dense secondary forks.
    ...
    raw_secondary_pairs: List[List[List[float]]] = []
    secondary_anchor_indices: List[int] = []
    secondary_side_branch_count = 0
    ...
    secondary_steps = [step for step in range(1, int(main_steps)) if step != primary_step]
    for j, step in enumerate(secondary_steps):
        idx = main_indices[step]
        if idx == primary_idx:
            continue
        axis = v22._node_axis(nodes, parents, idx)
        su, sv, _sw = v22._basis(axis)
        branch_signs = (-primary_sign, primary_sign) if (double_sides or j % 2 == 0) else ((-primary_sign if j % 3 else primary_sign),)
        for local_i, sign in enumerate(branch_signs):
            height_bias = 0.05 + 0.025 * ((j + local_i) % 3)
            side_dir = v22._unit(
                0.36 * axis
                + sign * float(rng.uniform(0.62, 0.76)) * su
                + (0.12 + 0.04 * local_i) * sv
                + np.array([0.0, 0.0, height_bias])
                + rng.normal(0.0, 0.010, 3)
            )
            length = float(side_length) * float(rng.uniform(0.34, 0.52)) * (1.0 - 0.035 * max(step - primary_step, 0))
            add_chain(idx, side_dir, length, max(int(side_depth) - 1, 2), 0.27)
            secondary_side_branch_count += 1
            secondary_anchor_indices.append(int(idx))
            anchor = np.asarray(nodes[idx], dtype=float)
            if len(raw_secondary_pairs) < 8:
                raw_secondary_pairs.append(
                    [
                        [float(x) for x in (anchor + side_dir * 0.044)],
                        [float(x) for x in (anchor + side_dir * 0.132)],
                    ]
                )
    ...
    secondary_pairs = [
        ...
        for pair in raw_secondary_pairs
    ]
    return nodes, parents, {
        "source": "custom_finite_dense_lsystem_branch_with_primary_and_secondary_saddle_neck_side_branches",
        "grammar_rule": "F -> F[+sF][-sF]F with explicit dense primary/secondary saddle-neck side branches",
        ...
        "secondary_fork_anchor_indices": sorted(set(int(idx) for idx in secondary_anchor_indices))[:32],
        "secondary_fork_zoom_pairs": secondary_pairs[:8],
        "secondary_side_branch_count": int(secondary_side_branch_count),
        "recursive_side_branch_event_count": int(saddle_neck_count),
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v41(
        seed,
    ...
    secondary_pairs = list(lsys_diag.get("secondary_fork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
        ...
        junction_zoom_pairs = primary_pairs + secondary_pairs + remaining_pairs
    ...
    controls.update(
        {
            ...
            "v41_lsystem_branch_dense_fork_naturalization": True,
            "v41_lsystem_branch_primary_fork_naturalization": True,
            ...
            "masked_local_naturalization_target": "dense L-system saddle-neck primary and secondary side-branch fork bands with wood-only tapered terminals",
            "mask_scope": "object_space_dense_primary_secondary_side_branch_fork_bands_only",
            "seam_mask_space": "object_xyz",
            "seam_mask_selection_rule": "degree>=2 dense L-system primary/secondary saddle junction root nodes and child-axis midpoints; no global resampling; terminal sleeve anchors excluded",
            "seam_mask_center_count": int(len(seam_centers)),
            ...
            "secondary_fork_zoom_pairs": secondary_pairs[:8],
            "secondary_fork_zoom_pair_count": int(len(secondary_pairs)),
            "secondary_fork_anchor_indices": list(lsys_diag.get("secondary_fork_anchor_indices", []))[:32],
            "secondary_side_branch_count": int(lsys_diag.get("secondary_side_branch_count", 0)),
            "recursive_side_branch_event_count": int(lsys_diag.get("recursive_side_branch_event_count", 0)),
    ...
    grammar = {
        ...
        "operator_to_traditional_mapping": {
            ...
            "secondary_fork_mask": "densify mid-branch side branches with additional connected saddle-neck fork roots",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("cedar", "cambium", "olive", "ridge"):
        path = guide_dir / f"V41_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_dense_primary_secondary_fork_mask",
    ...
    return {
        ...
        "grammar_guide": "v41_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v41_lsystem_branch_dense_fork_naturalization",
            "qa_priority": "dense_primary_secondary_side_branch_saddle_naturalness_terminal_taper_and_non_seam_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V41_lsys_branch_dense_fork_A", 4, 41001, 8, 3, 1, True, 0.38, 0.72, 0.82, 0.021, 0.704, 0.0013, 0.034, 0.086, 0.176, 0.066, 30, 92, 14, 0, 0, 0, 0, 0, 0.68, 0.76, "cambium", "dense-fork A: mid-branch primary fork plus many secondary saddle-neck side branches"),
        ("V41_lsys_branch_dense_fork_lowfrag_B", 5, 41002, 9, 2, 1, True, 0.31, 0.64, 0.76, 0.019, 0.714, 0.0012, 0.030, 0.082, 0.168, 0.064, 28, 88, 10, 0, 0, 0, 0, 0, 0.62, 0.70, "ridge", "dense-fork lowfrag B: conservative dense secondary branch lattice with no terminal patch geometry"),
        ("V41_lsys_branch_dense_fork_dense_C", 4, 41003, 8, 3, 1, True, 0.42, 0.78, 0.86, 0.021, 0.700, 0.0012, 0.032, 0.084, 0.170, 0.062, 34, 96, 16, 0, 0, 0, 0, 0, 0.70, 0.78, "olive", "dense-fork dense C: strongest traditional L-system branch-with-side-branches silhouette"),
        ("V41_lsys_branch_dense_fork_slim_D", 5, 41004, 9, 2, 1, True, 0.25, 0.60, 0.70, 0.017, 0.724, 0.0011, 0.028, 0.078, 0.158, 0.062, 26, 84, 8, 0, 0, 0, 0, 0, 0.58, 0.66, "cedar", "dense-fork slim D: V40-D continuity retained while adding denser secondary side branches"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V41_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V41_lsystem_branch_dense_fork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511.py"
    metadata["v41_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "explicit dense primary/secondary saddle-neck side-branch bases, stronger tip-parent taper, wood-only fine twig terminals, short segmented swept support, shallow object-space collars, bark ridges, and branch splits",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "dense primary/secondary fork object-space graph nodes plus child-axis midpoints, excluding terminal buds/sleeves and collar endpoints",
            ...
            "secondary_pairs": controls.get("secondary_fork_zoom_pairs", [])[:8],
        },
        "naturalization_regions": ["dense_primary_secondary_side_branch_saddle_fork_band", "wood_only_terminal_twigs", "bark_ridge_relief"],
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v41_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V41 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V41 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "secondary_side_branch_count": int(spec["controls"].get("secondary_side_branch_count", 0)),
            "recursive_side_branch_event_count": int(spec["controls"].get("recursive_side_branch_event_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v41_lsystem_dense_primary_secondary_fork_mask",
            }
        )
    (out_dir / "V41_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_secondary_side_branches": min(int(row["secondary_side_branch_count"]) for row in rows) if rows else 0,
            "min_recursive_side_branch_events": min(int(row["recursive_side_branch_event_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V41_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V41 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V41 repairs the V35/V36/V38 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, or as isolated terminal/collar wedges. "
