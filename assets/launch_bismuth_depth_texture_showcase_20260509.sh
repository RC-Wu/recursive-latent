...
RUN="${RUN:-bismuth_depth_textured_showcase_20260509}"
...
GUIDE="$ROOT/public_guides_20260508/processed/bismuth_crystal_square.png"
INPUT_ROOT="$ROOT/inputs/connected_scaffold_depth_cases_20260509/bismuth_hopper_depth"

run_stage() {
  local gpu="$1"
  local stage="$2"
  local label="bismuth_hopper_depth_stage_${stage}_bismuth"
  local mesh="$INPUT_ROOT/bismuth_hopper_depth_stage_${stage}/bismuth_hopper_depth_stage_${stage}.obj"
...
for pair in "4 01" "5 02" "6 03" "7 04"; do
  ...
  script="$LOG/run_gpu${gpu}_stage${stage}.sh"
  {
    ...
    echo "ROOT='$ROOT' RUN='$RUN' source '$ROOT/assets/launch_bismuth_depth_texture_showcase_20260509.sh' --worker '$gpu' '$stage'"
  } > "$script"
  chmod +x "$script"
