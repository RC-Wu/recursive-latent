...
RUN="${RUN:-strict_visual_matched_texture_v3b_connected_20260510}"
INPUTS="$ROOT/inputs/strict_visual_matched_cases_v3b_connected_20260510"
...
generate_inputs() {
  echo "=== GENERATE strict visual matched V3b connected OBJ inputs $(date)"
  "$PY" "$ROOT/assets/strict_visual_matched_cases_v3b_connected_20260510.py" \
    --root "$ROOT" \
    --out "$INPUTS" \
    --seed 20260510
  echo "=== GENERATED V3b connected $(date)"
}
...
for gpu in 4 5 6 7; do
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_v3b_connected_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_v3b_connected.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_v3b_connected:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_v3b_connected.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
