...
RUN="${RUN:-strict_visual_matched_texture_V59_lsystem_branch_smooth_short_bough_yfork_20260511}"
SEED="${SEED:-20260511}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V59_lsystem_branch_smooth_short_bough_yfork_20260511}"
...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V59 L-system branch only allows GPU 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  ...
  while IFS='|' read -r label mesh guide seed _gpu; do
    ...
    echo "=== START $label gpu=$gpu steps=$steps tex=$tex seed=$seed v59_lsystem_smooth_short_bough $(date)"
...
echo "=== GENERATE V59 L-system smooth short-bough OBJ inputs $(date)"
"$PY" "$ROOT/assets/strict_visual_matched_cases_V59_lsystem_branch_smooth_short_bough_yfork_20260511.py" \
  --root "$ROOT" --out "$INPUTS" --seed "$SEED"
echo "=== GENERATED V59 L-system smooth short-bough OBJ inputs $(date)"
...
for gpu in ${V59_GPUS:-4 5}; do
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V59_GPUS must only contain 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_V59_lsystem_branch_smooth_short_bough_yfork_20260511.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_V59_lsystem_smooth_short_bough.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_V59_lsystem_smooth_short_bough:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_V59_lsystem_smooth_short_bough.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
