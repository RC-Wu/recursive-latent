#!/usr/bin/env python3
"""V32 SC-tree canopy-veil seam naturalization inputs.
...
V31 improved short-hub metrics but still looked like a sparse branch lattice.
V32 changes the visible semantics: the branch/crown entry is covered by
shared-vertex canopy veils and leaf-mass clusters so the support reads as
growth inside the crown rather than a bare inserted scaffold.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V32_sc_tree_canopy_veil_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v32_sc_tree_canopy_veil_masked_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_predeclared_sc_tree_canopy_veil_candidates_one_per_gpu"
MESH_PBR_POLICY = "obj_inputs_canopy_veil_transition_guides_for_trellis2_glb_export_no_2d_backprojection_claim"
...
def _add_occlusion_shell(
    ...
    *,
    ...
    """Attach broad, low-detail bracts around the hub/crown transition."""
    ...
    for k, idx in enumerate(anchors):
        ...
        length = float(rng.uniform(0.060, 0.104)) * float(radius_scale)
        width = length * float(rng.uniform(0.34, 0.52))
        v22._add_lanceolate_leaf(
            mesh,
            centers[idx],
            anchor,
        ...
        counts["leaf_count"] += 1
        shell_centers.append([float(x) for x in anchor])
        if k % 3 == 0:
            ...
            counts["tendril_count"] += 1
    return counts, {
        "occlusion_shell_leaf_count": int(counts["leaf_count"]),
...
def _add_leaf_mass_cluster(
    ...
    *,
    ...
    for leaf_i in range(int(leaves)):
        phase = float(rng.uniform(0.0, math.tau)) + leaf_i * math.tau / max(int(leaves), 1)
        side = math.cos(phase) * u + math.sin(phase) * v
        ...
        width = length * float(rng.uniform(*width_scale))
        v22._add_lanceolate_leaf(mesh, anchor_idx, base, direction, length, width, rng, rows=6)
        centers.append([float(x) for x in base])
    return int(leaves), centers


def _add_canopy_veil_pads(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    """Add thick, attached crown pads and leafy mass over visible entry supports."""
    ...
    for k, idx in enumerate(anchors):
        ...
        leaf_count, leaf_centers = _add_leaf_mass_cluster(
            mesh,
            centers[idx],
            anchor,
            outward,
            rng,
            radius=float(rng.uniform(0.034, 0.062)) * float(radius_scale),
            leaves=2 + int(k % 3 == 0),
            length_range=(0.060 * float(radius_scale), 0.118 * float(radius_scale)),
            width_scale=(0.38, 0.64),
        )
        counts["leaf_count"] += leaf_count
        pad_centers.extend(leaf_centers)
    return counts, {
        "canopy_veil_pad_count": int(counts["flare_count"]),
        "canopy_veil_pad_leaf_count": int(counts["leaf_count"]),
        "canopy_veil_pad_anchor_count": int(len(set(anchors))),
        "canopy_veil_pad_centers": pad_centers[:80],
    }


def _add_mid_canopy_veil(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    for k, idx in enumerate(anchors):
        ...
        leaf_count, leaf_centers = _add_leaf_mass_cluster(
            ...
            outward,
            rng,
            radius=float(rng.uniform(0.030, 0.056)) * float(radius_scale),
            leaves=3,
            length_range=(0.050 * float(radius_scale), 0.100 * float(radius_scale)),
            width_scale=(0.38, 0.68),
        )
        counts["leaf_count"] += leaf_count
        veil_centers.extend(leaf_centers)
    return counts, {
        "mid_canopy_veil_count": int(counts["flare_count"]),
        "mid_canopy_veil_leaf_count": int(counts["leaf_count"]),
        "mid_canopy_veil_anchor_count": int(len(set(anchors))),
        "mid_canopy_veil_centers": veil_centers[:96],
    }
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("multistem", "occlusion", "braided", "softleaf"):
        path = guide_dir / f"V32_sc_tree_{key}_canopy_veil_lowcontrast_guide.png"
...
def _build_sc_tree_case(
    ...
    *,
    ...
    pad_counts, pad_diag = _add_canopy_veil_pads(
        mesh,
        nodes,
        parents,
        rng,
    ...
    veil_counts, veil_diag = _add_mid_canopy_veil(
        mesh,
        nodes,
        parents,
        rng,
    ...
    seam_centers = (
        ...
        + list(pad_diag.get("canopy_veil_pad_centers", []))
        + list(veil_diag.get("mid_canopy_veil_centers", []))
    ...
    controls = v22._finalize_controls(
        {
            ...
            "support_visibility_policy": "thin outer support below leaf veil while keeping shared-vertex support graph",
            "support_cross_section": "thin_outer_support_plus_attached_canopy_veil",
            "grammar_rule": "space-colonization growth rerooted as hidden crown-entry support, then masked canopy veil naturalization",
            **reroot_diag,
            **subdiv_diag,
        },
        nodes,
    ...
    controls.update(
        {
            ...
            "v32_sc_tree_canopy_veil_naturalization": True,
            ...
            "v32_base_from_v31": "V31 metric-improved compact-crown skeleton with outer support visually demoted under attached canopy veils",
            "masked_local_naturalization_target": "canopy veil branch/crown entry mass and terminal caps",
            "mask_scope": "object_space_canopy_veil_entry_band_and_terminal_caps_only",
            "seam_mask_space": "object_xyz",
            "seam_mask_selection_rule": "short entry hubs plus canopy veil pads, mid-canopy leaf mass, occlusion shell, and branch/crown junction flares; terminal caps tracked separately",
            ...
            "entry_sheath_count": int(counts["entry_sheath_count"]),
            "occlusion_shell_leaf_count": int(shell_diag.get("occlusion_shell_leaf_count", 0)),
            "occlusion_shell_lobe_count": int(shell_diag.get("occlusion_shell_lobe_count", 0)),
            "canopy_veil_pad_count": int(pad_diag.get("canopy_veil_pad_count", 0)),
            "canopy_veil_pad_leaf_count": int(pad_diag.get("canopy_veil_pad_leaf_count", 0)),
            "mid_canopy_veil_count": int(veil_diag.get("mid_canopy_veil_count", 0)),
            "mid_canopy_veil_leaf_count": int(veil_diag.get("mid_canopy_veil_leaf_count", 0)),
            ...
            "material_transition_policy": "object-space bark-to-olive-to-leaf canopy veil transition; Trellis guide remains whole-object low-contrast",
            "hard_tube_cap_mitigation": "canopy veil entry mass, visually demoted outer support, attached leaf clusters, entry sheaths, bark ridges, structural splits, and compact terminal buds",
        }
    )
    grammar = {
        "grammar_family": "Space colonization",
        "grammar_symbols": "attractor_field,influence_radius,kill_radius,step_size,hidden_entry_hub,junction_mask,canopy_veil,leaf_mass,bark_ridge,terminal_bud",
        ...
        "operator_to_traditional_mapping": {
            ...
            "canopy_occlusion_shell": "adds broad attached low-detail bracts around the entry to hide hard material and branch seams",
            "canopy_veil_entry_pad": "uses thick shared-vertex pads and leaf clusters to read as crown mass instead of a sparse branch lattice",
            "mid_canopy_veil": "turns the outer SC skeleton into attached leaf mass while preserving the support graph underneath",
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["Space colonization"]) + [
        ...
        "canopy_veil_entry_pads",
        "mid_canopy_leaf_mass",
    ...
    return {
        ...
        "recursive_mode": "tree-crown attractor competition with hidden support and masked canopy-veil naturalization",
        "mesh": mesh,
        "controls": controls,
        "guide_key": guide_key,
        "root_variant": root_variant,
        "grammar_guide": "v32_space_colonization_canopy_veil_lowcontrast_tree_guide",
        ...
        "case_role": "v32_sc_tree_canopy_veil_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V32_sc_tree_canopy_veil_A", 4, 32001, 620, 218, 0.222, 0.074, 0.033, 0.072, 0.49, 7, 5, 0.106, 0.054, 0.039, 0.858, 0.0080, 0.88, 0.138, 8, 0, 28, 28, 42, 6, 4, 4, 2, 0, 0, 0, 1.20, 0.82, 0.34, "occlusion", "canopy veil A: V31-C skeleton with demoted outer support and attached leaf-mass veil over the entry"),
        ("V32_sc_tree_leafmass_shell_B", 5, 32002, 660, 226, 0.226, 0.076, 0.033, 0.074, 0.51, 7, 6, 0.108, 0.052, 0.040, 0.856, 0.0082, 0.88, 0.136, 8, 0, 32, 32, 48, 6, 4, 4, 2, 0, 0, 0, 1.24, 0.80, 0.30, "softleaf", "leafmass shell B: densest attached canopy veil, intended to remove the sparse branch-lattice read"),
        ("V32_sc_tree_lowrod_crown_C", 6, 32003, 600, 210, 0.230, 0.078, 0.032, 0.068, 0.53, 8, 5, 0.096, 0.048, 0.040, 0.858, 0.0086, 0.90, 0.132, 6, 0, 28, 34, 54, 4, 4, 2, 1, 0, 0, 0, 1.28, 0.78, 0.28, "occlusion", "lowrod crown C: most aggressive anti-rod candidate with high cut depth, thinnest outer supports, and broad attached leaf mass"),
        ("V32_sc_tree_balanced_canopy_D", 7, 32004, 680, 232, 0.220, 0.074, 0.033, 0.078, 0.47, 7, 6, 0.112, 0.056, 0.038, 0.860, 0.0078, 0.88, 0.140, 10, 0, 30, 30, 44, 8, 5, 5, 2, 0, 0, 0, 1.18, 0.84, 0.36, "softleaf", "balanced canopy D: keeps slightly more support readability while covering the branch/crown seam with attached veil geometry"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V32_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V32_sc_tree_canopy_veil_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V32_sc_tree_canopy_veil_naturalization_20260511.py"
    metadata["v32_seam_naturalization_contract"] = {
        ...
        "geometry_operator": "hidden short-hub support plus shared-vertex subdivision, visually demoted outer support, canopy veil pads, attached leaf-mass clusters, compact entry sheaths, occlusion shell, bark ridges, splits, and terminal buds",
        "base_evidence": "V32 uses V31-C's low-fragment metric direction but replaces the sparse branch-lattice read with attached canopy veil geometry",
        "seam_mask": {
            "space": "object_xyz",
    ...
    metadata["v32_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V32 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V32 short-hub segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "occlusion_shell_leaf_count": int(spec["controls"].get("occlusion_shell_leaf_count", 0)),
            "canopy_veil_pad_count": int(spec["controls"].get("canopy_veil_pad_count", 0)),
            "canopy_veil_pad_leaf_count": int(spec["controls"].get("canopy_veil_pad_leaf_count", 0)),
            "mid_canopy_veil_count": int(spec["controls"].get("mid_canopy_veil_count", 0)),
            "mid_canopy_veil_leaf_count": int(spec["controls"].get("mid_canopy_veil_leaf_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v32_object_space_seam_mask",
            }
        )
    (out_dir / "V32_obj_zoom_manifest_seamtarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V32_sc_tree_canopy_veil_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "canopy_veil_gate": {
            ...
            "min_canopy_veil_pads": min(int(row["canopy_veil_pad_count"]) for row in rows) if rows else 0,
            "min_mid_canopy_veils": min(int(row["mid_canopy_veil_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V32_obj_zoom_manifest_seamtarget_20260511.json"),
        "do_not_launch_remote": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V32 SC Tree Canopy-Veil Naturalization Dry Run\n\n"
        "Predeclared canopy-veil branch/crown seam candidates. V32 uses "
