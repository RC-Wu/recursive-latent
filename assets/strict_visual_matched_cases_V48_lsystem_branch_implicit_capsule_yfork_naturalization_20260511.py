#!/usr/bin/env python3
"""V48 L-system branch implicit-capsule-yfork naturalization inputs.
...
dropped.  V48 therefore keeps the same strict branch-with-side-branches target
but moves the fork naturalization into the graph itself:

* two or three low/mid implicit-capsule Y-fork anchors, far from the terminal, are
...
* mid-depth implicit-capsule-yfork zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V48_lsystem_branch_implicit_capsule_yfork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v48_lsystem_branch_implicit_capsule_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v48_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
IMPLICIT_GRID_RESOLUTION = 112
IMPLICIT_FIELD_BOUND = 1.70
IMPLICIT_FIELD_LEVEL = 0.43
IMPLICIT_GAUSSIAN_SIGMA = 0.34
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V48 still tapers
...
def _lsystem_branch_graph_v48(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid implicit capsule yforks.
    ...
    low/mid implicit capsule yfork. The graph uses fewer free children and thicker
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_implicit_capsule_yforks",
        ...
        "implicit_capsule_yfork_anchor_index": int(primary_anchor_new),
        "implicit_capsule_yfork_child_index": int(primary_child_new),
        "implicit_capsule_yfork_anchor_indices": sorted(set(yfork_anchor_indices))[:16],
        "implicit_capsule_yfork_branch_count": int(yfork_branch_count),
        "implicit_capsule_yfork_zoom_pairs": yfork_pairs[:6],
...
def _implicit_capsule_yfork_target_pairs(
    ...
    *,
    ...
    for idx in range(len(boosted_radii)):
        if idx in anchor_set:
            boosted_radii[idx] = max(boosted_radii[idx] * (1.24 + 0.06 * lobe_scale), boosted_radii[idx] + 0.0025)
        elif idx in local_boost_nodes:
            boosted_radii[idx] = max(boosted_radii[idx] * 1.12, boosted_radii[idx] + 0.0015)
        if idx in terminals:
    ...
    for edge_i, (a, b) in enumerate(edges):
        primitive_count += v45._stamp_wood_segment(
            ...
            max(boosted_radii[a], 0.0094),
            max(boosted_radii[b], 0.0090),
            strength=1.14,
            samples_per_unit=118.0,
            rng=rng,
    ...
    for k, idx in enumerate(anchors):
        ...
        child_dirs.sort(reverse=True, key=lambda item: item[0])
        v45._stamp_ball(field, coords, anchor, max(float(boosted_radii[idx]) * 1.14, 0.023), 0.62, cutoff=2.10)
        primitive_count += 1
        for _score, child_dir, _child in child_dirs[:3]:
            ...
            v45._stamp_ellipsoid(
                ...
                (
                    float(0.026 + 0.006 * detail_radius_scale),
                    float(0.020 + 0.004 * detail_radius_scale),
                    float(0.088 + 0.014 * lobe_scale),
                ),
                0.24,
                cutoff=1.86,
            )
            ...
            v45._stamp_ellipsoid(
                ...
                (
                    float(0.020 + 0.004 * detail_radius_scale),
                    float(0.025 + 0.005 * detail_radius_scale),
                    float(0.082 + 0.012 * lobe_scale),
                ),
                0.22,
                cutoff=1.80,
            )
    ...
    for k, idx in enumerate(ridge_sites[: max(0, int(ridge_count) + int(split_count))]):
        ...
        v45._stamp_ellipsoid(
            ...
            (
                float(0.0065 + 0.002 * detail_radius_scale),
                float(0.0090 + 0.002 * detail_radius_scale),
                float(0.050 + 0.012 * rng.random()),
            ),
            0.13,
            cutoff=1.55,
        )
    ...
    for idx in terminals:
        ...
        v45._stamp_ball(field, coords, np.asarray(nodes[idx], dtype=float) + axis * 0.012, max(float(boosted_radii[idx]) * 1.04, 0.0080), 0.38, cutoff=1.82)
        primitive_count += 1
    ...
    mesh = v45._taubin_smooth(mesh, iterations=4, lam=0.24, mu=-0.25)
    ...
    return mesh, counts, {
        ...
        "implicit_capsule_yfork_centers": fork_centers[:128],
        ...
        "low_relief_lobe_policy": "no appended lobes; smooth capsule field carries the branch junction geometry",
    }


def _implicit_capsule_yfork_mesh_v48(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
    *,
    ...
    """Create a continuous implicit capsule branch with boosted Y-forks."""
    ...
    return mesh, counts, {
        ...
        "low_relief_lobe_policy": "transition lobes are intentionally small; fusion is carried by graph-implicit capsule plus radius boost",
    }


def _add_implicit_capsule_yfork_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    counts = _blank_counts()
    seam_centers: List[List[float]] = []
    implicit_capsule_yforks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        implicit_capsule_yforks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "implicit_capsule_yfork_centers": implicit_capsule_yforks[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_implicit_capsule_yfork_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "implicit_capsule_yfork_centers": yfork_centers[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v48(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _implicit_capsule_yfork_mesh_v48(
        nodes,
        parents,
        radii,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _implicit_capsule_yfork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("implicit_capsule_yfork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_implicit_capsule_yfork_read",
    ...
    controls.update(
        {
            ...
            "v48_lsystem_branch_implicit_capsule_yfork_naturalization": True,
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch implicit-capsule-yfork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_implicit_capsule_yfork_bands_only",
            ...
            "implicit_capsule_yfork_zoom_pairs": primary_pairs[:4],
            "implicit_capsule_yfork_zoom_pair_count": int(len(primary_pairs)),
            "implicit_capsule_yfork_anchor_indices": list(lsys_diag.get("implicit_capsule_yfork_anchor_indices", []))[:16],
            "implicit_capsule_yfork_branch_count": int(lsys_diag.get("implicit_capsule_yfork_branch_count", 0)),
            "implicit_capsule_inserted_count": int(lsys_diag.get("implicit_capsule_inserted_count", 0)),
            "implicit_capsule_topology": lsys_diag.get("implicit_capsule_topology", ""),
            ...
            "hard_tube_cap_mitigation": "generator-native implicit capsule graph topology with wood-only twig continuation; terminal sleeves and axis sleeve append geometry disabled",
            "implicit_capsule_yfork_zoom_target_policy": "first manifest pair is the explicit low/mid graph implicit capsule yfork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,implicit_capsule_yfork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "implicit_capsule_yfork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V48_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_implicit_capsule_yfork_mask",
    ...
    return {
        ...
        "grammar_guide": "v48_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v48_lsystem_branch_implicit_capsule_yfork_naturalization",
            "qa_priority": "implicit_capsule_wood_fork_naturalness_no_spherical_collar_mechanical_sleeve_or_leaf_shard_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V48_lsys_branch_implicit_capsule_yfork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.032, 0.782, 0.0060, 0.24, 0.42, 0.52, 0.075, 10, 8, 2, 0, 0, 0, 0, 0, 1.05, 1.06, "cambium", "implicit-capsule-yfork A: broad low/mid continuous wood saddles, minimal free ridges, no leaf-shard seam cover"),
        ("V48_lsys_branch_implicit_capsule_yfork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.030, 0.790, 0.0056, 0.22, 0.40, 0.54, 0.075, 8, 6, 1, 0, 0, 0, 0, 0, 0.98, 1.00, "ridge", "implicit-capsule-yfork lowfrag B: most conservative connected wood-fork candidate with thick support and very few thin details"),
        ("V48_lsys_branch_implicit_capsule_yfork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.034, 0.774, 0.0062, 0.24, 0.44, 0.52, 0.072, 12, 10, 3, 0, 0, 0, 0, 0, 1.10, 1.10, "cedar", "implicit-capsule-yfork dense C: strongest broad Y-fork silhouette while avoiding V41/V42 needle and blade fragments"),
        ("V48_lsys_branch_implicit_capsule_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.798, 0.0052, 0.22, 0.38, 0.56, 0.078, 7, 5, 1, 0, 0, 0, 0, 0, 0.94, 0.96, "olive", "implicit-capsule-yfork slim D: V40-D-like stability but with thicker low/mid saddle fork and no green leaf shards"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V48_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V48_lsystem_branch_implicit_capsule_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V48_lsystem_branch_implicit_capsule_yfork_naturalization_20260511.py"
    metadata["v48_lsystem_branch_naturalization_contract"] = {
        ...
        "zoom_targets": {
            "source": "implicit-capsule-yfork object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("implicit_capsule_yfork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_implicit_capsule_yfork_band", "wood_only_terminal_twigs", "low_relief_bark_ridges"],
        "implicit_capsule": {
    ...
    metadata["v48_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V48 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V48 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "implicit_capsule_yfork_branch_count": int(spec["controls"].get("implicit_capsule_yfork_branch_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v48_lsystem_explicit_implicit_capsule_yfork_mask",
            }
        )
    (out_dir / "V48_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V48_lsystem_branch_implicit_capsule_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_implicit_capsule_yfork_branches": min(int(row["implicit_capsule_yfork_branch_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V48_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V48 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V48 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It replaces V44's tube-plus-sleeve construction and V45's over-blobby whole-field union with a high-sided implicit capsule, junction-local radius boost, short transition lobes, and bark-ridge primitives. "
