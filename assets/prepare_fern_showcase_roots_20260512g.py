#!/usr/bin/env python3
"""Prepare 20260512g fern/fiddlehead roots for publication-targeted recursion.

The 20260512e sweep was stable but did not satisfy the visual requirement for
the first case: the crozier root was still closer to an equal-spacing spiral
than a logarithmic fern crozier.  This revision makes the root contract explicit:

- fiddleheads are true logarithmic spirals, r(theta)=r0*exp(-b*theta), with
  self-similar buds placed at approximately equal arc-length intervals along
  the spiral;
- fern fronds are two-generation compound fronds with thicker visible branchlet
  handles, so bounded grammars can make depth changes without fragmented
  sidecars.
"""
...
DEFAULT_OUT = ROOT_DIR / "results" / "fern_showcase_roots_20260512g"
...
def _solid_fiddlehead(seed: int, variant: str) -> Tuple[pb.Mesh, Dict]:
    ...
    parent = 0
    stem_steps = 2 if variant == "log_open" else 3
    for i in range(1, stem_steps + 1):
        t = i / float(stem_steps)
        direction = np.array([0.08 * math.sin(t * math.pi * 0.5), 0.045 * math.cos(i * 0.7), 0.20 + 0.022 * t])
    ...
    center = stem_tip + np.array([0.22 if variant == "log_open" else 0.30, 0.0, -0.03])
    turns = 2.65 if variant == "log_open" else 3.15
    steps = 104 if variant == "log_open" else 124
    theta0 = -0.30 if variant == "log_open" else 0.10
    theta_span = turns * math.tau
    radius0 = 1.30 if variant == "log_open" else 1.40
    radius_min = 0.060 if variant == "log_open" else 0.052
    ...
    for i in range(1, steps + 1):
        ...
        p = center + np.array(
            [
                r * math.cos(theta),
                0.135 * math.sin(theta * 0.48),
                r * math.sin(theta) + 0.055 * t,
            ],
            dtype=float,
        )
        direction = p - prev
        ...
        parent = _append_child(
            ...
            max(0.065 * math.exp(-0.60 * theta_rel / theta_span), 0.017),
        )
    ...
    detail_count = 18 if variant == "log_open" else 24
    arc_arr = np.asarray(arc_samples, dtype=float)
    target_arcs = np.linspace(arc_arr[6], arc_arr[-5], detail_count)
    selected = []
    for target in target_arcs:
        j = int(np.argmin(np.abs(arc_arr - target)))
        if 4 <= j < len(coil_indices) - 4 and (not selected or abs(j - selected[-1]) >= 3):
            selected.append(j)
    for k, j in enumerate(selected):
        idx = coil_indices[j]
        axis = _node_axis(normalized, parents, idx)
        radial = _unit(np.asarray(normalized[idx]) - coil_origin)
        tangent = _unit(0.28 * axis + 0.94 * radial + rng.normal(0.0, 0.010, 3))
        scale = 0.92 ** k
        length = float(rng.uniform(0.120, 0.190) * max(scale, 0.36))
        _add_tube_bud(mesh, centers, normalized, idx, tangent, length, float(rng.uniform(0.0065, 0.0105) * max(scale, 0.62)), rng, curl=0.18)
        if k % 4 == 1:
            _add_tube_bud(mesh, centers, normalized, idx, _unit(0.52 * axis + 0.72 * radial), length * 0.62, float(rng.uniform(0.0040, 0.0060)), rng, curl=0.30)
    ...
    return mesh, {
        ...
        "variant": variant,
        "seed": seed,
        "target": "logarithmic fern fiddlehead crozier with self-similar buds",
        "root_design": "short stem, true logarithmic spiral r(theta)=r0*exp(-b*theta), and approximately equal-arc-length attached tubular buds",
...
def _compound_frond(seed: int, variant: str) -> Tuple[pb.Mesh, Dict]:
    ...
    rachis = [0]
    parent = 0
    n = 28 if variant == "tiered" else 32
    ...
    for local_i, anchor in enumerate(rachis[2:-2], start=2):
        t = local_i / float(n)
        envelope = math.sin(math.pi * t) ** (0.36 if variant == "tiered" else 0.44)
        side_len = (1.42 if variant == "tiered" else 1.22) * envelope + 0.22
        for sign in (-1, 1):
            side_dir = np.array([sign * (1.24 + 0.05 * math.sin(local_i * 0.7)), 0.070 * math.cos(local_i * 0.5), 0.08 - 0.24 * t])
            p0 = _append_child(nodes, parents, radii, anchor, side_dir, side_len * 0.58, max(0.030 * (1.0 - 0.28 * t), 0.009))
            p1 = _append_child(nodes, parents, radii, p0, side_dir + np.array([0.0, 0.026 * sign, -0.11]), side_len * 0.34, max(0.021 * (1.0 - 0.28 * t), 0.006))
            anchors.extend([(p0, sign, t, 1), (p1, sign, t, 1)])
            if 0.14 < t < 0.90:
                for branch_sign in (-1, 1):
                    if variant == "tiered" and local_i % 2 and branch_sign < 0:
                        continue
                    branch_dir = _unit(side_dir + np.array([0.16 * sign, 0.055 * branch_sign, -0.25]))
                    p2 = _append_child(nodes, parents, radii, p1, branch_dir, side_len * (0.25 if variant == "tiered" else 0.28), max(0.013 * (1.0 - 0.18 * t), 0.0048))
    ...
    for k, (idx, sign, t, order) in enumerate(anchors):
        ...
        width = length * float(rng.uniform(0.24, 0.39))
        _add_thick_lamina(
            mesh,
            centers[idx],
            normalized[idx],
    ...
    return mesh, {
        ...
        "variant": variant,
        "seed": seed,
        "target": "compound fern frond with visible multi-generation pinnae hierarchy",
        "root_design": "tiered rachis, thicker primary/secondary pinnae, attached laminae, and explicit refinement anchors",
...
def _cases(seed: int):
    return [
        ("fiddlehead_log_spiral_i", _solid_fiddlehead(seed + 51, "log_open")),
        ("fiddlehead_log_dense_j", _solid_fiddlehead(seed + 52, "log_dense")),
        ("fern_tiered_frond_i", _compound_frond(seed + 43, "tiered")),
        ("fern_hierarchical_frond_j", _compound_frond(seed + 44, "hierarchical")),
    ]


def _render_contact(out_dir: Path, rows: List[Dict]) -> Dict:
    out = out_dir / "fern_showcase_roots_contact_sheet_20260512g.png"
    cmd = [
        sys.executable,
...
def main() -> None:
    ...
    summary = {
        ...
        "revision": "20260512g logarithmic crozier and hierarchical compound fern roots",
    }
