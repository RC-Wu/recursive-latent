#!/usr/bin/env python3
"""V42 L-system branch cluster-fork naturalization inputs.
...
dropped.  V42 therefore keeps the same strict branch-with-side-branches target
but uses only a few low/mid explicit cluster forks:

* three low/mid cluster anchors, far from the terminal, are written into the
  graph and into the zoom manifest,
...
* mid-depth cluster-fork zoom targets for local QA,
* no global dense secondary branching: density is local and wood-dominant.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v42_lsystem_branch_cluster_fork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v42_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
def _lsystem_branch_graph_v42(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid cluster forks.

    V41 failed by adding side branches everywhere.  Here the first zoom target
    is the object-space root of a deliberately placed low/mid cluster fork, and
    only two nearby cluster anchors are added.  This keeps the branch readable
    and gives Trellis2 fewer thin disconnected tokens to invent.
    """
    ...
    raw_cluster_pairs: List[List[List[float]]] = []
    cluster_anchor_indices: List[int] = []
    cluster_branch_count = 0
    ...
    cluster_steps = [
        ...
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.50)))),
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.64)))),
    ]
    cluster_steps = sorted(set(cluster_steps))
    primary_step = cluster_steps[0]
    ...
    def add_chain(parent_idx: int, start_dir: np.ndarray, length0: float, depth: int, spread: float) -> int:
        ...
        neck_dir = v22._unit(0.58 * parent_axis + 0.42 * start_dir)
        neck = _append_node(nodes, parents, parent_idx, np.asarray(nodes[parent_idx], dtype=float) + neck_dir * length0 * 0.30)
        saddle_neck_count += 1
        current = _append_node(nodes, parents, neck, np.asarray(nodes[neck], dtype=float) + start_dir * length0 * 0.50)
        if depth > 0:
            junction_count += 1
            cu, cv, _cw = v22._basis(start_dir)
            child_signs = (-1, 1) if depth >= 2 else (1,)
            for s_i, sign in enumerate(child_signs):
                child_dir = v22._unit(0.72 * start_dir + sign * spread * cu + (0.11 - 0.04 * s_i) * cv + rng.normal(0.0, 0.010, 3))
                child_len = length0 * float(rng.uniform(0.38, 0.52))
                add_chain(current, child_dir, child_len, depth - 1, spread * 0.72)
        else:
            for sign in (-1, 1):
                cu, cv, _cw = v22._basis(start_dir)
                twig_dir = v22._unit(0.90 * start_dir + sign * 0.08 * cu + 0.04 * cv)
                _append_node(nodes, parents, current, np.asarray(nodes[current], dtype=float) + twig_dir * length0 * float(rng.uniform(0.18, 0.26)))
                terminal_fork_count += 1
            terminal_side_count += 1
        return current

    primary_child = -1
    for j, step in enumerate(cluster_steps):
        ...
        branch_signs = (primary_sign,) if not double_sides or j == 0 else (primary_sign, -primary_sign)
        for local_i, sign in enumerate(branch_signs):
            side_dir = v22._unit(
                0.40 * axis
                + sign * (0.62 - 0.05 * local_i) * su
                + (0.16 + 0.035 * j) * sv
                + np.array([0.0, 0.0, 0.055])
                + rng.normal(0.0, 0.008, 3)
            )
            depth = max(1, int(side_depth) - (1 if j > 0 else 0))
            length = float(side_length) * float(rng.uniform(0.44, 0.62)) * (1.0 - 0.035 * j)
            child = add_chain(idx, side_dir, length, depth, 0.26 if j else 0.32)
            if primary_child < 0:
                primary_child = child
            cluster_branch_count += 1
            cluster_anchor_indices.append(int(idx))
            if len(raw_cluster_pairs) < 6:
                raw_cluster_pairs.append(
                    [
                        [float(x) for x in (anchor + side_dir * 0.052)],
                        [float(x) for x in (anchor + side_dir * 0.155)],
                    ]
                )
        primary_sign *= -1
    ...
    for sign in (-1, 1):
        fork_dir = v22._unit(0.86 * tip_axis + sign * 0.10 * tu + 0.045 * tv)
        child = _append_node(nodes, parents, tip_idx, tip_pos + fork_dir * 0.080)
        _append_node(nodes, parents, child, np.asarray(nodes[child], dtype=float) + fork_dir * 0.042)
    ...
    cluster_pairs = [
        ...
        for pair in raw_cluster_pairs
    ]
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_cluster_forks",
        "grammar_rule": "F -> F[cluster(F)]F with sparse low/mid saddle-neck branch clusters",
        ...
        "cluster_fork_anchor_index": int(main_indices[primary_step]),
        "cluster_fork_child_index": int(primary_child),
        "cluster_fork_anchor_indices": sorted(set(cluster_anchor_indices))[:16],
        "cluster_fork_branch_count": int(cluster_branch_count),
        "cluster_fork_zoom_pairs": cluster_pairs[:6],
...
def _cluster_fork_target_pairs(
...
def _add_cluster_fork_collars(
    ...
    *,
    ...
    cluster_forks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        cluster_forks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "cluster_fork_centers": cluster_forks[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v42(
        seed,
    ...
    collar_counts, collar_diag = _add_cluster_fork_collars(
        mesh,
        nodes,
        parents,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _cluster_fork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("cluster_fork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls.update(
        {
            ...
            "v42_lsystem_branch_cluster_fork_naturalization": True,
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch cluster-fork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_cluster_fork_bands_only",
            ...
            "cluster_fork_zoom_pairs": primary_pairs[:4],
            "cluster_fork_zoom_pair_count": int(len(primary_pairs)),
            "cluster_fork_anchor_indices": list(lsys_diag.get("cluster_fork_anchor_indices", []))[:16],
            "cluster_fork_branch_count": int(lsys_diag.get("cluster_fork_branch_count", 0)),
            ...
            "hard_tube_cap_mitigation": "wood-only twig continuation, shorter high-depth support, sparse explicit saddle-neck cluster side-branch bases, stronger parent-of-terminal taper, terminal sleeves disabled, and elongated bark ridges",
            "cluster_fork_zoom_target_policy": "first manifest pair is the explicit low/mid graph cluster fork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,cluster_fork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "cluster_fork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("cedar", "cambium", "olive", "ridge"):
        path = guide_dir / f"V42_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_cluster_fork_mask",
    ...
    return {
        ...
        "grammar_guide": "v42_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v42_lsystem_branch_cluster_fork_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V42_lsys_branch_cluster_fork_A", 4, 42001, 7, 3, 2, True, 0.34, 0.66, 0.80, 0.023, 0.705, 0.0015, 0.034, 0.086, 0.180, 0.066, 22, 64, 8, 0, 0, 0, 0, 0, 0.66, 0.72, "cambium", "cluster-fork A: three local wood-dominant branch clusters with stable V40-like support"),
        ("V42_lsys_branch_cluster_fork_lowfrag_B", 5, 42002, 8, 2, 2, True, 0.28, 0.58, 0.74, 0.020, 0.718, 0.0013, 0.030, 0.082, 0.170, 0.066, 18, 58, 6, 0, 0, 0, 0, 0, 0.60, 0.66, "ridge", "cluster-fork lowfrag B: conservative local clusters intended to preserve V40-D connectivity"),
        ("V42_lsys_branch_cluster_fork_dense_C", 4, 42003, 7, 3, 2, True, 0.39, 0.72, 0.84, 0.023, 0.700, 0.0014, 0.032, 0.084, 0.174, 0.064, 24, 66, 10, 0, 0, 0, 0, 0, 0.68, 0.74, "olive", "cluster-fork dense C: strongest local cluster silhouette without V41 global twig density"),
        ("V42_lsys_branch_cluster_fork_slim_D", 5, 42004, 8, 2, 2, True, 0.24, 0.55, 0.68, 0.019, 0.724, 0.0012, 0.028, 0.078, 0.160, 0.064, 16, 54, 5, 0, 0, 0, 0, 0, 0.56, 0.62, "cedar", "cluster-fork slim D: V40-D-like thin support with visible low/mid cluster forks"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V42_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V42_lsystem_branch_cluster_fork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511.py"
    metadata["v42_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "three sparse low/mid cluster forks with explicit saddle-neck side-branch bases, stronger tip-parent taper, wood-only fine twig terminals, short segmented swept support, shallow object-space cluster-fork collars, bark ridges, and branch splits",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "cluster-fork object-space graph node plus child-axis midpoint, using the first sparse low/mid cluster and excluding terminal buds/sleeves, collar endpoints, and V41-style global secondary twig targets",
            ...
            "explicit_primary_pair": controls.get("cluster_fork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["sparse_side_branch_saddle_cluster_fork_band", "wood_only_terminal_twigs", "bark_ridge_relief"],
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v42_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V42 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V42 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "cluster_fork_branch_count": int(spec["controls"].get("cluster_fork_branch_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v42_lsystem_explicit_cluster_fork_mask",
            }
        )
    (out_dir / "V42_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_cluster_fork_branches": min(int(row["cluster_fork_branch_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V42_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V42 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V42 repairs the V35/V36/V38/V40/V41 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        "isolated terminal/collar wedges, a topology-stable but too-sparse V40-D, or V41-style post-GLB needle fragmentation. "
        "It uses three sparse low/mid saddle-neck cluster forks, stronger high-depth support taper, shallow cluster-fork collars, paired graph-root zoom targets, "
