#!/usr/bin/env python3
"""V43 L-system branch wood-yfork naturalization inputs.
...
dropped.  V43 therefore keeps the same strict branch-with-side-branches target
but uses only a few low/mid explicit wood y-forks:

* two or three low/mid wood-yfork anchors, far from the terminal, are written
...
* mid-depth wood-yfork zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V43_lsystem_branch_wood_yfork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v43_lsystem_branch_wood_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v43_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V43 still tapers
...
def _lsystem_branch_graph_v43(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid wood y-forks.
    ...
    low/mid wood y-fork. The graph uses fewer free children and thicker
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_wood_yforks",
        ...
        "wood_yfork_anchor_index": int(main_indices[primary_step]),
        "wood_yfork_child_index": int(primary_child),
        "wood_yfork_anchor_indices": sorted(set(yfork_anchor_indices))[:16],
        "wood_yfork_branch_count": int(yfork_branch_count),
        "wood_yfork_zoom_pairs": yfork_pairs[:6],
...
def _add_wood_yfork_collars(
    ...
    *,
    ...
    wood_yforks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        wood_yforks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "wood_yfork_centers": wood_yforks[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_wood_yfork_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "wood_yfork_centers": yfork_centers[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v43(
        seed,
    ...
    collar_counts, collar_diag = _add_broad_wood_yfork_saddles(
        mesh,
        nodes,
        parents,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _wood_yfork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("wood_yfork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_wood_yfork_read",
    ...
    controls.update(
        {
            ...
            "v43_lsystem_branch_wood_yfork_naturalization": True,
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch wood-yfork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_wood_yfork_bands_only",
            ...
            "wood_yfork_zoom_pairs": primary_pairs[:4],
            "wood_yfork_zoom_pair_count": int(len(primary_pairs)),
            "wood_yfork_anchor_indices": list(lsys_diag.get("wood_yfork_anchor_indices", []))[:16],
            "wood_yfork_branch_count": int(lsys_diag.get("wood_yfork_branch_count", 0)),
            ...
            "wood_yfork_zoom_target_policy": "first manifest pair is the explicit low/mid graph wood y-fork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,wood_yfork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "wood_yfork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V43_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_wood_yfork_mask",
    ...
    return {
        ...
        "grammar_guide": "v43_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v43_lsystem_branch_wood_yfork_naturalization",
            "qa_priority": "broad_continuous_wood_yfork_saddle_naturalness_no_leaf_shard_or_seam_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V43_lsys_branch_wood_yfork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.032, 0.782, 0.0060, 0.24, 0.42, 0.52, 0.075, 10, 8, 2, 0, 0, 0, 0, 0, 1.05, 1.06, "cambium", "wood-yfork A: broad low/mid continuous wood saddles, minimal free ridges, no leaf-shard seam cover"),
        ("V43_lsys_branch_wood_yfork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.030, 0.790, 0.0056, 0.22, 0.40, 0.54, 0.075, 8, 6, 1, 0, 0, 0, 0, 0, 0.98, 1.00, "ridge", "wood-yfork lowfrag B: most conservative connected wood-fork candidate with thick support and very few thin details"),
        ("V43_lsys_branch_wood_yfork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.034, 0.774, 0.0062, 0.24, 0.44, 0.52, 0.072, 12, 10, 3, 0, 0, 0, 0, 0, 1.10, 1.10, "cedar", "wood-yfork dense C: strongest broad Y-fork silhouette while avoiding V41/V42 needle and blade fragments"),
        ("V43_lsys_branch_wood_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.798, 0.0052, 0.22, 0.38, 0.56, 0.078, 7, 5, 1, 0, 0, 0, 0, 0, 0.94, 0.96, "olive", "wood-yfork slim D: V40-D-like stability but with thicker low/mid saddle fork and no green leaf shards"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V43_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V43_lsystem_branch_wood_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V43_lsystem_branch_wood_yfork_naturalization_20260511.py"
    metadata["v43_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "two or three broad low/mid wood y-forks with explicit saddle-neck side-branch bases, thick shared cambium saddles, stronger tip-parent taper, wood-only fine twig terminals, short segmented swept support, and minimal free ridges/splits",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "wood-yfork object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("wood_yfork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_wood_yfork_band", "wood_only_terminal_twigs", "minimal_bark_relief"],
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v43_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V43 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V43 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "wood_yfork_branch_count": int(spec["controls"].get("wood_yfork_branch_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v43_lsystem_explicit_wood_yfork_mask",
            }
        )
    (out_dir / "V43_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V43_lsystem_branch_wood_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_wood_yfork_branches": min(int(row["wood_yfork_branch_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V43_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V43 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V43 repairs the V35/V36/V38/V40/V41/V42 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It uses two or three broad low/mid saddle-neck wood y-forks, stronger high-depth support taper, thick wood-yfork saddles, paired graph-root zoom targets, "
