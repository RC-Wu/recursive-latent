...
RUN="${RUN:-strict_visual_matched_texture_20260510}"
SEED="${SEED:-20260510}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_20260510}"
...
generate_inputs() {
  echo "=== GENERATE strict visual matched OBJ inputs seed=$SEED input=$INPUT_NAME $(date)"
  "$PY" "$ROOT/assets/strict_visual_matched_cases_20260510.py" \
    --root "$ROOT" \
    --out "$INPUTS" \
    --seed "$SEED"
  echo "=== GENERATED $(date)"
...
worker() {
  local gpu="$1"
  local case_file="$INPUTS/gpu${gpu}_cases.txt"
  if [[ ! -f "$case_file" ]]; then
...
for gpu in 4 5 6 7; do
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched:$!"
done
...
echo "=== LOG TAILS"
for f in "$LOG"/gpu*_strict_visual_matched.log; do
  echo "==== $(basename "$f")"
  tail -20 "$f" || true
done
