#!/usr/bin/env python3
"""V39 L-system branch junction-root naturalization inputs.
...
small terminal sleeves rendered as isolated wedges.  V39 keeps the same strict
branch-with-side-branches target and changes the local realization toward a
wood-only junction-root fork:
...
* mid-depth junction-root zoom targets for local QA.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v39_lsystem_branch_junction_root_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v39_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
def _lsystem_branch_graph_v39(
    ...
    *,
    ...
    def add_recursive_side(parent_idx: int, parent_axis: np.ndarray, target_dir: np.ndarray, length0: float, depth: int) -> None:
        ...
        neck_len = float(length0) * float(rng.uniform(0.24, 0.34))
        neck = _append_node(nodes, parents, parent_idx, parent_pos + neck_dir * neck_len)
        saddle_neck_count += 1
        branch_len = float(length0) * float(rng.uniform(0.72, 0.88))
        ...
        if depth >= 2:
            ...
            if length0 > 0.055:
                u, v, w = v22._basis(target_dir)
                for sign in (-1, 1):
                    twig_dir = v22._unit(0.82 * target_dir + sign * 0.16 * u + 0.06 * v)
                    _append_node(nodes, parents, child, np.asarray(nodes[child], dtype=float) + twig_dir * length0 * float(rng.uniform(0.24, 0.34)))
                    terminal_side_count += 1
            else:
                terminal_side_count += 1
    ...
    for step_i in range(1, int(main_steps) + 1):
        ...
        if t > 0.60:
            step_len *= 0.40
            direction = v22._unit(0.84 * direction + 0.18 * np.array([-0.35, 0.14, 0.04]) + rng.normal(0.0, 0.014, 3))
    ...
    # Convert the main-axis endpoint into fine twig continuations.  V39 avoids
    # visible terminal sleeves entirely; the main branch ends by tapering into
    # multiple small connected twigs.
    if parent > 0:
        tip_pos = np.asarray(nodes[parent], dtype=float)
        parent_axis = v22._node_axis(nodes, parents, parent)
        u, v, _w = v22._basis(parent_axis)
        for sign, lift in ((-1, -0.06), (1, 0.08), (0, 0.18)):
            fork_dir = v22._unit(0.80 * parent_axis + sign * 0.18 * u + lift * v + rng.normal(0.0, 0.014, 3))
            child = _append_node(nodes, parents, parent, tip_pos + fork_dir * max(length * 0.58, 0.052))
            terminal_fork_count += 1
            child_pos = np.asarray(nodes[child], dtype=float)
            _append_node(nodes, parents, child, child_pos + v22._unit(0.88 * fork_dir + sign * 0.07 * v) * max(length * 0.30, 0.030))
            terminal_fork_count += 1
...
def _junction_root_target_pairs(
...
def _add_junction_root_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    ...
    *,
    ...
    junction_roots: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        junction_roots.append([float(x) for x in anchor])
    ...
    return counts, {
        ...
        "junction_root_centers": junction_roots[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v39(
        seed,
    ...
    collar_counts, collar_diag = _add_junction_root_collars(
        mesh,
        nodes,
        parents,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _junction_root_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    ...
    controls.update(
        {
            ...
            "v39_lsystem_branch_junction_root_naturalization": True,
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch junction-root bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_junction_root_bands_only",
    ...
    grammar = {
        ...
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,junction_root_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "junction_root_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("cedar", "cambium", "olive", "ridge"):
        path = guide_dir / f"V39_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_junction_root_mask",
    ...
    return {
        ...
        "grammar_guide": "v39_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v39_lsystem_branch_junction_root_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V39_lsys_branch_junction_root_A", 4, 39001, 6, 3, 2, True, 0.36, 0.70, 0.82, 0.022, 0.700, 0.0014, 0.036, 0.090, 0.185, 0.068, 24, 70, 10, 0, 0, 0, 0, 0, 0.70, 0.78, "cambium", "junction-root A: wood-only fine twig terminals; zoom targets sit at side-branch roots"),
        ("V39_lsys_branch_junction_root_lowfrag_B", 5, 39002, 7, 2, 2, False, 0.30, 0.62, 0.76, 0.020, 0.710, 0.0013, 0.032, 0.085, 0.175, 0.066, 22, 72, 8, 0, 0, 0, 0, 0, 0.66, 0.74, "ridge", "junction-root lowfrag B: conservative wood-only backup with no terminal patch geometry"),
        ("V39_lsys_branch_junction_root_dense_C", 4, 39003, 6, 3, 2, True, 0.40, 0.76, 0.86, 0.022, 0.695, 0.0013, 0.034, 0.088, 0.178, 0.064, 28, 68, 12, 0, 0, 0, 0, 0, 0.72, 0.80, "olive", "junction-root dense C: strongest recursive side-branch hierarchy while suppressing blunt terminal geometry"),
        ("V39_lsys_branch_junction_root_slim_D", 5, 39004, 7, 2, 2, False, 0.24, 0.58, 0.70, 0.018, 0.720, 0.0012, 0.030, 0.080, 0.165, 0.064, 18, 64, 6, 0, 0, 0, 0, 0, 0.62, 0.70, "cedar", "junction-root slim D: thinnest branch candidate for low cap visibility and clean junction zoom"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V39_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V39_lsystem_branch_junction_root_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511.py"
    metadata["v39_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "explicit saddle-neck side-branch bases, stronger tip-parent taper, wood-only fine twig terminals, short segmented swept support, shallow object-space junction-root collars, bark ridges, and branch splits",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "junction-root object-space graph nodes plus child-axis midpoints, excluding terminal buds/sleeves and collar endpoints",
        ...
        "naturalization_regions": ["side_branch_saddle_junction_root_band", "wood_only_terminal_twigs", "bark_ridge_relief"],
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v39_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V39 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V39 short-segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v39_lsystem_object_space_junction_mask",
            }
        )
    (out_dir / "V39_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V39_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V39 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V39 repairs the V35/V36/V38 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, or as isolated terminal/collar wedges. "
        "It uses explicit saddle-neck side-branch bases, stronger high-depth support taper, shallow junction-root collars, paired graph-root zoom targets, "
