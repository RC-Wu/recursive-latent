...
RUN="${RUN:-connected_scaffold_v2_textured_glb_hq_20260509}"
...
run_case() {
  ...
  echo "=== START $label steps=$steps tex=$tex $(date)"
...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  case "$gpu" in
    4)
      run_case 4 "bismuth_hopper_bismuth_hq" \
        "$ROOT/inputs/connected_scaffold_cases_v2_20260509/bismuth_hopper_cluster/bismuth_hopper_cluster.obj" \
        "$ROOT/public_guides_20260508/processed/bismuth_crystal_square.png" 8 2048
      run_case 4 "bismuth_hopper_pyrite_hq" \
        "$ROOT/inputs/connected_scaffold_cases_v2_20260509/bismuth_hopper_cluster/bismuth_hopper_cluster.obj" \
        "$ROOT/public_guides_20260508/processed/pyrite_cubes_square.png" 8 2048
      ;;
    5)
      run_case 5 "volumetric_dla_coral_bismuth_hq" \
        "$ROOT/inputs/connected_scaffold_cases_v2_20260509/volumetric_dla_coral_cluster/volumetric_dla_coral_cluster.obj" \
        "$ROOT/public_guides_20260508/processed/bismuth_crystal_square.png" 8 2048
      run_case 5 "volumetric_dla_coral_octopus_hq" \
        "$ROOT/inputs/connected_scaffold_cases_v2_20260509/volumetric_dla_coral_cluster/volumetric_dla_coral_cluster.obj" \
        "$ROOT/public_guides_20260508/processed/octopus_suckers_square.png" 8 2048
      ;;
    7)
      run_case 7 "pyrite_lattice_pyrite_hq" \
        "$ROOT/inputs/connected_scaffold_cases_v2_20260509/pyrite_crystal_lattice_cluster/pyrite_crystal_lattice_cluster.obj" \
        "$ROOT/public_guides_20260508/processed/pyrite_cubes_square.png" 8 2048
      run_case 7 "porous_coral_bismuth_hq" \
...
for gpu in 4 5 7; do
  script="$LOG/run_gpu${gpu}.sh"
  {
    ...
    echo "ROOT='$ROOT' RUN='$RUN' source '$ROOT/assets/launch_connected_scaffold_v2_texture_hq_20260509.sh' --worker '$gpu'"
  } > "$script"
  chmod +x "$script"
done

nohup "$LOG/run_gpu4.sh" > "$LOG/gpu4_v2_texture_hq.log" 2>&1 &
echo "gpu4_v2_texture_hq:$!"
nohup "$LOG/run_gpu5.sh" > "$LOG/gpu5_v2_texture_hq.log" 2>&1 &
echo "gpu5_v2_texture_hq:$!"
nohup "$LOG/run_gpu7.sh" > "$LOG/gpu7_v2_texture_hq.log" 2>&1 &
echo "gpu7_v2_texture_hq:$!"

sleep 3
nvidia-smi --query-gpu=index,memory.used,memory.total --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_v2_texture_hq.log; do
  echo "==== $(basename "$f")"
  tail -8 "$f" || true
done
