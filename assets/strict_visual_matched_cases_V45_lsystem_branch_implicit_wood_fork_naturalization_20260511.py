#!/usr/bin/env python3
"""V45 L-system branch implicit-wood-fork naturalization inputs.
...
dropped.  V45 therefore keeps the same strict branch-with-side-branches target
but uses only a few low/mid explicit implicit wood forks:

* two or three low/mid implicit-wood-fork anchors, far from the terminal, are written
  into the graph and into the zoom manifest,
* side branches leave through shared saddle-neck segments instead of abrupt
  tube insertions,
* broad cambium saddle collars at actual side-branch roots, with zoom targets
  stored at graph junction roots rather than lobe endpoints,
* no terminal sleeves/bracts, so terminals are solved by thin twig continuation
  rather than by visible cap patches,
* mid-depth implicit-wood-fork zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v45_lsystem_branch_implicit_wood_fork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v45_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
IMPLICIT_GRID_RESOLUTION = 92
IMPLICIT_FIELD_BOUND = 1.72
IMPLICIT_FIELD_LEVEL = 0.47
IMPLICIT_GAUSSIAN_SIGMA = 0.38
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V45 still tapers
...
def _lsystem_branch_graph_v45(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid implicit wood forks.
    ...
    low/mid implicit wood fork. The graph uses fewer free children and thicker
    ...
    def add_chain(parent_idx: int, start_dir: np.ndarray, length0: float, depth: int, spread: float) -> int:
        ...
        neck = _append_node(nodes, parents, parent_idx, np.asarray(nodes[parent_idx], dtype=float) + neck_dir * length0 * 0.38)
        saddle_neck_count += 1
        turn_dir = v22._unit(0.34 * parent_axis + 0.66 * start_dir)
        current = _append_node(nodes, parents, neck, np.asarray(nodes[neck], dtype=float) + turn_dir * length0 * 0.58)
        if depth > 0:
            junction_count += 1
        ...
        for local_i, sign in enumerate(branch_signs):
            ...
            if len(raw_yfork_pairs) < 6:
                raw_yfork_pairs.append(
                    [
                        [float(x) for x in (anchor + side_dir * 0.070)],
                        [float(x) for x in (anchor + side_dir * 0.190)],
                    ]
                )
        primary_sign *= -1
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_implicit_wood_forks",
        ...
        "implicit_wood_fork_anchor_index": int(main_indices[primary_step]),
        "implicit_wood_fork_child_index": int(primary_child),
        "implicit_wood_fork_anchor_indices": sorted(set(yfork_anchor_indices))[:16],
        "implicit_wood_fork_branch_count": int(yfork_branch_count),
        "implicit_wood_fork_zoom_pairs": yfork_pairs[:6],
...
def _implicit_wood_fork_target_pairs(
...
def _taubin_smooth(mesh: pb.Mesh, iterations: int = 8, lam: float = 0.32, mu: float = -0.34) -> pb.Mesh:
    return v16._taubin_smooth(mesh, iterations=iterations, lam=lam, mu=mu)


def _stamp_wood_segment(
    field: np.ndarray,
    coords: np.ndarray,
    a: np.ndarray,
    b: np.ndarray,
    ...
    *,
    ...
    axis = np.asarray(b, dtype=float) - np.asarray(a, dtype=float)
    length = float(np.linalg.norm(axis))
    if length < 1e-8:
        return 0
    u, v, _w = v22._basis(axis)
    samples = max(10, int(length * samples_per_unit))
    count = 0
    for s in range(samples + 1):
        t = s / max(float(samples), 1.0)
        center = np.asarray(a, dtype=float) * (1.0 - t) + np.asarray(b, dtype=float) * t
        radius = float(radius_a) * (1.0 - t) + float(radius_b) * t
        bark_offset = (
            u * math.sin(2.0 * math.pi * t + phase)
            + v * math.cos(2.0 * math.pi * t + phase * 0.73)
        ) * radius * 0.055
        taper_noise = 1.0 + 0.030 * math.sin(7.0 * t + phase) + 0.018 * math.sin(17.0 * t + phase * 0.41)
        _stamp_ball(field, coords, center + bark_offset, max(radius * taper_noise, 0.0065), strength)
        count += 1
    return count
...
def _implicit_wood_fork_mesh_v45(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
    *,
    ...
    """Create the branch as one implicit field, not tube geometry plus sleeves."""
    ...
    for edge_i, (a, b) in enumerate(edges):
        boost = 1.0
        if a in anchor_set or b in anchor_set:
            boost = 1.62
        elif a in local_boost_nodes or b in local_boost_nodes:
            boost = 1.28
        terminal_factor = 0.96 if b not in terminals else 1.08
        primitive_count += _stamp_wood_segment(
            field,
            coords,
            np.asarray(nodes[a], dtype=float),
            np.asarray(nodes[b], dtype=float),
            max(float(radii[a]) * boost, 0.0075),
            max(float(radii[b]) * boost * terminal_factor, 0.0075),
            strength=1.05,
            samples_per_unit=92.0,
            rng=rng,
            phase=edge_i * 0.79 + rng.uniform(-0.25, 0.25),
        )
    ...
    for k, idx in enumerate(anchors):
        ...
        child_dirs.sort(reverse=True, key=lambda item: item[0])
        base_radius = max(float(radii[idx]) * float(1.72 + 0.16 * lobe_scale), 0.034)
        _stamp_ball(field, coords, anchor, base_radius, 1.28, cutoff=2.55)
        primitive_count += 1
        for _score, child_dir, child in child_dirs[:3]:
            ...
            axes = (side_dir, normal, blend_dir)
            mid = anchor + blend_dir * float(0.042 + 0.014 * (k % 2))
            _stamp_ellipsoid(
                field,
                coords,
                mid,
                axes,
                (
                    float(0.050 + 0.010 * lobe_scale),
                    float(0.032 + 0.007 * detail_radius_scale),
                    float(0.142 + 0.020 * lobe_scale),
                ),
                0.46,
                cutoff=2.18,
            )
            _stamp_ellipsoid(
                field,
                coords,
                anchor + child_dir * 0.105,
                (normal, side_dir, child_dir),
                (
                    float(0.034 + 0.006 * detail_radius_scale),
                    float(0.044 + 0.008 * detail_radius_scale),
                    float(0.122 + 0.018 * lobe_scale),
                ),
                0.38,
                cutoff=2.05,
            )
            seam_centers.extend([[float(x) for x in mid], [float(x) for x in (anchor + child_dir * 0.105)]])
    ...
    for k, idx in enumerate(ridge_sites[: max(0, int(ridge_count) + int(split_count))]):
        ...
        center = anchor + lateral * max(float(radii[idx]) * 0.70, 0.010) + axis * 0.026
        _stamp_ellipsoid(
            field,
            coords,
            center,
            ...
            (
                float(0.010 + 0.003 * detail_radius_scale),
                float(0.014 + 0.003 * detail_radius_scale),
                float(0.075 + 0.018 * rng.random()),
            ),
            0.20,
            cutoff=1.90,
        )
    ...
    for idx in terminals:
        ...
        _stamp_ball(field, coords, np.asarray(nodes[idx], dtype=float) + axis * 0.015, max(float(radii[idx]) * 1.26, 0.009), 0.72, cutoff=2.15)
        primitive_count += 1

    field = gaussian_filter(field, sigma=IMPLICIT_GAUSSIAN_SIGMA)
    ...
    mesh = _taubin_smooth(mesh, iterations=7)
    ...
    return mesh, counts, {
        ...
        "implicit_wood_fork_centers": fork_centers[:128],
        "seam_mask_radius": 0.152,
        "implicit_union_support": True,
        "mechanical_sleeve_disabled": True,
        "direct_axis_sleeve_count": 0,
        "junction_implicit_union_count": int(len(set(anchors))),
...
def _add_implicit_wood_fork_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    counts = _blank_counts()
    seam_centers: List[List[float]] = []
    implicit_wood_forks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        implicit_wood_forks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "implicit_wood_fork_centers": implicit_wood_forks[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_implicit_wood_fork_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "implicit_wood_fork_centers": yfork_centers[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v45(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _implicit_wood_fork_mesh_v45(
        nodes,
        parents,
        radii,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _implicit_wood_fork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("implicit_wood_fork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_implicit_wood_fork_read",
    ...
    controls.update(
        {
            ...
            "v45_lsystem_branch_implicit_wood_fork_naturalization": True,
            "implicit_union_support": bool(collar_diag.get("implicit_union_support", False)),
            ...
            "junction_implicit_union_count": int(collar_diag.get("junction_implicit_union_count", 0)),
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch implicit-wood-fork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_implicit_wood_fork_bands_only",
            ...
            "implicit_wood_fork_zoom_pairs": primary_pairs[:4],
            "implicit_wood_fork_zoom_pair_count": int(len(primary_pairs)),
            "implicit_wood_fork_anchor_indices": list(lsys_diag.get("implicit_wood_fork_anchor_indices", []))[:16],
            "implicit_wood_fork_branch_count": int(lsys_diag.get("implicit_wood_fork_branch_count", 0)),
            ...
            "hard_tube_cap_mitigation": "generator-native implicit union wood support with wood-only twig continuation; terminal sleeves and axis sleeve append geometry disabled",
            "implicit_wood_fork_zoom_target_policy": "first manifest pair is the explicit low/mid graph implicit wood fork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,implicit_wood_fork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "implicit_wood_fork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V45_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_implicit_wood_fork_mask",
    ...
    return {
        ...
        "grammar_guide": "v45_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v45_lsystem_branch_implicit_wood_fork_naturalization",
            "qa_priority": "implicit_union_wood_fork_naturalness_no_spherical_collar_mechanical_sleeve_or_leaf_shard_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V45_lsys_branch_implicit_wood_fork_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.032, 0.782, 0.0060, 0.24, 0.42, 0.52, 0.075, 10, 8, 2, 0, 0, 0, 0, 0, 1.05, 1.06, "cambium", "implicit-wood-fork A: broad low/mid continuous wood saddles, minimal free ridges, no leaf-shard seam cover"),
        ("V45_lsys_branch_implicit_wood_fork_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.030, 0.790, 0.0056, 0.22, 0.40, 0.54, 0.075, 8, 6, 1, 0, 0, 0, 0, 0, 0.98, 1.00, "ridge", "implicit-wood-fork lowfrag B: most conservative connected wood-fork candidate with thick support and very few thin details"),
        ("V45_lsys_branch_implicit_wood_fork_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.034, 0.774, 0.0062, 0.24, 0.44, 0.52, 0.072, 12, 10, 3, 0, 0, 0, 0, 0, 1.10, 1.10, "cedar", "implicit-wood-fork dense C: strongest broad Y-fork silhouette while avoiding V41/V42 needle and blade fragments"),
        ("V45_lsys_branch_implicit_wood_fork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.798, 0.0052, 0.22, 0.38, 0.56, 0.078, 7, 5, 1, 0, 0, 0, 0, 0, 0.94, 0.96, "olive", "implicit-wood-fork slim D: V40-D-like stability but with thicker low/mid saddle fork and no green leaf shards"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V45_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V45_lsystem_branch_implicit_wood_fork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511.py"
    metadata["v45_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "whole-branch implicit/metaball union over parent, neck, child root, saddle swell, and bark ridge primitives before marching cubes; no appended axis sleeves or tube-plus-collar construction",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "implicit-wood-fork object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("implicit_wood_fork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_implicit_wood_fork_band", "wood_only_terminal_twigs", "implicit_bark_relief"],
        "implicit_union": {
            "enabled": controls.get("implicit_union_support"),
            "grid_resolution": controls.get("implicit_grid_resolution"),
            "field_level": controls.get("implicit_field_level"),
            ...
            "junction_implicit_union_count": controls.get("junction_implicit_union_count"),
    ...
    metadata["v45_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V45 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V45 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "implicit_wood_fork_branch_count": int(spec["controls"].get("implicit_wood_fork_branch_count", 0)),
            "implicit_union_support": str(bool(spec["controls"].get("implicit_union_support", False))).lower(),
            ...
            "junction_implicit_union_count": int(spec["controls"].get("junction_implicit_union_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v45_lsystem_explicit_implicit_wood_fork_mask",
            }
        )
    (out_dir / "V45_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_implicit_wood_fork_branches": min(int(row["implicit_wood_fork_branch_count"]) for row in rows) if rows else 0,
            "implicit_union_support_required": True,
            "mechanical_sleeve_disabled_required": True,
            "min_junction_implicit_unions": min(int(row["junction_implicit_union_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V45_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V45 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V45 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It replaces V44's tube-plus-sleeve construction with generator-native implicit union over parent, neck, child-root, saddle swell, and bark-ridge primitives before marching cubes. "
