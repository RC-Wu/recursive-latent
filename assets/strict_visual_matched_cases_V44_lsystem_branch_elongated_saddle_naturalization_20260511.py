#!/usr/bin/env python3
"""V44 L-system branch elongated-saddle naturalization inputs.
...
dropped.  V44 therefore keeps the same strict branch-with-side-branches target
but uses only a few low/mid explicit elongated saddles:

* two or three low/mid elongated-saddle anchors, far from the terminal, are written
...
* mid-depth elongated-saddle zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V44_lsystem_branch_elongated_saddle_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v44_lsystem_branch_elongated_saddle_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v44_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V44 still tapers
...
def _lsystem_branch_graph_v44(
    ...
    *,
    ...
    """Finite L-system branch with a few explicit low/mid elongated saddles.
    ...
    low/mid elongated saddle. The graph uses fewer free children and thicker
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_three_local_elongated_saddles",
        ...
        "elongated_saddle_anchor_index": int(main_indices[primary_step]),
        "elongated_saddle_child_index": int(primary_child),
        "elongated_saddle_anchor_indices": sorted(set(yfork_anchor_indices))[:16],
        "elongated_saddle_branch_count": int(yfork_branch_count),
        "elongated_saddle_zoom_pairs": yfork_pairs[:6],
...
def _elongated_saddle_target_pairs(
...
def _add_elongated_saddle_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    ...
    *,
    ...
    elongated_saddles: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        elongated_saddles.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "elongated_saddle_centers": elongated_saddles[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_elongated_saddle_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "elongated_saddle_centers": yfork_centers[:128],
        "seam_mask_radius": 0.126,
    }


def _add_axis_sleeve(
    mesh: pb.Mesh,
    anchor_idx: int,
    anchor: np.ndarray,
    ...
    *,
    ...
    rings: int = 7,
    sides: int = 12,
    longitudinal_bias: float = 1.55,
) -> np.ndarray:
    """Low-relief elongated sleeve along a branch axis.

    Unlike transition lobes, the radius is strongest near the root and then
    falls off gently.  It produces a bark swelling that is longer than it is
    round, which is the visual target for V44.
    """
    ...
    root = np.asarray(anchor, dtype=float)
    for ring_i in range(1, int(rings) + 1):
        t = ring_i / float(rings)
        center = root + direction * float(length) * t
        centers.append(center)
        profile = 0.72 * (1.0 - 0.62 * t) + 0.18
        ring: List[int] = []
        for side_i in range(sides):
            theta = math.tau * side_i / sides + phase * 0.07
            oval = 1.0 + 0.20 * math.cos(theta)
            pos = center + (
                math.cos(theta) * u * oval * float(longitudinal_bias)
                + math.sin(theta) * v * (1.0 / float(longitudinal_bias))
            ) * float(radius) * profile
    ...
    last = ring_ids[-1]
    tip_center = centers[-1] + direction * float(length) * 0.025
...
def _add_elongated_cambium_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    counts = _blank_counts()
    seam_centers: List[List[float]] = []
    elongated_centers: List[List[float]] = []
    for idx in anchors:
        children = child_map.get(idx, [])
        if not children:
            continue
        anchor = np.asarray(nodes[idx], dtype=float)
        elongated_centers.append([float(x) for x in anchor])
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
        parent_axis = v22._unit(anchor - np.asarray(nodes[parent_idx], dtype=float)) if parent_idx != idx else v22._node_axis(nodes, parents, idx)
        child_dirs = [v22._unit(np.asarray(nodes[ch], dtype=float) - anchor) for ch in children[:2]]
        axes = [parent_axis, -parent_axis]
        for child_dir in child_dirs:
            axes.extend([v22._unit(0.60 * parent_axis + 0.40 * child_dir), child_dir])
        for direction in axes[:6]:
            center = _add_axis_sleeve(
                mesh,
                centers[idx],
                anchor,
                direction,
                length=float(rng.uniform(0.095, 0.165)) * float(lobe_scale),
                radius=float(rng.uniform(0.010, 0.017)) * float(radius_scale),
                rng=rng,
                rings=7,
                sides=12,
                longitudinal_bias=float(rng.uniform(1.35, 1.75)),
            )
            seam_centers.append([float(x) for x in center])
            counts["collar_count"] += 1
            counts["flare_count"] += 1
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "elongated_saddle_centers": elongated_centers[:128],
        "seam_mask_radius": 0.138,
    }
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v44(
        seed,
    ...
    mesh = v22._smooth_support_mesh(nodes, edges, radii, sides=18, ovality=0.18)
    collar_counts, collar_diag = _add_elongated_cambium_saddles(
        mesh,
        nodes,
        parents,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _elongated_saddle_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("elongated_saddle_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_elongated_saddle_read",
    ...
    controls.update(
        {
            ...
            "v44_lsystem_branch_elongated_saddle_naturalization": True,
            ...
            "masked_local_naturalization_target": "L-system saddle-neck side-branch elongated-saddle bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_elongated_saddle_bands_only",
            ...
            "elongated_saddle_zoom_pairs": primary_pairs[:4],
            "elongated_saddle_zoom_pair_count": int(len(primary_pairs)),
            "elongated_saddle_anchor_indices": list(lsys_diag.get("elongated_saddle_anchor_indices", []))[:16],
            "elongated_saddle_branch_count": int(lsys_diag.get("elongated_saddle_branch_count", 0)),
            ...
            "hard_tube_cap_mitigation": "wood-only twig continuation, broad low/mid saddle y-fork support, stronger parent-of-terminal taper, terminal sleeves disabled, and thin free ridges minimized",
            "elongated_saddle_zoom_target_policy": "first manifest pair is the explicit low/mid graph elongated saddle, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,elongated_saddle_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "elongated_saddle_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key, colors in palettes.items():
        path = guide_dir / f"V44_lsystem_branch_{key}_lowcontrast_guide.png"
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_elongated_saddle_mask",
    ...
    return {
        ...
        "grammar_guide": "v44_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v44_lsystem_branch_elongated_saddle_naturalization",
            "qa_priority": "elongated_low_relief_cambium_saddle_naturalness_no_spherical_collar_or_leaf_shard_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V44_lsys_branch_elongated_saddle_A", 4, 43001, 7, 2, 2, True, 0.30, 0.60, 0.82, 0.032, 0.782, 0.0060, 0.24, 0.42, 0.52, 0.075, 10, 8, 2, 0, 0, 0, 0, 0, 1.05, 1.06, "cambium", "elongated-saddle A: broad low/mid continuous wood saddles, minimal free ridges, no leaf-shard seam cover"),
        ("V44_lsys_branch_elongated_saddle_lowfrag_B", 5, 43002, 8, 2, 2, True, 0.26, 0.54, 0.78, 0.030, 0.790, 0.0056, 0.22, 0.40, 0.54, 0.075, 8, 6, 1, 0, 0, 0, 0, 0, 0.98, 1.00, "ridge", "elongated-saddle lowfrag B: most conservative connected wood-fork candidate with thick support and very few thin details"),
        ("V44_lsys_branch_elongated_saddle_dense_C", 4, 43003, 7, 2, 2, True, 0.34, 0.66, 0.86, 0.034, 0.774, 0.0062, 0.24, 0.44, 0.52, 0.072, 12, 10, 3, 0, 0, 0, 0, 0, 1.10, 1.10, "cedar", "elongated-saddle dense C: strongest broad Y-fork silhouette while avoiding V41/V42 needle and blade fragments"),
        ("V44_lsys_branch_elongated_saddle_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.798, 0.0052, 0.22, 0.38, 0.56, 0.078, 7, 5, 1, 0, 0, 0, 0, 0, 0.94, 0.96, "olive", "elongated-saddle slim D: V40-D-like stability but with thicker low/mid saddle fork and no green leaf shards"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V44_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V44_lsystem_branch_elongated_saddle_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V44_lsystem_branch_elongated_saddle_naturalization_20260511.py"
    metadata["v44_lsystem_branch_naturalization_contract"] = {
        ...
        "geometry_operator": "two or three broad low/mid elongated saddles with explicit saddle-neck side-branch bases, low-relief axis-aligned cambium sleeves, stronger tip-parent taper, wood-only fine twig terminals, short segmented swept support, and minimal free ridges/splits",
        "seam_mask": {
            "space": "object_xyz",
        ...
        "zoom_targets": {
            "source": "elongated-saddle object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("elongated_saddle_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_elongated_saddle_band", "wood_only_terminal_twigs", "minimal_bark_relief"],
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v44_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V44 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V44 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "elongated_saddle_branch_count": int(spec["controls"].get("elongated_saddle_branch_count", 0)),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v44_lsystem_explicit_elongated_saddle_mask",
            }
        )
    (out_dir / "V44_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V44_lsystem_branch_elongated_saddle_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_elongated_saddle_branches": min(int(row["elongated_saddle_branch_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V44_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V44 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V44 repairs the V35/V36/V38/V40/V41/V42 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It uses two or three broad low/mid saddle-neck elongated saddles, stronger high-depth support taper, thick elongated-saddle saddles, paired graph-root zoom targets, "
        "wood-only twig terminals, minimal free ridges/splits, and whole-object low-contrast guides. "
