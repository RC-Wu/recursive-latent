...
RUN="${RUN:-seam_aware_texturing_batch_20260510_remote}"
INPUT_NAME="${INPUT_NAME:-seam_aware_texturing_batch_20260510}"
...
run_case() {
  ...
  echo "=== START $label gpu=$gpu steps=$steps tex=$tex seed=$seed seam-aware $(date)"
...
for gpu in 4 5 6; do
  nohup bash "$ROOT/assets/launch_seam_aware_texturing_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_seam_aware_texturing.log" 2>&1 &
  echo "gpu${gpu}_seam_aware_texturing:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_seam_aware_texturing.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
