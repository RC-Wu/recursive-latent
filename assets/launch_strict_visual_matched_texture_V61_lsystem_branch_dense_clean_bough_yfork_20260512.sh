...
RUN="${RUN:-strict_visual_matched_texture_V61_lsystem_branch_dense_clean_bough_yfork_BC_20260512_remote}"
SEED="${SEED:-20260511}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V61_lsystem_branch_dense_clean_bough_yfork_20260512}"
...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V61 L-system branch only allows GPU 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  ...
  while IFS='|' read -r label mesh guide seed _gpu; do
    [[ -z "${label:-}" ]] && continue
    [[ "$_gpu" == "$gpu" ]] || continue
    if [[ "${V61_PRIORITY_BC_ONLY:-1}" == "1" ]]; then
      case "$label" in
        V61_lsys_branch_dense_clean_B|V61_lsys_branch_balanced_clean_C) ;;
        *) echo "=== SKIP $label not priority B/C for first V61 remote pass"; continue ;;
      esac
    fi
    steps="${STEPS:-8}"
    ...
    echo "=== START $label gpu=$gpu steps=$steps tex=$tex seed=$seed v61_lsystem_dense_clean_bough $(date)"
...
echo "=== GENERATE V61 L-system dense clean-bough OBJ inputs $(date)"
"$PY" "$ROOT/assets/strict_visual_matched_cases_V61_lsystem_branch_dense_clean_bough_yfork_20260512.py" \
  --root "$ROOT" --out "$INPUTS" --seed "$SEED"
echo "=== GENERATED V61 L-system dense clean-bough OBJ inputs $(date)"
...
for gpu in ${V61_GPUS:-4 5}; do
  case "$gpu" in
    4|5|6|7) ;;
    *) echo "ERROR: V61_GPUS must only contain 4/5/6/7, got '$gpu'" >&2; exit 2 ;;
  esac
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_V61_lsystem_branch_dense_clean_bough_yfork_20260512.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_V61_lsystem_dense_clean_bough.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_V61_lsystem_dense_clean_bough:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_V61_lsystem_dense_clean_bough.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
