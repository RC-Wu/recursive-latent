#!/usr/bin/env python3
"""V54 L-system branch centered-bough highres-yfork naturalization inputs.
...
dropped. V49 then made the branch connected and much more coherent, but white
zoom QA still exposed rounded terminal bulbs. V50 repaired the OBJ terminal
recipe and kept post-GLB connectivity, yet textured white renders still showed
dark terminal drops and occasional side fragments as the dominant read. V54
therefore keeps the same strict branch-with-side-branches target and moves the
terminal repair from an after-field ellipsoid into the graph itself:

* two or three low/mid highres-smooth Y-fork anchors, far from the terminal, are
...
* no terminal sleeves/bracts, terminal cap balls, or terminal ellipsoid stamps;
  every exposed terminal is extended by graph-native two-step taper twigs before
  implicit field construction,
* mid-depth highres-smooth-yfork zoom targets for local QA,
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V54_lsystem_branch_centered_bough_yfork_20260511_dryrun"

SURFACE_STRATEGY = "v54_lsystem_branch_centered_bough_yfork_sidebranch_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_v54_lsystem_branch_candidates_local_qa_then_two_gpu_remote"
...
SWEEP_SIDES = 28
JUNCTION_RADIUS_BOOST = 1.30
JUNCTION_NEIGHBOR_RADIUS_BOOST = 1.11
IMPLICIT_GRID_RESOLUTION = 224
IMPLICIT_FIELD_BOUND = 1.70
IMPLICIT_FIELD_LEVEL = 0.410
IMPLICIT_GAUSSIAN_SIGMA = 0.60
    ...
    *,
    ...
    for idx, depth in enumerate(depths):
        ...
        if len(child_map.get(idx, [])) <= 1 and t > 0.34:
            # V42 became visually needle-like after GLB. V54 still tapers
        ...
        if idx in terminal_lineage and idx not in terminals:
            ...
            radius *= min(1.0, float(tip_parent_shrink) + 0.028 * distance)
        if idx in terminals:
            radius = max(float(floor) * 0.44, radius * float(terminal_shrink))
        else:
...
def _lsystem_branch_graph_v54(
    ...
    *,
    ...
    """Finite L-system branch with distributed centered-bough side branches.

    V52 fixed several local metrics but still staged the visual as a broom-like
    cluster around one thick branch; its second zoom was dominated by a coarse
    trunk surface. V54 spreads side branches along the main axis and chooses
    zoom pairs at interior Y-forks, so the first read is a real woody bough
    rather than a junction patch attached to a cut tube.
    """
    ...
    yfork_steps = [
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.24)))),
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.42)))),
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.62)))),
        max(2, min(int(main_steps) - 2, int(round(main_steps * 0.78)))),
    ]
    ...
    def add_chain(parent_idx: int, start_dir: np.ndarray, length0: float, depth: int, spread: float) -> int:
        ...
        neck_dir = v22._unit(0.72 * parent_axis + 0.28 * start_dir)
        neck = _append_node(nodes, parents, parent_idx, np.asarray(nodes[parent_idx], dtype=float) + neck_dir * length0 * 0.24)
        saddle_neck_count += 1
        turn_dir = v22._unit(0.44 * parent_axis + 0.56 * start_dir)
        current = _append_node(nodes, parents, neck, np.asarray(nodes[neck], dtype=float) + turn_dir * length0 * 0.42)
        current = _append_node(nodes, parents, current, np.asarray(nodes[current], dtype=float) + start_dir * length0 * 0.36)
        if depth > 0:
            junction_count += 1
            cu, cv, _cw = v22._basis(start_dir)
            child_signs = (-1, 1) if depth >= 2 else (1,)
            for s_i, sign in enumerate(child_signs):
                child_dir = v22._unit(0.84 * start_dir + sign * spread * 0.62 * cu + (0.06 - 0.025 * s_i) * cv + rng.normal(0.0, 0.006, 3))
                child_len = length0 * float(rng.uniform(0.44, 0.58))
                add_chain(current, child_dir, child_len, depth - 1, spread * 0.42)
        else:
            cu, cv, _cw = v22._basis(start_dir)
            twig_dir = v22._unit(0.97 * start_dir + 0.020 * cu + 0.020 * cv)
            _append_node(nodes, parents, current, np.asarray(nodes[current], dtype=float) + twig_dir * length0 * float(rng.uniform(0.34, 0.46)))
    ...
    for j, step in enumerate(yfork_steps):
        ...
        anchor = np.asarray(nodes[idx], dtype=float)
        branch_signs = (primary_sign,) if (not double_sides or j not in {1, 3}) else (primary_sign, -primary_sign)
        for local_i, sign in enumerate(branch_signs):
            side_dir = v22._unit(
                0.60 * axis
                + sign * (0.42 - 0.035 * local_i) * su
                + (0.08 + 0.012 * j) * sv
                + np.array([0.0, 0.0, 0.034])
                + rng.normal(0.0, 0.006, 3)
            )
            depth = max(1, int(side_depth) - (1 if j >= 2 else 0))
            length = float(side_length) * float(rng.uniform(0.42, 0.62)) * (1.0 - 0.055 * j)
            child = add_chain(idx, side_dir, length, depth, 0.20 if j else 0.24)
            if primary_child < 0:
                primary_child = child
            ...
            if len(raw_yfork_pairs) < 6:
                raw_yfork_pairs.append(
                    [
                        [float(x) for x in (anchor + side_dir * 0.060)],
                        [float(x) for x in (anchor + side_dir * 0.135)],
                    ]
                )
        primary_sign *= -1
    ...
    tip_child = _append_node(nodes, parents, tip_idx, tip_pos + tip_axis * 0.070)
    _append_node(nodes, parents, tip_child, np.asarray(nodes[tip_child], dtype=float) + tip_axis * 0.038)
    terminal_fork_count += 1

    # V54 turns the terminal fix into graph topology.  V50's weak ellipsoid
    # continuations still gave Trellis2 a round dark end-token after texturing.
    # Here every exposed side terminal receives three short taper nodes before
    # marching cubes, so the only remaining cap is tiny, branch-axis aligned,
    # and visually less likely to become the dark terminal token seen in V51.
    ...
    for terminal in initial_terminals:
        ...
        first = _append_node(nodes, parents, terminal, tip + axis * float(rng.uniform(0.052, 0.078)))
        second = _append_node(nodes, parents, first, np.asarray(nodes[first], dtype=float) + axis * float(rng.uniform(0.034, 0.052)))
        _append_node(nodes, parents, second, np.asarray(nodes[second], dtype=float) + axis * float(rng.uniform(0.020, 0.034)))
    ...
    return nodes, parents, {
        "source": "custom_finite_lsystem_branch_with_distributed_centered_bough_yforks",
        "grammar_rule": "F -> F[shallow_bough(F)]F with distributed saddle-neck woody side-branches",
        ...
        "highres_smooth_yfork_zoom_pairs": yfork_pairs[:6],
        "implicit_capsule_inserted_count": int(len(implicit_capsule_by_step)),
        "implicit_capsule_topology": "main branch continuation and distributed side branches share explicit centerline necks before splitting",
...
def _highres_smooth_yfork_target_pairs(
    ...
    *,
    ...
    for idx in anchors:
        ...
        root_target = anchor + v22._unit(0.62 * parent_axis + 0.38 * child_dir) * min(float(np.linalg.norm(child_pos - anchor)) * 0.12, 0.035)
        branch_target = anchor + child_dir * min(float(np.linalg.norm(child_pos - anchor)) * 0.24, 0.068)
    ...
    *,
    ...
    for idx in range(len(boosted_radii)):
        if idx in anchor_set:
            boosted_radii[idx] = max(boosted_radii[idx] * (1.10 + 0.03 * lobe_scale), boosted_radii[idx] + 0.0015)
        elif idx in local_boost_nodes:
            boosted_radii[idx] = max(boosted_radii[idx] * 1.06, boosted_radii[idx] + 0.0010)
        if idx in terminals:
            boosted_radii[idx] = max(boosted_radii[idx] * 0.76, 0.0052)
    ...
    for edge_i, (a, b) in enumerate(edges):
        primitive_count += v45._stamp_wood_segment(
            ...
            max(boosted_radii[a], 0.0105),
            max(boosted_radii[b], 0.0100),
            strength=1.06,
            samples_per_unit=168.0,
            rng=rng,
            phase=edge_i * 0.71 + rng.uniform(-0.16, 0.16),
        )
    ...
    for k, idx in enumerate(anchors):
        ...
        child_dirs.sort(reverse=True, key=lambda item: item[0])
        v45._stamp_ball(field, coords, anchor, max(float(boosted_radii[idx]) * 0.92, 0.018), 0.30, cutoff=1.78)
        primitive_count += 1
        for _score, child_dir, _child in child_dirs[:3]:
            blend_dir = v22._unit(0.58 * parent_axis + 0.42 * child_dir)
            side_dir = v22._unit(child_dir - 0.42 * parent_axis)
            ...
            mid = anchor + blend_dir * float(0.032 + 0.006 * (k % 2))
            v45._stamp_ellipsoid(
                ...
                (
                    float(0.013 + 0.003 * detail_radius_scale),
                    float(0.012 + 0.002 * detail_radius_scale),
                    float(0.052 + 0.008 * lobe_scale),
                ),
                0.090,
                cutoff=1.46,
            )
            root = anchor + child_dir * 0.052
            v45._stamp_ellipsoid(
                field,
                coords,
                root,
                ...
                (
                    float(0.012 + 0.002 * detail_radius_scale),
                    float(0.014 + 0.003 * detail_radius_scale),
                    float(0.050 + 0.007 * lobe_scale),
                ),
                0.082,
                cutoff=1.44,
            )
            seam_centers.extend([[float(x) for x in mid], [float(x) for x in root]])
            saddle_swell_count += 2
            primitive_count += 2
    ...
    for k, idx in enumerate(ridge_sites[: max(0, int(ridge_count) + int(split_count))]):
        ...
        center = anchor + lateral * max(float(boosted_radii[idx]) * 0.64, 0.008) + axis * 0.022
        v45._stamp_ellipsoid(
            field,
            coords,
            center,
            (lateral, v22._unit(np.cross(w, lateral)), w),
            (
                float(0.0028 + 0.0008 * detail_radius_scale),
                float(0.0044 + 0.0010 * detail_radius_scale),
                float(0.026 + 0.006 * rng.random()),
            ),
            0.050,
            cutoff=1.25,
        )
    ...
    field = v45.gaussian_filter(field, sigma=IMPLICIT_GAUSSIAN_SIGMA)
    ...
    mesh, raw_component_count, retained_ratio = v45._largest_component_mesh(mesh)
    mesh = v45._taubin_smooth(mesh, iterations=12, lam=0.30, mu=-0.32)
    ...
    return mesh, counts, {
        ...
        "terminal_taper_source": "graph_native_three_step_taper_segments_no_terminal_ellipsoid_stamps",
        "sweep_sides": 0,
        ...
        "low_relief_lobe_policy": "high-resolution smoothed field carries branch junction geometry; terminals are graph-native taper twigs, with no terminal ellipsoid stamps",
    }


def _highres_smooth_yfork_mesh_v54(
    nodes: List[np.ndarray],
    parents: List[int],
    radii: List[float],
    rng: np.random.Generator,
    *,
    ...
    """Create a continuous highres smooth branch with boosted Y-forks."""
    ...
    return mesh, counts, {
        ...
        "low_relief_lobe_policy": "transition lobes are intentionally small; fusion is carried by graph-highres smooth plus radius boost",
    }


def _add_highres_smooth_yfork_collars(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    counts = _blank_counts()
    seam_centers: List[List[float]] = []
    highres_smooth_yforks: List[List[float]] = []
    for k, idx in enumerate(anchors):
        anchor = np.asarray(nodes[idx], dtype=float)
        highres_smooth_yforks.append([float(x) for x in anchor])
        axis = v22._node_axis(nodes, parents, idx)
        parent_idx = int(parents[idx]) if int(parents[idx]) >= 0 else idx
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "highres_smooth_yfork_centers": highres_smooth_yforks[:128],
        "seam_mask_radius": 0.106,
    }


def _add_broad_highres_smooth_yfork_saddles(
    mesh: pb.Mesh,
    nodes: List[np.ndarray],
    parents: List[int],
    rng: np.random.Generator,
    *,
    ...
    return counts, {
        ...
        "seam_mask_center_count": int(len(seam_centers)),
        "seam_mask_centers": seam_centers[:128],
        "highres_smooth_yfork_centers": yfork_centers[:128],
...
def _build_lsystem_branch_case(
    ...
    *,
    ...
    nodes, parents, lsys_diag = _lsystem_branch_graph_v54(
        seed,
    ...
    edges = v22._graph_edges(parents)
    mesh, collar_counts, collar_diag = _highres_smooth_yfork_mesh_v54(
        nodes,
        parents,
        radii,
        rng,
    ...
    junction_anchor_indices, junction_zoom_pairs, junction_zoom_centers = _highres_smooth_yfork_target_pairs(
        nodes,
        parents,
        count=collar_count,
        pair_count=24,
    )
    primary_pairs = list(lsys_diag.get("highres_smooth_yfork_zoom_pairs", []))
    if primary_pairs:
        remaining_pairs = [
            pair
    ...
    controls = v22._finalize_controls(
        {
            ...
            **subdiv_diag,
            ...
            "support_cross_section": "broad_oval_highres_smooth_yfork_read",
            ...
            "implicit_capsule_inserted_count": int(lsys_diag.get("implicit_capsule_inserted_count", 0)),
            "implicit_capsule_topology": lsys_diag.get("implicit_capsule_topology", ""),
        },
        nodes,
        edges,
        {
    ...
    controls.update(
        {
            ...
            "v54_lsystem_branch_centered_bough_yfork_naturalization": True,
            "implicit_capsule_support": bool(collar_diag.get("implicit_capsule_support", False)),
            ...
            "junction_implicit_capsule_count": int(collar_diag.get("junction_implicit_capsule_count", 0)),
            "implicit_saddle_swell_count": int(collar_diag.get("implicit_saddle_swell_count", 0)),
            "terminal_taper_count": int(lsys_diag.get("graph_terminal_taper_count", 0)),
            "graph_terminal_taper_count": int(lsys_diag.get("graph_terminal_taper_count", 0)),
            "field_terminal_taper_count": int(collar_diag.get("field_terminal_taper_count", 0)),
            "terminal_taper_source": collar_diag.get("terminal_taper_source", "graph_native_three_step_taper_segments_no_terminal_ellipsoid_stamps"),
            ...
            "implicit_grid_resolution": int(collar_diag.get("implicit_grid_resolution", 0)),
            "implicit_field_bound": float(collar_diag.get("implicit_field_bound", 0.0)),
            "implicit_field_level": float(collar_diag.get("implicit_field_level", 0.0)),
            "gaussian_sigma": float(collar_diag.get("gaussian_sigma", 0.0)),
            ...
            "v48_failure_addressed": "low-resolution implicit capsule kept connectivity but pure-white zoom exposed faceted wedge nodes and coarse blob-like junctions",
            "v49_failure_addressed": "connected highres-yfork branch still exposed rounded terminal bulbs/caps in white zoom",
            "v50_failure_addressed": "post-GLB connectivity remained strong, but white renders still showed dark terminal drops and occasional isolated side fragments; V54 moves terminal taper into graph topology before implicit field construction",
            "masked_local_naturalization_target": "L-system saddle-neck side-branch highres-smooth-yfork bands with wood-only tapered terminals",
            "mask_scope": "object_space_side_branch_saddle_highres_smooth_yfork_bands_only",
            ...
            "highres_smooth_yfork_zoom_pairs": primary_pairs[:4],
            "highres_smooth_yfork_zoom_pair_count": int(len(primary_pairs)),
            "highres_smooth_yfork_anchor_indices": list(lsys_diag.get("highres_smooth_yfork_anchor_indices", []))[:16],
            "highres_smooth_yfork_branch_count": int(lsys_diag.get("highres_smooth_yfork_branch_count", 0)),
            "implicit_capsule_inserted_count": int(lsys_diag.get("implicit_capsule_inserted_count", 0)),
            "implicit_capsule_topology": lsys_diag.get("implicit_capsule_topology", ""),
            ...
            "hard_tube_cap_mitigation": "generator-native graph taper twigs plus highres smooth field and pale low-contrast bark guide; terminal ellipsoid stamps, terminal sleeves, cap balls, and axis sleeve append geometry are disabled",
            "highres_smooth_yfork_zoom_target_policy": "first manifest pair is the explicit low/mid graph highres smooth yfork, not an inferred collar, terminal, or V41-style global dense twig",
        }
    )
    grammar = {
        "grammar_family": "L-system",
        "grammar_symbols": "F,+,-,[,],side_branch,saddle_neck,highres_smooth_yfork_mask,cambium_collar,bark_ridge,terminal_twig",
        "target_symbol": "F",
        "operator_to_traditional_mapping": {
            ...
            "highres_smooth_yfork_mask": "select side-branch attachment nodes and child-axis midpoints after rewriting",
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    guides: Dict[str, str] = {}
    palettes = {
        "pale_cambium": [(124, 92, 58), (148, 108, 66), (174, 132, 82), (192, 154, 104)],
        "centered_bough": [(104, 76, 48), (130, 92, 56), (158, 116, 72), (176, 138, 90)],
        "warm_cedar": [(94, 76, 48), (116, 92, 56), (138, 108, 66), (154, 128, 82)],
        "matte_ridge": [(92, 68, 42), (118, 82, 50), (144, 106, 66), (166, 128, 84)],
    }
    for key, colors in palettes.items():
        path = guide_dir / f"V54_lsystem_branch_{key}_lowcontrast_guide.png"
        rng = np.random.default_rng(v26._stable_seed(path.stem))
        img = Image.new("RGB", (768, 768), (122, 96, 64))
        ...
        for _ in range(56):
            ...
            draw.ellipse((x - r, y - r, x + r, y + r), fill=(*color, int(rng.integers(22, 58))))
        ...
        for _ in range(24):
            ...
            draw.line(pts, fill=(*color, int(rng.integers(46, 86))), width=int(rng.integers(4, 8)))
            ...
            draw.line((fork, end), fill=(*color, int(rng.integers(38, 74))), width=int(rng.integers(2, 5)))
...
def _spec(
    *,
    ...
    operators = list(v23.OPERATORS_BY_FAMILY["L-system"]) + [
        ...
        "object_space_side_branch_highres_smooth_yfork_mask",
    ...
    return {
        ...
        "grammar_guide": "v54_lsystem_branch_lowcontrast_bark_cambium_guide",
        ...
        "strict_match_notes": reason,
            "case_role": "v54_lsystem_branch_centered_bough_yfork_naturalization",
            "qa_priority": "highres_smooth_wood_fork_naturalness_no_faceted_blob_spherical_collar_mechanical_sleeve_or_leaf_shard_read",
        "rerun_reason": reason,
        "boundary_tag": "",
    }
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V54_lsys_branch_centered_bough_yfork_A", 4, 43001, 7, 2, 2, False, 0.30, 0.60, 0.82, 0.030, 0.786, 0.0058, 0.14, 0.30, 0.58, 0.070, 7, 1, 0, 0, 0, 0, 0, 0, 0.56, 0.48, "centered_bough", "centered-bough yfork A: centered junction targets, softened saddle swells, and a less broom-like side-branch layout"),
        ("V54_lsys_branch_centered_bough_yfork_lowfrag_B", 5, 43002, 8, 2, 2, False, 0.26, 0.54, 0.78, 0.029, 0.794, 0.0054, 0.12, 0.28, 0.60, 0.070, 7, 1, 0, 0, 0, 0, 0, 0, 0.50, 0.44, "pale_cambium", "centered-bough lowfrag B: conservative centered wood fork with pale material guide and minimized lobe bulging"),
        ("V54_lsys_branch_centered_bough_yfork_dense_C", 4, 43003, 7, 2, 2, False, 0.34, 0.66, 0.86, 0.031, 0.780, 0.0060, 0.14, 0.30, 0.58, 0.068, 8, 1, 0, 0, 0, 0, 0, 0, 0.58, 0.50, "warm_cedar", "centered-bough dense C: stronger branch silhouette with centered zoom targets and muted warm-cedar guide"),
        ("V54_lsys_branch_centered_bough_yfork_slim_D", 5, 43004, 8, 2, 2, False, 0.22, 0.50, 0.72, 0.028, 0.802, 0.0050, 0.11, 0.26, 0.62, 0.074, 6, 1, 0, 0, 0, 0, 0, 0, 0.48, 0.42, "pale_cambium", "centered-bough slim D: sparse stable branch with centered junction targets and pale material guide"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V54_", ""),
                parameter_variant=(
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V54_lsystem_branch_centered_bough_yfork_naturalization_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V54_lsystem_branch_centered_bough_yfork_20260511.py"
    metadata["v54_lsystem_branch_naturalization_contract"] = {
        ...
        "zoom_targets": {
            "source": "highres-smooth-yfork object-space graph node plus child-axis midpoint, using the first broad low/mid y-fork and excluding terminal buds/sleeves, collar endpoints, and V41/V42-style thin twig targets",
            ...
            "explicit_primary_pair": controls.get("highres_smooth_yfork_zoom_pairs", [])[:1],
        },
        "naturalization_regions": ["broad_side_branch_saddle_highres_smooth_yfork_band", "wood_only_terminal_twigs", "low_relief_bark_ridges"],
        "implicit_capsule": {
            "enabled": controls.get("implicit_capsule_support"),
            ...
            "implicit_grid_resolution": controls.get("implicit_grid_resolution"),
            "implicit_field_level": controls.get("implicit_field_level"),
            "gaussian_sigma": controls.get("gaussian_sigma"),
            ...
            "junction_implicit_capsule_count": controls.get("junction_implicit_capsule_count"),
            "implicit_saddle_swell_count": controls.get("implicit_saddle_swell_count"),
            "terminal_taper_count": controls.get("terminal_taper_count"),
            "graph_terminal_taper_count": controls.get("graph_terminal_taper_count"),
            "field_terminal_taper_count": controls.get("field_terminal_taper_count"),
            "terminal_taper_source": controls.get("terminal_taper_source"),
            "implicit_capsule_inserted_count": controls.get("implicit_capsule_inserted_count"),
            "implicit_capsule_topology": controls.get("implicit_capsule_topology"),
        },
        "sdedit_seam_backprojection_available": False,
    ...
    metadata["v54_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V54 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V54 short-segment gate: {spec['controls']}")
        ...
        row = {
            ...
            "highres_smooth_yfork_branch_count": int(spec["controls"].get("highres_smooth_yfork_branch_count", 0)),
            "implicit_capsule_inserted_count": int(spec["controls"].get("implicit_capsule_inserted_count", 0)),
            "implicit_capsule_support": str(bool(spec["controls"].get("implicit_capsule_support", False))).lower(),
            ...
            "junction_implicit_capsule_count": int(spec["controls"].get("junction_implicit_capsule_count", 0)),
            "implicit_saddle_swell_count": int(spec["controls"].get("implicit_saddle_swell_count", 0)),
            "terminal_taper_count": int(spec["controls"].get("terminal_taper_count", 0)),
            "graph_terminal_taper_count": int(spec["controls"].get("graph_terminal_taper_count", 0)),
            "field_terminal_taper_count": int(spec["controls"].get("field_terminal_taper_count", 0)),
            "terminal_taper_source": spec["controls"].get("terminal_taper_source", ""),
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v54_lsystem_explicit_highres_smooth_yfork_mask",
            }
        )
    (out_dir / "V54_obj_zoom_manifest_junctiontarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V54_lsystem_branch_centered_bough_yfork_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "lsystem_branch_gate": {
            ...
            "min_highres_smooth_yfork_branches": min(int(row["highres_smooth_yfork_branch_count"]) for row in rows) if rows else 0,
            "min_implicit_capsule_inserted": min(int(row["implicit_capsule_inserted_count"]) for row in rows) if rows else 0,
            "implicit_capsule_support_required": True,
            "mechanical_sleeve_disabled_required": True,
            "min_junction_implicit_capsules": min(int(row["junction_implicit_capsule_count"]) for row in rows) if rows else 0,
            "min_implicit_saddle_swells": min(int(row["implicit_saddle_swell_count"]) for row in rows) if rows else 0,
            "min_terminal_tapers": min(int(row["terminal_taper_count"]) for row in rows) if rows else 0,
            "min_graph_terminal_tapers": min(int(row["graph_terminal_taper_count"]) for row in rows) if rows else 0,
            "max_field_terminal_tapers": max(int(row["field_terminal_taper_count"]) for row in rows) if rows else 0,
        ...
        "obj_zoom_manifest": str(out_dir / "V54_obj_zoom_manifest_junctiontarget_20260511.json"),
        "do_not_launch_remote_before_local_visual_qa": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V54 L-system Branch Junction-Root Naturalization Dry Run\n\n"
        "V54 repairs the V35/V36/V38/V40/V41/V42/V44 visual failure modes: connected metrics but pure-white zoom still read as a tube with attached side detail, "
        ...
        "It replaces V44's tube-plus-sleeve construction and V45's over-blobby whole-field union with a high-sided highres smooth, junction-local radius boost, short transition lobes, and bark-ridge primitives. "
