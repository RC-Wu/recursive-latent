#!/usr/bin/env python3
"""V30 SC-tree occluded-hub seam naturalization inputs.
...
hidden multi-stem hubs, but its GLB still had r0 fragmentation.  V30 starts
from V29-D's lower-fragment skeleton, borrows V29-B's crown occlusion, and
adds a short-hub subdivision gate so the entry support cannot read as a long
straight tube.
...
DEFAULT_OUT = ROOT_DIR / "results" / "strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511_dryrun"

SURFACE_STRATEGY = "v30_sc_tree_occluded_short_hub_masked_naturalization"
GENERATION_POLICY = "generate_new_on_a100_2_no_local_selection_or_postprocessing"
SELECTION_BUDGET = "four_predeclared_sc_tree_occluded_hub_candidates_one_per_gpu"
MESH_PBR_POLICY = "obj_inputs_occluded_short_hub_transition_guides_for_trellis2_glb_export_no_2d_backprojection_claim"
EXTERNAL_SUPPORT_MAX_SEGMENT_GATE = 0.22
...
def _write_guides(out_dir: Path) -> Dict[str, str]:
    ...
    for key in ("multistem", "occlusion", "braided", "softleaf"):
        path = guide_dir / f"V30_sc_tree_{key}_occluded_hub_lowcontrast_guide.png"
...
def _build_sc_tree_case(
    ...
    *,
    ...
    counts = _merge_counts(base_counts, entry_counts, shell_counts, flare_counts, ridge_counts, split_counts, bud_counts)
    seam_centers = (
    ...
    controls.update(
        {
            ...
            "v30_sc_tree_occluded_hub_naturalization": True,
            "v24_v29_failure_addressed": "visible branch/crown seam, smooth inserted rod, hard terminal cap, and post-GLB tiny fragments",
            "v30_base_from_v29": "V29_sc_tree_softleaf_entry_D geometry floor plus V29_sc_tree_canopy_occlusion_B occlusion read",
            "masked_local_naturalization_target": "occluded short-hub branch/crown entry band and terminal caps",
            "mask_scope": "object_space_occluded_short_hub_junction_band_and_terminal_caps_only",
            "seam_mask_space": "object_xyz",
            "seam_mask_selection_rule": "short entry hubs plus compact occlusion shell and branch/crown junction flares; terminal caps tracked separately",
            ...
            "material_transition_policy": "object-space branch bark to olive transition to leaf green; Trellis guide remains whole-object low-contrast",
            "hard_tube_cap_mitigation": "occluded short-hub entry, compact entry sheaths, canopy occlusion shell, bark ridges, structural splits, and terminal buds",
        }
    )
    grammar = {
...
def _spec(
    *,
    ...
    return {
        ...
        "recursive_mode": "tree-crown attractor competition with occluded short-hub entry and masked local naturalization",
        "mesh": mesh,
        "controls": controls,
        "guide_key": guide_key,
        "root_variant": root_variant,
        "grammar_guide": "v30_space_colonization_occluded_hub_lowcontrast_tree_guide",
        ...
        "case_role": "v30_sc_tree_occluded_hub_naturalization",
...
def _case_specs(seed: int) -> List[Dict]:
    settings = [
        ("V30_sc_tree_softleaf_occluded_hubs_A", 4, 30001, 880, 300, 0.202, 0.056, 0.031, 0.11, 0.28, 6, 5, 0.126, 0.072, 0.031, 0.842, 0.0042, 0.70, 0.205, 20, 0, 18, 16, 18, 16, 18, 0, 24, 12, 0.96, 0.98, "softleaf", "softleaf occluded hubs A: V29-D low-fragment skeleton plus V29-B-like attached occlusion shell and short-hub segment gate"),
        ("V30_sc_tree_short_hub_bark_transition_B", 5, 30002, 860, 304, 0.206, 0.054, 0.030, 0.12, 0.25, 7, 5, 0.118, 0.068, 0.030, 0.848, 0.0043, 0.71, 0.200, 18, 0, 16, 22, 18, 14, 16, 0, 18, 12, 0.98, 0.94, "braided", "short hub bark transition B: emphasizes non-round bark ridges and object-space bark-to-olive-to-leaf material transition without long exterior rods"),
        ("V30_sc_tree_lowdetail_connected_entry_C", 6, 30003, 840, 294, 0.200, 0.057, 0.031, 0.10, 0.30, 6, 5, 0.132, 0.076, 0.030, 0.846, 0.0044, 0.72, 0.198, 16, 0, 14, 16, 14, 14, 14, 0, 18, 10, 0.94, 0.96, "occlusion", "lowdetail connected entry C: sacrifices fine leaves for a thicker, lower-fragment connected entry shell to recover post-GLB r0 stability"),
        ("V30_sc_tree_broad_occlusion_shell_D", 7, 30004, 900, 292, 0.196, 0.058, 0.030, 0.10, 0.32, 6, 6, 0.136, 0.074, 0.030, 0.844, 0.0042, 0.70, 0.205, 18, 0, 22, 14, 16, 14, 16, 0, 22, 10, 0.92, 1.02, "softleaf", "broad occlusion shell D: six short hubs and broad attached bracts make the branch/crown seam read as crown interior rather than a material join"),
    ]
    rows: List[Dict] = []
    for (
        case_id,
        ...
        rows.append(
            _spec(
                ...
                gpu=gpu,
                seed=seed + offset,
                root_variant=case_id.replace("V30_", ""),
                parameter_variant=(
                    ...
                    f"_entry{entry_sheath_count}_shell{occlusion_shell_count}_ridge{ridge_count}"
                ),
                reason=reason,
            )
        )
...
def _metadata_for(spec: Dict, mesh_path: Path, guide_path: str, metrics: Dict) -> Dict:
    ...
    metadata["root_selection_log"]["root_source_type"] = "V30_sc_tree_occluded_hub_input_generator"
    metadata["root_selection_log"]["source_generator"] = "assets/strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511.py"
    metadata["v30_seam_naturalization_contract"] = {
        ...
        "base_evidence": "V30 uses V29-D as the lower-fragment skeleton and borrows V29-B's crown-occlusion read",
        "seam_mask": {
            "space": "object_xyz",
    ...
    metadata["v30_remote_cache_policy"] = {
...
def materialize(root: Path, out_dir: Path, seed: int = 20260511, case_limit: Optional[int] = None) -> Dict:
    ...
    for spec in specs:
        ...
        if metrics["largest_mesh_component_vertex_ratio"] < CONNECTIVITY_LCR_MIN:
            raise RuntimeError(f"{spec['case_id']} failed V30 connectivity gate: {metrics}")
        if float(spec["controls"].get("external_support_max_segment_after_subdivision", 999.0)) > EXTERNAL_SUPPORT_MAX_SEGMENT_GATE + 1e-6:
            raise RuntimeError(f"{spec['case_id']} failed V30 short-hub segment gate: {spec['controls']}")
        ...
        obj_zoom_cases.append(
            {
                ...
                "detail_target_source": "v30_object_space_seam_mask",
            }
        )
    (out_dir / "V30_obj_zoom_manifest_seamtarget_20260511.json").write_text(
    ...
    summary = {
        ...
        "storage_root": REMOTE_STORAGE_ROOT,
        "storage_limit_gb": STORAGE_LIMIT_GB,
        "surface_generator": "strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization",
        "mesh_input_policy": "obj_mesh_inputs_only",
        "mesh_pbr_policy": MESH_PBR_POLICY,
        "surface_strategy": SURFACE_STRATEGY,
        "connectivity_gate": {
        ...
        "obj_zoom_manifest": str(out_dir / "V30_obj_zoom_manifest_seamtarget_20260511.json"),
        "do_not_launch_remote": True,
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "README.md").write_text(
        "# V30 SC Tree Occluded-Hub Naturalization Dry Run\n\n"
        "Predeclared occluded short-hub branch/crown seam candidates. V30 uses "
