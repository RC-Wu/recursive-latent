...
RUN="${RUN:-strict_visual_matched_texture_v5_hybrid_20260510}"
SEED="${SEED:-20260510}"
INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_v5_hybrid_20260510}"
...
generate_inputs() {
  echo "=== GENERATE strict visual matched V5 hybrid OBJ inputs $(date)"
  "$PY" "$ROOT/assets/strict_visual_matched_cases_v5_hybrid_20260510.py" \
    --root "$ROOT" \
    --out "$INPUTS" \
    --seed "$SEED"
  "$PY" - "$INPUTS" <<'PY'
...
for gpu in 4 5 6 7; do
  nohup bash "$ROOT/assets/launch_strict_visual_matched_texture_v5_hybrid_20260510.sh" --worker "$gpu" \
    > "$LOG/gpu${gpu}_strict_visual_matched_v5_hybrid.log" 2>&1 &
  echo "gpu${gpu}_strict_visual_matched_v5_hybrid:$!"
done

sleep 5
nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | sed -n '5,8p'
for f in "$LOG"/gpu*_strict_visual_matched_v5_hybrid.log; do
  echo "==== $(basename "$f")"
  tail -12 "$f" || true
done
