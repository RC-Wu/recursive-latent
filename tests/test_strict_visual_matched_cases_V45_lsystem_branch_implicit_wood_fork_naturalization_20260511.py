...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511", SCRIPT)
...
def test_v45_lsystem_branch_materializes_implicit_wood_fork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization"
    assert summary["surface_strategy"] == "v45_lsystem_branch_implicit_wood_fork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_implicit_wood_fork_branches"] >= 3
    assert summary["lsystem_branch_gate"]["implicit_union_support_required"] is True
    assert summary["lsystem_branch_gate"]["mechanical_sleeve_disabled_required"] is True
    assert summary["lsystem_branch_gate"]["min_junction_implicit_unions"] >= 3
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V45_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V45_lsys_branch_implicit_wood_fork_A",
        "V45_lsys_branch_implicit_wood_fork_lowfrag_B",
        "V45_lsys_branch_implicit_wood_fork_dense_C",
        "V45_lsys_branch_implicit_wood_fork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v45_lsystem_branch_implicit_wood_fork_sidebranch_naturalization" for row in rows)
    ...
    assert all(int(row["implicit_wood_fork_branch_count"]) >= 3 for row in rows)
    assert all(row["implicit_union_support"] == "true" for row in rows)
    ...
    assert all(int(row["junction_implicit_union_count"]) >= 3 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511.py"
        assert metadata["v45_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v45_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v45_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v45_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["implicit_union"]["enabled"] is True
        assert contract["implicit_union"]["mechanical_sleeve_disabled"] is True
        assert contract["implicit_union"]["direct_axis_sleeve_count"] == 0
        assert contract["implicit_union"]["junction_implicit_union_count"] >= 3
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("implicit-wood-fork")
        ...
        assert controls["v45_lsystem_branch_implicit_wood_fork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch implicit-wood-fork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_implicit_wood_fork_bands_only"
        ...
        assert controls["implicit_union_support"] is True
        assert controls["mechanical_sleeve_disabled"] is True
        assert controls["direct_axis_sleeve_count"] == 0
        assert controls["junction_implicit_union_count"] >= 3
        ...
        assert controls["implicit_wood_fork_branch_count"] >= 3
        assert len(controls["implicit_wood_fork_anchor_indices"]) >= 2
        ...
        assert controls["implicit_wood_fork_zoom_pair_count"] >= 3
        assert controls["junction_zoom_pairs"][0] == controls["implicit_wood_fork_zoom_pairs"][0]
        ...
        assert controls["hard_tube_cap_mitigation"].startswith("generator-native implicit union")
    ...
    zoom_manifest = json.loads((tmp_path / "V45_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v45_lsystem_explicit_implicit_wood_fork_mask" for item in zoom_manifest["cases"])
...
def test_v45_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511}"' in text
    assert "for gpu in ${V45_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V45_lsystem_branch_implicit_wood_fork_naturalization_20260511.py" in text
