...
SCRIPT = Path(__file__).resolve().parents[1] / "assets" / "strict_visual_matched_cases_v13_smooth_coral_crystal_20260510.py"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_v13_smooth_coral_crystal_20260510", SCRIPT)
...
def test_v13_materializes_implicit_single_component_coral_crystal_inputs(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "implicit_metaball_marching_cubes"
    ...
    for row in rows:
        ...
        assert "implicit_metaball_union" in row["operator_composition"]
        ...
        assert metadata["root_selection_log"]["root_source_type"] == "v13_implicit_smooth_coral_crystal_input_generator"
        ...
        assert controls["generated_nodes"] >= 120
        assert controls["direct_tube_mesh"] is False
        assert controls["implicit_grid_resolution"] >= 68
        assert controls["metaball_sample_count"] >= 900
        assert controls["rounded_terminal_lobe_count"] >= 18
        assert controls["ridge_line_count"] >= 28
        assert controls["subtractive_pore_count"] >= 16
        assert "marching cubes" in controls["surface_strategy"]
        assert "tube" in controls["v13_failure_addressed"]
        ...
        assert "round cylinder rods" in contract["v12_failure_addressed"]
        assert "implicit field" in contract["frontier_structure"]
        assert "detached shards" in contract["negative_constraint"]
        ...
        assert m["curved_branch_edges"] >= 100
        assert m["ridge_fin_count"] >= 28
        assert m["perforated_membrane_count"] >= 16
