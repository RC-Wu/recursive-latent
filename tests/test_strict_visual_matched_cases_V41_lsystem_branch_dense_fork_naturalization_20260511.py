...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V41_lsystem_branch_dense_fork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511", SCRIPT)
...
def test_v41_lsystem_branch_materializes_dense_primary_secondary_fork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization"
    assert summary["surface_strategy"] == "v41_lsystem_branch_dense_fork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_branch_junctions"] >= 9
    assert summary["lsystem_branch_gate"]["min_saddle_necks"] >= 16
    assert summary["lsystem_branch_gate"]["min_terminal_forks"] >= 4
    assert summary["lsystem_branch_gate"]["min_secondary_side_branches"] >= 8
    assert summary["lsystem_branch_gate"]["min_recursive_side_branch_events"] >= 12
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V41_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V41_lsys_branch_dense_fork_A",
        "V41_lsys_branch_dense_fork_lowfrag_B",
        "V41_lsys_branch_dense_fork_dense_C",
        "V41_lsys_branch_dense_fork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v41_lsystem_branch_dense_fork_sidebranch_naturalization" for row in rows)
    assert all(row["block_or_token_stamping"] == "false" for row in rows)
    assert all(int(row["branch_junction_count"]) >= 9 for row in rows)
    assert all(int(row["saddle_neck_count"]) >= 16 for row in rows)
    ...
    assert all(int(row["ridge_count"]) >= 84 for row in rows)
    assert all(int(row["secondary_side_branch_count"]) >= 8 for row in rows)
    assert all(int(row["recursive_side_branch_event_count"]) >= 12 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511.py"
        assert metadata["v41_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v41_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v41_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v41_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("dense primary/secondary fork")
        ...
        assert controls["v41_lsystem_branch_dense_fork_naturalization"] is True
        assert controls["v41_lsystem_branch_primary_fork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "dense L-system saddle-neck primary and secondary side-branch fork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_dense_primary_secondary_side_branch_fork_bands_only"
        ...
        assert controls["terminal_fork_count"] >= 4
        ...
        assert controls["primary_fork_zoom_pair_count"] == 1
        assert controls["primary_fork_anchor_index"] >= 0
        assert controls["secondary_side_branch_count"] >= 8
        assert controls["secondary_fork_zoom_pair_count"] >= 2
        assert controls["recursive_side_branch_event_count"] >= 12
        assert len(controls["secondary_fork_anchor_indices"]) >= 2
        assert controls["junction_zoom_pairs"][0] == controls["primary_fork_zoom_pairs"][0]
        ...
        assert 120 <= m["semantic_detail_count"] <= 520
        assert m["branch_junction_count"] >= 9
        assert m["saddle_neck_count"] >= 16
        assert m["terminal_fork_count"] >= 4
        assert m["terminal_sleeve_count"] == 0
        assert m["junction_zoom_target_count"] > 0
        assert m["secondary_side_branch_count"] >= 8
        assert m["recursive_side_branch_event_count"] >= 12
    ...
    zoom_manifest = json.loads((tmp_path / "V41_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v41_lsystem_dense_primary_secondary_fork_mask" for item in zoom_manifest["cases"])
...
def test_v41_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V41_lsystem_branch_dense_fork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511}"' in text
    assert "for gpu in ${V41_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V41_lsystem_branch_dense_fork_naturalization_20260511.py" in text
