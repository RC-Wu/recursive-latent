...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V53_lsystem_branch_natural_bough_graph_yfork_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511", SCRIPT)
...
def test_v53_lsystem_branch_materializes_highres_smooth_yfork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_naturalization"
    assert summary["surface_strategy"] == "v53_lsystem_branch_natural_bough_graph_yfork_sidebranch_naturalization"
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V53_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V53_lsys_branch_natural_bough_graph_yfork_A",
        "V53_lsys_branch_natural_bough_graph_yfork_lowfrag_B",
        "V53_lsys_branch_natural_bough_graph_yfork_dense_C",
        "V53_lsys_branch_natural_bough_graph_yfork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v53_lsystem_branch_natural_bough_graph_yfork_sidebranch_naturalization" for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511.py"
        assert metadata["v53_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v53_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v53_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v53_lsystem_branch_naturalization_contract"]
        ...
        assert controls["v53_lsystem_branch_natural_bough_graph_yfork_naturalization"] is True
        ...
        assert "distributed side branches" in controls["implicit_capsule_topology"]
    ...
    zoom_manifest = json.loads((tmp_path / "V53_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v53_lsystem_explicit_highres_smooth_yfork_mask" for item in zoom_manifest["cases"])
    for item in zoom_manifest["cases"]:
        assert 2.4 <= item["zoom_divisor"] <= 2.9
...
def test_v53_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V53_lsystem_branch_natural_bough_graph_yfork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511}"' in text
    assert "for gpu in ${V53_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V53_lsystem_branch_natural_bough_graph_yfork_20260511.py" in text
