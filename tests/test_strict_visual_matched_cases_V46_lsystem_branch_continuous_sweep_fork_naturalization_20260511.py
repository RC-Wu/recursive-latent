...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511", SCRIPT)
...
def test_v46_lsystem_branch_materializes_continuous_sweep_fork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization"
    assert summary["surface_strategy"] == "v46_lsystem_branch_continuous_sweep_fork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_continuous_sweep_fork_branches"] >= 3
    assert summary["lsystem_branch_gate"]["continuous_sweep_support_required"] is True
    assert summary["lsystem_branch_gate"]["mechanical_sleeve_disabled_required"] is True
    assert summary["lsystem_branch_gate"]["min_junction_continuous_sweeps"] >= 3
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V46_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V46_lsys_branch_continuous_sweep_fork_A",
        "V46_lsys_branch_continuous_sweep_fork_lowfrag_B",
        "V46_lsys_branch_continuous_sweep_fork_dense_C",
        "V46_lsys_branch_continuous_sweep_fork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v46_lsystem_branch_continuous_sweep_fork_sidebranch_naturalization" for row in rows)
    ...
    assert all(int(row["continuous_sweep_fork_branch_count"]) >= 3 for row in rows)
    assert all(row["continuous_sweep_support"] == "true" for row in rows)
    ...
    assert all(int(row["junction_continuous_sweep_count"]) >= 3 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511.py"
        assert metadata["v46_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v46_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v46_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v46_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["continuous_sweep"]["enabled"] is True
        assert contract["continuous_sweep"]["mechanical_sleeve_disabled"] is True
        assert contract["continuous_sweep"]["direct_axis_sleeve_count"] == 0
        assert contract["continuous_sweep"]["junction_continuous_sweep_count"] >= 3
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("continuous-sweep-fork")
        ...
        assert controls["v46_lsystem_branch_continuous_sweep_fork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch continuous-sweep-fork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_continuous_sweep_fork_bands_only"
        ...
        assert controls["continuous_sweep_support"] is True
        assert controls["mechanical_sleeve_disabled"] is True
        assert controls["direct_axis_sleeve_count"] == 0
        assert controls["junction_continuous_sweep_count"] >= 3
        ...
        assert controls["continuous_sweep_fork_branch_count"] >= 3
        assert len(controls["continuous_sweep_fork_anchor_indices"]) >= 2
        ...
        assert controls["continuous_sweep_fork_zoom_pair_count"] >= 3
        assert controls["junction_zoom_pairs"][0] == controls["continuous_sweep_fork_zoom_pairs"][0]
        ...
        assert controls["hard_tube_cap_mitigation"].startswith("generator-native continuous sweep")
    ...
    zoom_manifest = json.loads((tmp_path / "V46_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v46_lsystem_explicit_continuous_sweep_fork_mask" for item in zoom_manifest["cases"])
...
def test_v46_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511}"' in text
    assert "for gpu in ${V46_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V46_lsystem_branch_continuous_sweep_fork_naturalization_20260511.py" in text
