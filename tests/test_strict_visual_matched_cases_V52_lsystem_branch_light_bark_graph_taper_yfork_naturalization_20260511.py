...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511", SCRIPT)
...
def test_v52_lsystem_branch_materializes_highres_smooth_yfork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization"
    assert summary["surface_strategy"] == "v52_lsystem_branch_light_bark_graph_taper_yfork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_highres_smooth_yfork_branches"] >= 3
    ...
    assert summary["lsystem_branch_gate"]["min_graph_terminal_tapers"] > 0
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V52_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V52_lsys_branch_light_bark_graph_taper_yfork_A",
        "V52_lsys_branch_light_bark_graph_taper_yfork_lowfrag_B",
        "V52_lsys_branch_light_bark_graph_taper_yfork_dense_C",
        "V52_lsys_branch_light_bark_graph_taper_yfork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v52_lsystem_branch_light_bark_graph_taper_yfork_sidebranch_naturalization" for row in rows)
    ...
    assert all(int(row["highres_smooth_yfork_branch_count"]) >= 3 for row in rows)
    ...
    assert all(int(row["ridge_count"]) == 1 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511.py"
        assert metadata["v52_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v52_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v52_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v52_lsystem_branch_naturalization_contract"]
        ...
        assert contract["implicit_capsule"]["terminal_taper_count"] > 0
        assert contract["implicit_capsule"]["graph_terminal_taper_count"] > 0
        assert contract["implicit_capsule"]["field_terminal_taper_count"] == 0
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("highres-smooth-yfork")
        ...
        assert controls["v52_lsystem_branch_light_bark_graph_taper_yfork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch highres-smooth-yfork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_highres_smooth_yfork_bands_only"
        ...
        assert controls["terminal_taper_count"] > 0
        assert controls["graph_terminal_taper_count"] > 0
        assert controls["field_terminal_taper_count"] == 0
        assert controls["terminal_taper_source"].startswith("graph_native_three_step")
        assert controls["implicit_grid_resolution"] >= 224
        assert 0.58 <= controls["gaussian_sigma"] <= 0.62
        assert 0.38 <= controls["implicit_field_level"] <= 0.45
        ...
        assert controls["highres_smooth_yfork_branch_count"] >= 3
        ...
        assert len(controls["highres_smooth_yfork_anchor_indices"]) >= 2
        assert controls["terminal_sleeve_count"] == 0
        assert controls["terminal_bud_count"] == 0
        assert controls["ridge_count"] == 1
        assert controls["junction_zoom_target_count"] > 0
        assert controls["junction_zoom_pair_count"] > 0
        assert controls["highres_smooth_yfork_zoom_pair_count"] >= 3
        assert controls["junction_zoom_pairs"][0] == controls["highres_smooth_yfork_zoom_pairs"][0]
        ...
        assert "pale low-contrast bark guide" in controls["hard_tube_cap_mitigation"]
        ...
        assert m["terminal_taper_count"] > 0
        assert m["graph_terminal_taper_count"] > 0
        assert m["field_terminal_taper_count"] == 0
        assert m["ridge_count"] == 1
        assert m["branch_junction_count"] >= 4
        assert m["saddle_neck_count"] > 0
        assert m["highres_smooth_yfork_branch_count"] >= 3
    ...
    zoom_manifest = json.loads((tmp_path / "V52_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v52_lsystem_explicit_highres_smooth_yfork_mask" for item in zoom_manifest["cases"])
    for item in zoom_manifest["cases"]:
        assert item["zoom_divisor"] >= 3.8
...
def test_v52_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511}"' in text
    assert "for gpu in ${V52_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V52_lsystem_branch_light_bark_graph_taper_yfork_naturalization_20260511.py" in text
