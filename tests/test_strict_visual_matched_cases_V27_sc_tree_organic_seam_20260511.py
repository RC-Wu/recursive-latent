...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V27_sc_tree_organic_seam_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V27_sc_tree_organic_seam_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V27_sc_tree_organic_seam_20260511", SCRIPT)
...
def test_v27_sc_tree_organic_seam_materializes_four_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V27_sc_tree_organic_seam"
    assert summary["surface_strategy"] == "v27_sc_tree_terminal_bud_and_junction_feather_masked_naturalization"
    ...
    assert summary["organic_seam_gate"]["mask_scope"] == "branch_crown_junction_band_and_terminal_caps_only"
    assert summary["organic_seam_gate"]["min_seam_mask_centers"] > 0
    assert summary["organic_seam_gate"]["min_terminal_buds"] > 0
    assert summary["organic_seam_gate"]["min_junction_feathers"] > 0
    assert summary["organic_seam_gate"]["organic_guides"] is True
    ...
    assert all(row["mesh_pbr_policy"] == "obj_inputs_organic_tree_guides_for_trellis2_glb_export_plus_object_space_pbr_qa" for row in rows)
    assert all(row["surface_strategy"] == "v27_sc_tree_terminal_bud_and_junction_feather_masked_naturalization" for row in rows)
    assert all(row["block_or_token_stamping"] == "false" for row in rows)
    assert all(row["selection_budget"] == "four_predeclared_sc_tree_organic_seam_candidates_one_per_gpu" for row in rows)
    assert all(int(row["seam_mask_center_count"]) > 0 for row in rows)
    assert all(int(row["terminal_bud_count"]) > 0 for row in rows)
    assert all(int(row["junction_feather_count"]) > 0 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V27_sc_tree_organic_seam_20260511.py"
        assert metadata["v27_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v27_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v27_organic_seam_contract"]["mask_scope"] == "branch_crown_junction_band_and_terminal_caps_only"
        assert metadata["v27_organic_seam_contract"]["seam_mask_center_count"] > 0
        assert metadata["v27_organic_seam_contract"]["terminal_bud_count"] > 0
        assert metadata["v27_organic_seam_contract"]["junction_feather_count"] > 0
        assert metadata["v27_organic_seam_contract"]["texture_operator"].startswith("organic")

        controls = metadata["controls"]
        assert controls["v27_sc_tree_organic_seam"] is True
        ...
        assert controls["junction_feather_count"] > 0
        assert controls["hard_tube_cap_mitigation"] == "terminal buds plus junction feather twigs/leaves"

        m = metrics_by_case[row["case_id"]]
        assert m["vertices"] > 4500
        assert m["faces"] > 9000
        assert m["mesh_component_count"] == 1
        assert m["largest_mesh_component_vertex_ratio"] >= 0.999
        assert m["support_edge_count"] >= 20
        assert m["semantic_detail_count"] >= 180
        assert m["seam_mask_center_count"] > 0
        assert m["terminal_bud_count"] > 0
        assert m["junction_feather_count"] > 0
...
def test_v27_launcher_is_generate_only_safe_uses_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V27_sc_tree_organic_seam_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V27_sc_tree_organic_seam_20260511}"' in text
    ...
    assert "strict_visual_matched_cases_V27_sc_tree_organic_seam_20260511.py" in text
