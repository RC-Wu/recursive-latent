...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V42_lsystem_branch_cluster_fork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511", SCRIPT)
...
def test_v42_lsystem_branch_materializes_cluster_fork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization"
    assert summary["surface_strategy"] == "v42_lsystem_branch_cluster_fork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_cluster_fork_branches"] >= 3
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V42_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V42_lsys_branch_cluster_fork_A",
        "V42_lsys_branch_cluster_fork_lowfrag_B",
        "V42_lsys_branch_cluster_fork_dense_C",
        "V42_lsys_branch_cluster_fork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v42_lsystem_branch_cluster_fork_sidebranch_naturalization" for row in rows)
    ...
    assert all(int(row["cluster_fork_branch_count"]) >= 3 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511.py"
        assert metadata["v42_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v42_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v42_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v42_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("cluster-fork")
        ...
        assert controls["v42_lsystem_branch_cluster_fork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch cluster-fork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_cluster_fork_bands_only"
        ...
        assert controls["cluster_fork_branch_count"] >= 3
        assert len(controls["cluster_fork_anchor_indices"]) >= 2
        ...
        assert controls["cluster_fork_zoom_pair_count"] >= 3
        assert controls["junction_zoom_pairs"][0] == controls["cluster_fork_zoom_pairs"][0]
        ...
        assert 80 <= m["semantic_detail_count"] <= 420
        assert m["branch_junction_count"] >= 4
        assert m["saddle_neck_count"] > 0
        assert m["cluster_fork_branch_count"] >= 3
    ...
    zoom_manifest = json.loads((tmp_path / "V42_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v42_lsystem_explicit_cluster_fork_mask" for item in zoom_manifest["cases"])
...
def test_v42_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V42_lsystem_branch_cluster_fork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511}"' in text
    assert "for gpu in ${V42_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V42_lsystem_branch_cluster_fork_naturalization_20260511.py" in text
