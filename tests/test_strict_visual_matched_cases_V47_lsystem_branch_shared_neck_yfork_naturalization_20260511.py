...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511", SCRIPT)
...
def test_v47_lsystem_branch_materializes_shared_neck_yfork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization"
    assert summary["surface_strategy"] == "v47_lsystem_branch_shared_neck_yfork_sidebranch_naturalization"
    ...
    assert summary["lsystem_branch_gate"]["min_shared_neck_yfork_branches"] >= 3
    assert summary["lsystem_branch_gate"]["min_shared_neck_inserted"] >= 3
    assert summary["lsystem_branch_gate"]["shared_neck_support_required"] is True
    assert summary["lsystem_branch_gate"]["mechanical_sleeve_disabled_required"] is True
    assert summary["lsystem_branch_gate"]["min_junction_shared_necks"] >= 3
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V47_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V47_lsys_branch_shared_neck_yfork_A",
        "V47_lsys_branch_shared_neck_yfork_lowfrag_B",
        "V47_lsys_branch_shared_neck_yfork_dense_C",
        "V47_lsys_branch_shared_neck_yfork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v47_lsystem_branch_shared_neck_yfork_sidebranch_naturalization" for row in rows)
    ...
    assert all(int(row["shared_neck_yfork_branch_count"]) >= 3 for row in rows)
    assert all(int(row["shared_neck_inserted_count"]) >= 3 for row in rows)
    assert all(row["shared_neck_support"] == "true" for row in rows)
    ...
    assert all(int(row["junction_shared_neck_count"]) >= 3 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511.py"
        assert metadata["v47_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v47_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v47_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v47_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["shared_neck"]["enabled"] is True
        assert contract["shared_neck"]["mechanical_sleeve_disabled"] is True
        assert contract["shared_neck"]["direct_axis_sleeve_count"] == 0
        assert contract["shared_neck"]["junction_shared_neck_count"] >= 3
        assert contract["shared_neck"]["shared_neck_inserted_count"] >= 3
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("shared-neck-yfork")
        ...
        assert controls["v47_lsystem_branch_shared_neck_yfork_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch shared-neck-yfork bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_shared_neck_yfork_bands_only"
        ...
        assert controls["shared_neck_yfork_branch_count"] >= 3
        assert controls["shared_neck_inserted_count"] >= 3
        assert controls["shared_neck_topology"].startswith("main branch continuation")
        assert len(controls["shared_neck_yfork_anchor_indices"]) >= 2
        ...
        assert controls["shared_neck_yfork_zoom_pair_count"] >= 3
        assert controls["junction_zoom_pairs"][0] == controls["shared_neck_yfork_zoom_pairs"][0]
        ...
        assert controls["hard_tube_cap_mitigation"].startswith("generator-native shared neck")
    ...
    zoom_manifest = json.loads((tmp_path / "V47_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v47_lsystem_explicit_shared_neck_yfork_mask" for item in zoom_manifest["cases"])
...
def test_v47_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511}"' in text
    assert "for gpu in ${V47_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V47_lsystem_branch_shared_neck_yfork_naturalization_20260511.py" in text
