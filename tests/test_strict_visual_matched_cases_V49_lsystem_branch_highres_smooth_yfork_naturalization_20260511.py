...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511", SCRIPT)
...
def test_v49_lsystem_branch_materializes_highres_smooth_yfork_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization"
    assert summary["surface_strategy"] == "v49_lsystem_branch_highres_smooth_yfork_sidebranch_naturalization"
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V49_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V49_lsys_branch_highres_smooth_yfork_A",
        "V49_lsys_branch_highres_smooth_yfork_lowfrag_B",
        "V49_lsys_branch_highres_smooth_yfork_dense_C",
        "V49_lsys_branch_highres_smooth_yfork_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v49_lsystem_branch_highres_smooth_yfork_sidebranch_naturalization" for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511.py"
        assert metadata["v49_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v49_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v49_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v49_lsystem_branch_naturalization_contract"]
        ...
        assert controls["v49_lsystem_branch_highres_smooth_yfork_naturalization"] is True
        ...
        assert controls["implicit_grid_resolution"] >= 160
        assert 0.45 <= controls["gaussian_sigma"] <= 0.60
        assert 0.34 <= controls["implicit_field_level"] <= 0.45
    ...
    zoom_manifest = json.loads((tmp_path / "V49_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v49_lsystem_explicit_highres_smooth_yfork_mask" for item in zoom_manifest["cases"])
...
def test_v49_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511}"' in text
    assert "for gpu in ${V49_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V49_lsystem_branch_highres_smooth_yfork_naturalization_20260511.py" in text
