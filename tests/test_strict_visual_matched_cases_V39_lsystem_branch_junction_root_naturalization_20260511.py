...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V39_lsystem_branch_junction_root_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511", SCRIPT)
...
def test_v39_lsystem_branch_materializes_junction_root_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization"
    assert summary["surface_strategy"] == "v39_lsystem_branch_junction_root_sidebranch_naturalization"
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V39_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V39_lsys_branch_junction_root_A",
        "V39_lsys_branch_junction_root_lowfrag_B",
        "V39_lsys_branch_junction_root_dense_C",
        "V39_lsys_branch_junction_root_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v39_lsystem_branch_junction_root_sidebranch_naturalization" for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511.py"
        assert metadata["v39_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v39_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v39_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v39_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("junction-root")
        ...
        assert controls["v39_lsystem_branch_junction_root_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch junction-root bands with wood-only tapered terminals"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_junction_root_bands_only"
    ...
    zoom_manifest = json.loads((tmp_path / "V39_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v39_lsystem_object_space_junction_mask" for item in zoom_manifest["cases"])
...
def test_v39_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V39_lsystem_branch_junction_root_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511}"' in text
    assert "for gpu in ${V39_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V39_lsystem_branch_junction_root_naturalization_20260511.py" in text
