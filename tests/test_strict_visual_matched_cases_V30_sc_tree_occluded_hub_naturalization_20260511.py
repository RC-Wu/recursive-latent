...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V30_sc_tree_occluded_hub_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511", SCRIPT)
...
def test_v30_sc_tree_occluded_hub_materializes_four_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization"
    assert summary["surface_strategy"] == "v30_sc_tree_occluded_short_hub_masked_naturalization"
    ...
    assert summary["occluded_hub_gate"]["mask_scope"] == "object_space_occluded_short_hub_junction_band_and_terminal_caps_only"
    assert summary["occluded_hub_gate"]["min_seam_mask_centers"] > 0
    assert summary["occluded_hub_gate"]["min_entry_sheaths"] > 0
    assert summary["occluded_hub_gate"]["min_hidden_hubs"] >= 5
    assert summary["occluded_hub_gate"]["max_external_support_segment_after_subdivision"] <= 0.220001
    assert summary["occluded_hub_gate"]["sdedit_seam_backprojection_available"] is False
    assert summary["occluded_hub_gate"]["image_generation_considered"] is True
    ...
    expected_cases = {
        "V30_sc_tree_softleaf_occluded_hubs_A",
        "V30_sc_tree_short_hub_bark_transition_B",
        "V30_sc_tree_lowdetail_connected_entry_C",
        "V30_sc_tree_broad_occlusion_shell_D",
    }
    ...
    assert all(row["mesh_pbr_policy"] == "obj_inputs_occluded_short_hub_transition_guides_for_trellis2_glb_export_no_2d_backprojection_claim" for row in rows)
    assert all(row["surface_strategy"] == "v30_sc_tree_occluded_short_hub_masked_naturalization" for row in rows)
    assert all(row["block_or_token_stamping"] == "false" for row in rows)
    assert all(row["selection_budget"] == "four_predeclared_sc_tree_occluded_hub_candidates_one_per_gpu" for row in rows)
    ...
    assert all(float(row["external_support_max_segment_after_subdivision"]) <= 0.220001 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511.py"
        assert metadata["v30_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v30_remote_cache_policy"]["no_system_tmp"] is True

        contract = metadata["v30_seam_naturalization_contract"]
        ...
        assert "V29-D" in contract["base_evidence"]

        controls = metadata["controls"]
        assert controls["v30_sc_tree_occluded_hub_naturalization"] is True
        assert controls["removed_visible_low_depth_rod"] is True
        assert controls["short_hub_gate_pass"] is True
        assert controls["masked_local_naturalization_target"] == "occluded short-hub branch/crown entry band and terminal caps"
        assert controls["mask_scope"] == "object_space_occluded_short_hub_junction_band_and_terminal_caps_only"
        ...
        assert controls["external_support_max_segment_after_subdivision"] <= 0.220001
        assert controls["hard_tube_cap_mitigation"].startswith("occluded short-hub")
        ...
        assert 110 <= m["semantic_detail_count"] <= 205
        ...
        assert m["external_support_max_segment_after_subdivision"] <= 0.220001
...
def test_v30_launcher_is_generate_only_safe_uses_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V30_sc_tree_occluded_hub_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511}"' in text
    ...
    assert "strict_visual_matched_cases_V30_sc_tree_occluded_hub_naturalization_20260511.py" in text
