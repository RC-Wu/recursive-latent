...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V28_sc_tree_flare_bark_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization_20260511", SCRIPT)
...
def test_v28_sc_tree_flare_bark_materializes_four_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization"
    assert summary["surface_strategy"] == "v28_sc_tree_compact_flare_bark_ridge_masked_naturalization"
    ...
    assert summary["flare_bark_gate"]["mask_scope"] == "branch_crown_junction_band_and_terminal_caps_only"
    assert summary["flare_bark_gate"]["min_seam_mask_centers"] > 0
    assert summary["flare_bark_gate"]["min_terminal_buds"] > 0
    assert summary["flare_bark_gate"]["min_junction_flares"] > 0
    assert summary["flare_bark_gate"]["min_bark_ridges"] > 0
    ...
    expected_cases = {
        "V28_sc_tree_branch_flare_A",
        "V28_sc_tree_bark_ridge_B",
        "V28_sc_tree_multiscale_canopy_C",
        "V28_sc_tree_hybrid_flare_bud_D",
    }
    ...
    assert all(row["mesh_pbr_policy"] == "obj_inputs_compact_flare_tree_guides_for_trellis2_glb_export_plus_object_space_pbr_qa" for row in rows)
    assert all(row["surface_strategy"] == "v28_sc_tree_compact_flare_bark_ridge_masked_naturalization" for row in rows)
    assert all(row["block_or_token_stamping"] == "false" for row in rows)
    assert all(row["selection_budget"] == "four_predeclared_sc_tree_flare_bark_candidates_one_per_gpu" for row in rows)
    ...
    assert all(int(row["structural_split_count"]) > 0 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization_20260511.py"
        assert metadata["v28_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v28_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v28_flare_bark_contract"]["target_failure"].startswith("V27 GLB")
        assert metadata["v28_flare_bark_contract"]["seam_mask_center_count"] > 0
        assert metadata["v28_flare_bark_contract"]["terminal_bud_count"] > 0
        assert metadata["v28_flare_bark_contract"]["junction_flare_count"] > 0
        assert metadata["v28_flare_bark_contract"]["ridge_count"] > 0
        assert metadata["v28_flare_bark_contract"]["texture_operator"].startswith("low-fragment")

        controls = metadata["controls"]
        assert controls["v28_sc_tree_flare_bark_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "tree branch/crown junction band and terminal caps"
        ...
        assert controls["hard_tube_cap_mitigation"].startswith("compact flare")
        ...
        assert 120 <= m["semantic_detail_count"] <= 230
...
def test_v28_launcher_is_generate_only_safe_uses_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V28_sc_tree_flare_bark_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization_20260511}"' in text
    ...
    assert "strict_visual_matched_cases_V28_sc_tree_flare_bark_naturalization_20260511.py" in text
