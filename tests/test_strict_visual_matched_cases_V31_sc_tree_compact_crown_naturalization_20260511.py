...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V31_sc_tree_compact_crown_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511", SCRIPT)
...
def test_v31_sc_tree_compact_crown_materializes_four_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization"
    assert summary["surface_strategy"] == "v31_sc_tree_compact_crown_masked_naturalization"
    ...
    assert summary["compact_crown_gate"]["mask_scope"] == "object_space_compact_crown_entry_band_and_terminal_caps_only"
    assert summary["compact_crown_gate"]["min_seam_mask_centers"] > 0
    assert summary["compact_crown_gate"]["min_entry_sheaths"] > 0
    assert summary["compact_crown_gate"]["min_hidden_hubs"] >= 5
    assert summary["compact_crown_gate"]["max_external_support_segment_after_subdivision"] <= 0.180001
    assert summary["compact_crown_gate"]["sdedit_seam_backprojection_available"] is False
    assert summary["compact_crown_gate"]["image_generation_considered"] is True
    ...
    expected_cases = {
        "V31_sc_tree_compact_entry_A",
        "V31_sc_tree_crown_mass_B",
        "V31_sc_tree_lowfrag_shell_C",
        "V31_sc_tree_balanced_leafmass_D",
    }
    ...
    assert all(row["mesh_pbr_policy"] == "obj_inputs_compact_crown_transition_guides_for_trellis2_glb_export_no_2d_backprojection_claim" for row in rows)
    assert all(row["surface_strategy"] == "v31_sc_tree_compact_crown_masked_naturalization" for row in rows)
    assert all(row["block_or_token_stamping"] == "false" for row in rows)
    assert all(row["selection_budget"] == "four_predeclared_sc_tree_compact_crown_candidates_one_per_gpu" for row in rows)
    ...
    assert all(float(row["external_support_max_segment_after_subdivision"]) <= 0.180001 for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511.py"
        assert metadata["v31_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v31_remote_cache_policy"]["no_system_tmp"] is True

        contract = metadata["v31_seam_naturalization_contract"]
        ...
        assert controls["v31_sc_tree_compact_crown_naturalization"] is True
        assert controls["removed_visible_low_depth_rod"] is True
        assert controls["short_hub_gate_pass"] is True
        assert controls["masked_local_naturalization_target"] == "compact crown-entry mass and terminal caps"
        assert controls["mask_scope"] == "object_space_compact_crown_entry_band_and_terminal_caps_only"
        ...
        assert controls["external_support_max_segment_after_subdivision"] <= 0.180001
        assert controls["hard_tube_cap_mitigation"].startswith("compact crown-entry")
        ...
        assert 80 <= m["semantic_detail_count"] <= 180
        ...
        assert m["external_support_max_segment_after_subdivision"] <= 0.180001
...
def test_v31_launcher_is_generate_only_safe_uses_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V31_sc_tree_compact_crown_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511}"' in text
    ...
    assert "strict_visual_matched_cases_V31_sc_tree_compact_crown_naturalization_20260511.py" in text
