...
SCRIPT = ROOT / "assets" / "strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization_20260511.py"
LAUNCHER = ROOT / "assets" / "launch_strict_visual_matched_texture_V38_lsystem_branch_tipless_saddle_naturalization_20260511.sh"


def load_module():
    spec = importlib.util.spec_from_file_location("strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization_20260511", SCRIPT)
...
def test_v38_lsystem_branch_materializes_tipless_saddle_case_manifest_and_metrics(tmp_path):
    mod = load_module()
    ...
    assert summary["surface_generator"] == "strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization"
    assert summary["surface_strategy"] == "v38_lsystem_branch_tipless_sidebranch_saddle_naturalization"
    ...
    for name in (
        ...
        "README.md",
        "summary.json",
        "V38_obj_zoom_manifest_junctiontarget_20260511.json",
    ):
    ...
    expected_cases = {
        "V38_lsys_branch_tipless_saddle_A",
        "V38_lsys_branch_tipless_midfork_B",
        "V38_lsys_branch_tipless_densefork_C",
        "V38_lsys_branch_tipless_slim_D",
    }
    ...
    assert all(row["surface_strategy"] == "v38_lsystem_branch_tipless_sidebranch_saddle_naturalization" for row in rows)
    ...
    for row in rows:
        ...
        assert metadata["root_selection_log"]["source_generator"] == "assets/strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization_20260511.py"
        assert metadata["v38_remote_cache_policy"]["cache_root"].endswith("/cache")
        assert metadata["v38_remote_cache_policy"]["no_system_tmp"] is True
        assert metadata["v38_remote_cache_policy"]["max_simultaneous_gpus"] == 2

        contract = metadata["v38_lsystem_branch_naturalization_contract"]
        assert contract["target_failure"].startswith("V35 remained connected")
        assert contract["seam_mask"]["space"] == "object_xyz"
        assert contract["zoom_targets"]["source"].startswith("junction-only")
        ...
        assert controls["v38_lsystem_branch_tipless_saddle_naturalization"] is True
        assert controls["masked_local_naturalization_target"] == "L-system saddle-neck side-branch junction bands plus tiny terminal sleeves"
        assert controls["mask_scope"] == "object_space_side_branch_saddle_junction_bands_and_tiny_terminal_caps_only"
        ...
        assert controls["hard_tube_cap_mitigation"].startswith("tipless main-axis fork")
    ...
    zoom_manifest = json.loads((tmp_path / "V38_obj_zoom_manifest_junctiontarget_20260511.json").read_text(encoding="utf-8"))
    assert len(zoom_manifest["cases"]) == 4
    assert all(item["detail_target_source"] == "v38_lsystem_object_space_junction_mask" for item in zoom_manifest["cases"])


def test_v38_launcher_is_generate_only_safe_uses_two_default_gpu4567_and_project_cache():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert 'ROOT="${ROOT:-/mnt/beegfs/ruocheng/recursive_3d_generative_growth_20260507}"' in text
    assert 'RUN="${RUN:-strict_visual_matched_texture_V38_lsystem_branch_tipless_saddle_naturalization_20260511}"' in text
    assert 'INPUT_NAME="${INPUT_NAME:-strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization_20260511}"' in text
    assert "for gpu in ${V38_GPUS:-4 5}" in text
    ...
    assert "strict_visual_matched_cases_V38_lsystem_branch_tipless_saddle_naturalization_20260511.py" in text
